	...
	public function __construct() {
		...
		if($this->in->exists('login') || $this->in->exists('logout')){
			...
			if ( $this->in->exists('login') && ($this->user->data['user_id'] <= 0) ){
				$redirect	= ( $this->in->exists('redirect') ) ? $this->in->get('redirect') : 'index.php';
			...
			if($this->in->get('splash')) redirect('maintenance/task_manager.php?splash=true');
			$redirect_url = ( $this->in->exists('redirect') ) ? preg_replace('#^.*?redirect=(.+?)&(.+?)$#', '\\1' . $this->SID . '&\\2', $this->in->get('redirect')) : 'index.php';
		...
		if(!$this->user->check_auth('a_maintenance', false)){
			$this->tpl->assign_vars(array(
				...
				'REASON'					=> ($this->config->get('pk_maintenance_message') != "") ? $this->user->lang('reason').sanitize($this->config->get('pk_maintenance_message')) : '',
