	...
	public function __construct() {
		...
		if(!in_array($task, array_keys($task_list))){
			...
			if(!empty($task_obj->dependencies)){
				...
				$form .= '<tr><th><a href="./task.php?task='.$task_obj->dependencies[0].'">'.$this->user->lang('start_here').'</a></th></tr>';
				$form .= '</table>';
			...
			$this->core->create_breadcrump($this->user->lang($task_obj->type), 'task_manager.php?type='.$task_obj->type);
