...
while ( $row = $db->fetch_record($raids_result) )
{
     ...
     if ($pm->check(PLUGIN_INSTALLED, 'bossloot')){
            include_once ($eqdkp_root_path.'plugins/bossloot/include/extfunc.php');
            $raidnote = bl_note2link(stripslashes($row['raid_note']));
     } else if ( ($pm->check(PLUGIN_INSTALLED, 'bosssuite')) && ($eqdkp->config['bs_linkBL']) ){
            require_once ($eqdkp_root_path.'plugins/bosssuite/mods/note2link.php');
            $raidnote = bl_note2link(stripslashes($row['raid_note']));
     }else{
            $raidnote = stripslashes($row['raid_note']);
     }
    ...
    $tpl->assign_block_vars('raids_row', array(
        ...
        'NOTE' => ( !empty($row['raid_note']) ) ? news_parse($raidnote) : ' ',
