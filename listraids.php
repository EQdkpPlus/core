...
while ( $row = $db->fetch_record($raids_result) )
{
    ...
    $sql = 'SELECT count(member_name) from '. RAID_ATTENDEES_TABLE .' WHERE raid_id ='.$row['raid_id'];
    $membercount = $db->query_first($sql);
    ...
    if ($i < 10)
    {
        ...
        $additionals = array('DKP_VALUE'  		=> runden($row['raid_value']),
        					 'NOTE_LINKED'  	=> bl_note2link(stripslashes($row['raid_note'])),
        					 'EVENT_ICON'  		=> $pcache->BuildLink().getEventIcon($row['raid_name'],'16',true),
