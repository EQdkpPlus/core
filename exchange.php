...
if($in->get('out') != ''){
	switch ($in->get('out')){
		case 'news':
			$myOut		= $pcache->FilePath('last_news.xml', 'eqdkp', false);
			$myPerm		= (($conf_plus['ex_disable_news']) ? false : true);
			break;
		case 'items':
			$myOut		= $pcache->FilePath('last_items.xml', 'eqdkp', false);
			$myPerm		= (($conf_plus['ex_disable_items']) ? false : true);
			break;
		case 'raids':
			$myOut		= $pcache->FilePath('last_raids.xml', 'eqdkp', false);
			$myPerm		= (($conf_plus['ex_disable_raids']) ? false : true);
			break;
		case 'member':
			$myOut		= $pcache->FilePath('member.xml', 'eqdkp', false);
			$myPerm		= (($conf_plus['ex_disable_members']) ? false : true);
			break;
		case 'shoutbox':
			$myOut		= $pcache->FilePath('shoutbox.xml', 'shoutbox', false);
			$myPerm		= (($pm->check(PLUGIN_INSTALLED, 'shoutbox') && !$conf_plus['ex_disable_shoutbox']) ? true : false);
			break;
		case 'raidplan':
			$myOut		= $pcache->FilePath('rss.xml', 'raidplan', false);
			$myPerm		= (($pm->check(PLUGIN_INSTALLED, 'raidplan') && !$conf_plus['ex_disable_raidplan']) ? true : false);
			break;
		case 'data':
			...
			$myOut = '<?xml version="1.0" encoding="ISO-8859-1"?>
				<rss version="2.0">
					<channel>
						...
						<feeds_enabled>
							<news>'.(($pcache->FileExists('last_news.xml', 'eqdkp') && !$conf_plus['ex_disable_news']) ? 'true' : 'false').'</news>
							<items>'.(($pcache->FileExists('last_items.xml', 'eqdkp') && !$conf_plus['ex_disable_items']) ? 'true' : 'false').'</items>
							<raids>'.(($pcache->FileExists('last_raids.xml', 'eqdkp') && !$conf_plus['ex_disable_raids']) ? 'true' : 'false').'</raids>
							<members>'.(($pcache->FileExists('member.xml', 'eqdkp') && !$conf_plus['ex_disable_members']) ? 'true' : 'false').'</members>
							<shoutbox>'.(($pcache->FileExists('shoutbox.xml', 'shoutbox') && $pm->check(PLUGIN_INSTALLED, 'shoutbox') && !$conf_plus['ex_disable_shoutbox']) ? 'true' : 'false').'</shoutbox>
							<raidplan>'.(($pcache->FileExists('rss.xml', 'raidplan') && $pm->check(PLUGIN_INSTALLED, 'raidplan') && !$conf_plus['ex_disable_raidplan']) ? 'true' : 'false').'</raidplan>
						</feeds_enabled>
  ...
  <port name='TestServerPort' binding='TestServerBinding'> 
	...
	if(is_file($myOut)){
		if($myPerm){
			...
			die((isset($outdata)) ? $outdata : 'no_data');
		}else{
			die('disabled');
		}
	}else{
