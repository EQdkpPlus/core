	...
	switch (registry::register('input')->get('out')){
		...
		case 'icalfeed':
			...
			if (isset($permissions[$modulename]) && registry::fetch('user')->check_auth($permissions[$modulename], false, $userid)){
				...
				$v->setProperty('x-wr-calname',	registry::fetch('user')->lang('icalfeed_name'));
					...
					case 'calendar':
						...
						if(is_array($caleventids) && count($caleventids) > 0){
							foreach($caleventids as $calid){
								...
								$e = new vevent;
								$e->setProperty('dtstart',		array("timestamp" => registry::register('plus_datahandler')->get('calendar_events', 'time_start', array($calid))));
								$e->setProperty('dtend',		array("timestamp" => registry::register('plus_datahandler')->get('calendar_events', 'time_end', array($calid))));
