	...
	public function display() {
		...
		if(in_array(4, $raidcal_status)){
			...
			foreach($this->unsigned as $us_memberid => $us_classdata){
				...
				$this->tpl->assign_block_vars('notsignedin', array(
					'ROLES'			=> (isset($eventdata['extension']['raidmode']) && $eventdata['extension']['raidmode'] == 'role') ? $this->html->DropDown('memrole['.$us_memberid.']', $myrolesrry, $this->pdh->get('member', 'defaultrole', array($us_memberid)), '', '', 'input', 'memroles'.$us_memberid) : '',
					'ID'			=> $us_memberid,
		...
		$optionsmenu = array(
			...
			1 => array(
				...
				'link'	=> ($eventdata['closed'] == '1') ? $this->root_path.'calendar/viewcalraid.php'.$this->SID.'&amp;eventid='.$this->url_id.'&amp;closedstatus=open' :  $this->root_path.'calendar/viewcalraid.php'.$this->SID.'&amp;eventid='.$this->url_id.'&amp;closedstatus=close&amp;link_hash='.$this->CSRFGetToken('closedstatus'),
		...
		$this->jquery->Dialog('TransformRaid', $this->user->lang('raidevent_raid_transform'), array('url'=>"transform.php".$this->SID."&simple_head=true&eventid='+eventid+'", 'width'=>'400', 'height'=>'160', 'onclose' => $this->env->link.'calendar/viewcalraid.php'.$this->SID.'&eventid='.$this->url_id, 'withid' => 'eventid'));
