...
class viewcalraid extends page_generic {
	...
	public function __construct() {
		$handler = array(
			'closedstatus'	=> array(
				array('process' => 'close_raid',	'value' => 'close'),
				array('process' => 'open_raid',		'value' => 'open'),
			),
			'ajax'	=> array(
				array('process' => 'role_ajax',	'value' => 'role'),
			),
			'savenote'			=> array('process' => 'save_raidnote'),
			'update_status'		=> array('process' => 'update_status'),
			'moderate_status'	=> array('process' => 'moderate_status'),
			'confirmall'		=> array('process' => 'confirm_all'),
			'ical'				=> array('process' => 'generate_ical'),
			'add_notsigned'		=> array('process' => 'add_notsigned_chars'),
			'changecharmenu'	=> array('process' => 'change_char'),
			'changenotemenu'	=> array('process' => 'change_note'),
			'guestid'			=> array('process' => 'delete_guest'),
		);
	...
	private function email_statuschange($a_attendees, $status=0){
		if($this->config->get('calendar_email_statuschange') == 1){
			...
			$bodyvars = array(
				...
				'DATE'			=> $raiddate,
				'RAID_LINK'		=> $this->env->link.'calendar/viewcalraid.php?eventid='.$this->url_id,
			);
			...
			if(is_array($a_attendees) && count($a_attendees) > 0){
				foreach($a_attendees as $attendeeid){
					...
					$emailadress	= $this->pdh->get('user', 'email', array($attuserid));
	...
	public function generate_ical(){
		$eventdata	= $this->pdh->get('calendar_events', 'data', array($this->url_id));
		require($this->root_path.'libraries/vcalendar/vcalendar.class.php');
		$v = new vcalendar;
		$v->setConfig('unique_id', $this->config->get('server_name'));

		// Generate the vevents...
		$e = registry::register('vevent');
		$e->setProperty('dtstart',		array("timestamp" => $eventdata['timestamp_start']));
		$e->setProperty('dtend',		array("timestamp" => $eventdata['timestamp_end']));
	...
	public function display() {
		...
		foreach($attendeeids as $mattid){
			...
			if(is_array($filter_attuserids)){
				foreach($filter_attuserids as $attmemid){
					if($this->pdh->get('calendar_raids_attendees', 'status', array($this->url_id, $attmemid)) != 4){
		...
		if(in_array(4, $raidcal_status)){
			$a_js_disable = array();
			foreach($this->unsigned as $us_memberid => $us_classdata){
				$myrolesrry = $this->pdh->get('roles', 'memberroles', array($us_classdata['classid']));
				$this->tpl->assign_block_vars('notsignedin', array(
					'ROLES'			=> (isset($eventdata['extension']['raidmode']) && $eventdata['extension']['raidmode'] == 'role') ? $this->html->DropDown('memrole['.$us_memberid.']', $myrolesrry, $this->pdh->get('member', 'defaultrole', array($us_memberid)), '', 'id="memroles'.$us_memberid.'"', 'input') : '',
					'ID'			=> $us_memberid,
					'NAME'			=> $us_classdata['name'],
					'CLASSID'		=> $us_classdata['classid'],
					'CLASSICON'		=> $this->game->decorate('classes', array($us_classdata['classid'])),
					'USERID'		=> $us_classdata['userid'],
				));

				// Build the JS error for the dsiable feature
				$a_js_disable[$us_classdata['userid']][] = $us_memberid;
			}
			$this->tpl->add_js('var disabledChars = '.json_encode($a_js_disable).';');
		}
			...
			foreach ($this->raidcategories as $classid=>$classname){
				...
				if(isset($this->attendees[$statuskey][$classid]) && is_array($this->attendees[$statuskey][$classid])){
					foreach($this->attendees[$statuskey][$classid] as $memberid=>$memberdata){
						...
						if($eventdata['extension']['raidmode'] == 'role'){
							$memberrole = $this->jquery->dd_ajax_request('charchange_char', 'charchange_role', $drpdwn_twinks, array(), 0, 'viewcalraid.php?ajax=role');
		...
		$optionsmenu = array(
			...
			1 => array(
				...
				'link'	=> ($eventdata['closed'] == '1') ? $this->root_path.'calendar/viewcalraid.php'.$this->SID.'&amp;eventid='.$this->url_id.'&amp;closedstatus=open' :  $this->root_path.'calendar/viewcalraid.php'.$this->SID.'&amp;eventid='.$this->url_id.'&amp;closedstatus=close',
		...
		if($eventdata['extension']['raidmode'] == 'role'){
			$memberrole = $this->jquery->dd_ajax_request('member_id', 'member_role', $drpdwn_members, array(), $presel_charid, 'viewcalraid.php?ajax=role');
		}

		// jQuery Windows
		$this->jquery->Dialog('AddRaid', $this->user->lang('calendar_win_add'), array('url'=>"addevent.php?simple_head=true", 'width'=>'900', 'height'=>'580', 'onclose' => $this->env->link.'calendar/viewcalraid.php?s=&eventid='.$this->url_id));
		$this->jquery->Dialog('EditRaid', $this->user->lang('calendar_win_edit'), array('url'=>"addevent.php?s=&eventid=".$this->url_id."&simple_head=true", 'width'=>'900', 'height'=>'650', 'onclose' => $this->env->link.'calendar/viewcalraid.php?s=&eventid='.$this->url_id));
		$this->jquery->Dialog('ExportDialog', $this->user->lang('raidevent_raid_export_win'), array('url'=>"raidexport/exports.php?eventid=".$this->url_id, 'width'=>'640', 'height'=>'470'));
		$this->jquery->Dialog('EditGuest', $this->user->lang('raidevent_raid_editguest_win'), array('url'=>"guests.php?simple_head=true&guestid='+id+'", 'width'=>'490', 'height'=>'160', 'onclose' => $this->env->link.'calendar/viewcalraid.php?s=&eventid='.$this->url_id, 'withid' => 'id'));
		$this->jquery->Dialog('TransformRaid', $this->user->lang('raidevent_raid_transform'), array('url'=>"transform.php?simple_head=true&eventid='+eventid+'", 'width'=>'400', 'height'=>'160', 'onclose' => $this->env->link.'calendar/viewcalraid.php?s=&eventid='.$this->url_id, 'withid' => 'eventid'));
		$this->jquery->Dialog('AddGuest', $this->user->lang('raidevent_raid_addguest_win'), array('url'=>"guests.php?eventid='+eventid+'&simple_head=true", 'width'=>'490', 'height'=>'230', 'onclose' => $this->env->link.'calendar/viewcalraid.php?s=&eventid='.$this->url_id, 'withid' => 'eventid'));
		...
		$this->tpl->assign_vars(array(
			...
			// settings endabled?
			'S_NOTSIGNED_VISIBLE'	=> (in_array(4, $raidcal_status) && ($this->user->check_auth('u_cal_event_add', false) || $this->config->get('calendar_raid_shownotsigned'))) ? true : false,
			'IS_OPERATOR'			=> $this->user->check_auth('u_cal_event_add', false),
			'SHOW_GUESTS'			=> ($this->config->get('calendar_raid_guests') && ($this->user->check_auth('u_cal_event_add', false) || count($this->guests) > 0)) ? true : false,
			...
			'RAIDICON'				=> $this->pdh->get('event', 'html_icon', array($eventdata['extension']['raid_eventid'], 40, 40)),
			...
			'RAIDNOTE'				=> ($eventdata['notes']) ? $eventdata['notes'] : '',
