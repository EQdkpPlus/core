...
class addevent extends page_generic {
	...
	public function __construct() {
		$handler = array(
			'template_dd'	=> array('process' => 'json_ajax'),
			'templateid'	=> array('process' => 'delete_template'),
			'save_template'	=> array('process' => 'save_template'),
	...
	public function json_ajax(){
		$jsondata = $this->pdh->get('calendar_raids_templates', 'templates', array($this->in->get('template_dd', 0)));
		$tmparray = array();
		$distribution = '';
		...
		if($this->config->get('calendar_email_newraid') == 1){
			...
			if(is_array($a_users) && count($a_users) > 0){
				foreach($a_users as $userid){
					$emailadress	= $this->pdh->get('user', 'email', array($userid));
					if($emailadress){
	...
	public function delete_template(){
		$this->pdh->put('calendar_raids_templates', 'delete_template', array($this->in->get('templateid', 0)));
		$this->pdh->process_hook_queue();
	...
	public function display() {
		// add custom css
		$this->tpl->add_css('.raid, #raidmode_role{
			display:none;
		}');
		...
		$beforeclosefunc = "
			$.post('addevent.php?ajax_dropdown=true', { raidevent_id: $('body').data('raidevent_id') }, function(data) {
		...
		$this->jquery->Dialog('AddEventDialog', $this->user->lang('raidevent_raidevent_add'), array('url'=>$this->root_path.'admin/manage_events.php?s=&upd=true&simple_head=true&calendar=true', 'width'=>'600', 'height'=>'420', 'beforeclose'=>$beforeclosefunc));
		$this->tpl->assign_vars(array(
			'IS_EDIT'			=> ($this->url_id > 0) ? true : false,
			'IS_CLONED'			=> ((isset($eventdata['repeating']) && $eventdata['repeating'] != 'none') ? true : false),
			'DR_CALENDARS'		=> $this->html->DropDown('calendar_id', $this->pdh->aget('calendars', 'name', 0, array($this->pdh->get('calendars', 'idlist', array('free2add')))), ((isset($eventdata['calendar_id'])) ? $eventdata['calendar_id'] : '')),
			...
			'DR_CALENDARMODE'	=> $this->html->DropDown('calendarmode', $calendar_mode_array, ((isset($eventdata['extension']) && $eventdata['extension']['calendarmode']) ? $eventdata['extension']['calendarmode'] : (($this->config->get('calendar_addevent_mode')) ? $this->config->get('calendar_addevent_mode') : 'event')), '', '', 'input', 'selectmode'),
