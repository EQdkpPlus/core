...
if ($in->get(URI_ITEM, 0))
{
    ...
    $item_name = $db->query_first('SELECT item_name FROM ' . ITEMS_TABLE . " WHERE item_id='".$in->get(URI_ITEM, 0)."'");
    ...
    $item_id = $in->get(URI_ITEM, 0);
    #search for the gameid
  	$sql = 'SELECT game_itemid
            FROM ' . ITEMS_TABLE . "
            WHERE item_id=".$db->sql_escape($item_id) ;
	...
	//3d Model
	if( (strtolower($eqdkp->config['default_game']) == 'wow') and
		(version_compare(phpversion(), "5.0.0", ">=") ) and
		($game_id>1) and
		(!$conf_plus['pk_disable_3ditem']==1))
	{
    ...
    $tpl->assign_vars(array(
		'ITEM_STATS' => $html->itemstats_itemHtml($item_name,$game_id,true) ,
