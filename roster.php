...
$sql = 'SELECT class_id, class_name from __classes';
...
if ( $class_result = $db->query($sql)) 
{
	while ( $class_row = $db->fetch_record($class_result) )
	{
		...
		$class_array = array(
          ...
          'CLASS_ICONS'	  => get_ClassIcon('',$class_row['class_id'],true) 
			);
	    $tpl->assign_block_vars('class_row', $class_array );	
		
	    //Member SQL
		$sql = 'SELECT m.member_name, m.member_id , m.member_status, m.member_level, m.member_class_id
				FROM __members m				
		...
		if ($pm->check(PLUGIN_INSTALLED, 'charmanager'))
		{
			...
			$sql = 'SELECT m.member_name, m.member_id , m.member_status, m.member_level, m.member_class_id, ma.*
		...
		while ( $row = $db->fetch_record($members_result) )
		{
			...
			{			
		       	if(is_object($armory_light))
		       	{
					...
					$roster_data[$class_row['class_name']][$skill['spec']] = $row['member_name'];
				}
				
				//Member Template Vars				
				$member_array = array(
		          'MEMBER_NAME'    		=> $row['member_name'],
		          'MEMBER_SPEC'    		=> $skill['spec'],
		          'MEMBER_CLASS_ICON'   => get_classImgListmembers($row['member_class'] , $row['member_class_id']) ,
		          'MEMBER_RACE_ICON'	=> get_RaceIcon($row['member_race_id'],$row['member_name'] ),
