...
class user_settings extends page_generic {
	...
	public function __construct() {
		...
		$handler = array(
			'newexchangekey' => array('process' => 'renew_exchangekey', 'session_key' => true),
			'submit' => array('process' => 'update', 'session_key' => true),
			'mode' => array(
				array('process' => 'delete_authaccount', 'value' => 'delauthacc', 'session_key' => true),
				array('process' => 'add_authaccount', 'value' => 'addauthacc'),
				array('process' => 'delete_avatar', 'value' => 'deleteavatar',  'session_key' => true),
			),
			...
			'profile'	=> array(
				'adduser_tab_profile'	=> array(
					...
					'user_avatar'	=> array(
						...
						'options'	=> array('deletelink'	=> 'settings.php'.$this->SID.'&mode=deleteavatar&session_key'.$this->SKEY),
					),
					'work'	=> array(
						'fieldtype'	=> 'text',
						'name'	=> 'user_work',
				...
				if (isset($this->user->data['auth_account'][$method]) && strlen($this->user->data['auth_account'][$method])){
					$auth_array['registration_information']['auth_accounts'] = array(
						'auth_account_'.$method	=> array(
							...
							'text'	=> $this->user->data['auth_account'][$method].' <a href="settings.php'.$this->SID.'&amp;mode=delauthacc&amp;lmethod='.$method.'&amp;session_key='.$this->SKEY.'"><img src="'.$this->root_path.'images/global/delete.png" alt="Delete" /></a>',
