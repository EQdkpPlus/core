...
if ( isset($_POST['submit']) )
{
    ...
    if ( (!empty($_POST['new_user_password1'])) || (!empty($_POST['new_user_password2'])) )
    {
    	...
    	if(preg_match('+([^\w|#\$%!])+', $_POST['new_user_password1'])) {
    		$fv->errors['user_password'] = $user->lang['error_invalid_password'];
    	} else {
    ...
    if ( ($change_username) || ($change_password) )
    {
        $sql = 'SELECT user_id
                ...
                AND user_password='".md5($db->sql_escape($_POST['user_password']))."'";
        if ( $db->num_rows($db->query($sql)) == 0 )
        {
...
switch ( $action )
{
    ...
    case 'update':
        ...
        $privArray['priv_tel_all'] = $_POST['priv_tel_all'];
        $privArray['priv_tel_cript'] = $_POST['priv_tel_cript'];
    ...
    case 'account_settings':
    	$privacy = unserialize($user->data['privacy_settings']) ;
        $tpl->assign_vars(array(
            'IS_USER'				 => true,
            ...
            'BIRTHDAY' => $jqueryp->Calendar('birthday', stripslashes($user->data['birthday']), '', array('change_fields'=>true, 'year_range'=>'-80:0')),
