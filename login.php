...
class login extends page_generic {
	...
	public function __construct() {
		$handler = array(
			...
			'logout' 				=> array('process' => 'process_logout'),
			'mode' => array(
				array('process' => 'display_lost_password', 'value' => 'lostpassword'),
				array('process' => 'display_new_password', 'value' => 'newpassword'),
			),
			'new_password' 			=> array('process' => 'process_new_password', 'session_key' => true),
			'lost_password' 		=> array('process' => 'process_lost_password', 'session_key' => true),
	...
	public function process_login(){
		if (!$this->user->is_signedin()){
			$redirect = (( $this->in->exists('redirect') ) ? $this->in->get('redirect') : base64_encode('index.php'));
			...
			if ( !$this->user->login($this->in->get('username'), $this->in->get('password'), $blnAutoLogin) ){
				...
				//success
				$redirect_url = ( $this->in->exists('redirect') ) ? preg_replace('#^.*?redirect=(.+?)&(.+?)$#', '\\1' . $this->SID . '&\\2', base64_decode($this->in->get('redirect'))) : 'index.php'.$this->SID;
