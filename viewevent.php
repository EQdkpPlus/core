...
if ($in->get(URI_EVENT, 0) )
{
    ...
    // Items
    //
    $sql = 'SELECT i.item_date, i.raid_id, i.item_name, i.item_buyer, i.item_id, i.item_value, c.class_id, c.class_name
            FROM ' . ITEMS_TABLE . ' i
    ...
    while ( $row = $db->fetch_record($result) )
    {
        $tpl->assign_block_vars('items_row', array(
            ...
            'BUYER'         => get_classNameImgViewmembers(stripslashes($row['item_buyer']),$row['class_name'],$row['class_id'] ),
