...
if ($conf_plus['pk_itemstats'] == 1)
{
	...
	//ToDo: check why single news don't work cached....
	if( (!$news_array)) 
	{
		...
		{	
			...
			$SHOWCOMMENT = false;
	      	...
	      	$news_array[] = array(
		        ...
		        'COMMENT'			=> $COMMENT,
		        'MESSAGE'			=> $message,
		        'NEWS_FLAGS'		=> $news['news_flags'],
		        'NEWS_PERMISSION'	=> $news['news_permissions'],
	      	);
	...
	foreach ($news_array as $news)
	{
		...
		if($news['NEWS_FLAGS'])
    	{
      		$news['HEADLINE'] = $user->lang['sticky_news_prefix'].' '.$news['HEADLINE'];
      		$sticky_news++;
    	}

		// Show a new date row if it's not the same as the last			
	    if ( date($user->style['date_notime_long'], $news['TIME_RAW']) != date($user->style['date_notime_long'], $previous_date) )
	    {
	        $tpl->assign_block_vars('date_row', array(
	            'DATE' => strftime($user->style['strtime_date'],$news['TIME_RAW']))
	        );
	        $previous_date = $news['TIME_RAW'];
	    }		
