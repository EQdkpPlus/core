			...
			foreach($news_array as $news_id => $news) {
				...
				if(($this->url_id) and (strlen($news['extended_message']) >1)){
					...
					if (strlen($news['extended_message'])>1){
						$message .= '<br><a class="button news_rmlink" href="viewnews.php?id='.$news['news_id'].'">'.$this->user->lang('news_readmore').'</a>';
					}
				}
				...
				$userlink = '<a href="'.$this->root_path.'listusers.php?s=&amp;u='.$news['user_id'].'">'.sanitize($news['username']).'</a>';
		...
		foreach($raid_ids as $raid_ID) {
			...
			if($raid_ID) {
				...
				foreach ($itemlist as $item) {
					...
					$loot .= ' &raquo; <a href="'.$this->root_path.'viewcharacter.php?member_id='.$buyer.'">';
