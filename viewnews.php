 ...
 *
 * $Id: viewnews.php 6 2006-05-08 17:11:35Z tsigo $
 *
 ******************************/
      ...
      {      	 
		...
		if (($results = $db->query($sql2)) )
		{
			...
			while ( $item = $db->fetch_record($results) )
			{
				...
				if ($conf_plus['pk_itemstats'] == 1){
					$loot .= itemstats_decorate_name(stripslashes($item['item_name'])).'</a> -> ';
				}else{
					$loot .= stripslashes($item['item_name']).'</a> -> ';
				}
				$loot .= '<a '.getClassHtmlColorLinkCode(get_classNamebyMemberName($item['item_buyer'])).' href=viewmember.php'.$SID.'&amp;' . URI_NAME . '='.$item['item_buyer'].'>'. $item['item_buyer'].' </a> ('.round($item['item_value']).' DKP)<br>';
			}
			...
			if ( ($results = $db->query($sql2)) )
			{
				...
				{
					$event_icon = "<img height='24' width='24'  src='images/events/".$event_icon."'> " ;
				} 
				else
				{
				$event_icon = "";
...
if ($conf_plus['pk_itemstats'] == 1){
	...
	{
		$localtiem = date("H:i", $news['news_date']);
	}
	else
	{
		$localtiem = date("h:ia T", $news['news_date']);
	}
