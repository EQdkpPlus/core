...
*/
include_once($eqdkp_root_path . 'gplcore/db/dbal.php');
...
class dbal_mysql extends dbal
{
    ...
    function dbal_mysql($pdl=false, $root_path=false, $table_prefix=false)
    {
        $this->dbal('', $pdl);
    }
	...
	*/
    ...
    function sql_query($query, $params = false)
    {
        if ( $query != '' )
        {
            ...
            if(preg_match('#[^\w]__[^\s]+#', $query) != 0) {
            	$query = str_replace('__', $this->table_prefix, $query);
            }
            ...
            $this->pdl->log($this->debug_prefix."sql_query", $query);
    ...
    function sql_fetchrow($query_id = false, $assoc = true)
    {
        ...
        if ($query_id !== false)
        {
            $int_query_id = intval($query_id);
            $this->record[$int_query_id] = @mysql_fetch_array($query_id, $result_type);
            return $this->record[$int_query_id];
        }
