...
elseif ( $_GET[URI_PAGE] == 'history' )
{
    ...
    $sql = 'SELECT i.item_id, i.item_name, i.item_buyer, i.item_date, i.raid_id, i.item_value, i.game_itemid, r.raid_name, c.class_name, c.class_id
			FROM (' . RAIDS_TABLE . ' r INNER JOIN ' . ITEMS_TABLE . ' i ON r.raid_id = i.raid_id) 
...
while ( $item = $db->fetch_record($items_result) )
{
    ...
    $tpl->assign_block_vars('items_row', array(
        ...
        'BUYER' => ( !empty($item['item_buyer']) ) ? get_classNameImgViewmembers($item['item_buyer'],$item['class_name'],$item['class_id'] ) : '&lt;<i>Not Found</i>&gt;',
