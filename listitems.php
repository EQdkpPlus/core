...
if ( $in->get(URI_PAGE, 'values') == 'values' )
{
    ...
    $sql = 'SELECT i.item_id, i.item_name, i.item_buyer, i.item_date, i.game_itemid, i.raid_id, min(i.item_value) AS item_value, r.raid_name, m.member_class_id as class_id
            FROM ' . ITEMS_TABLE . ' i, ' . RAIDS_TABLE . ' r, ' . MEMBERS_TABLE . ' m
	    WHERE i.raid_id = r.raid_id
	    AND m.member_name = i.item_buyer
	    GROUP BY item_name
            ORDER BY '.$current_order['sql']. '
            LIMIT '.$db->sql_escape($start).','.$user->data['user_ilimit'];
...
while ( $item = $db->fetch_record($items_result) )
{
	...
	$_item = stripslashes($item['item_name']);
    ...
    $tpl->assign_block_vars('items_row', array(
        ...
        'NAME' => $html->itemstats_item($_item, $item['game_itemid']) ,
    ...
    if ($i < 20)
    {
        ...
        $additionals = array('DKP_VALUE'  => runden($item['item_value']),
        					 'ITEM_ICON'     => $html->itemstats_item($_item,$item['game_itemid'],true, false, true, false),
        					 'ITEM_COLOR'    => $html->itemstats_item($_item,$item['game_itemid'],false, false, false, true),
        					 'Looter_ICON'   => $pcache->BuildLink().get_ClassIcon('',$item['class_id'],true,true),
        					 'ITEMSTATS'  	 => "<![CDATA[".$html->itemstats_item($_item, $item['game_itemid'])."]]>",
        					 'EVENT_ICON'  	 => $pcache->BuildLink().getEventIcon($item['raid_name'],'16',true),
