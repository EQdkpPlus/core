...
function process_step2()
{
    ...
    // Standard Game
    //
    $game_path = $eqdkp_root_path . 'games/';
    if ( $dir = @opendir($game_path) )
    {
        while ( $file = @readdir($dir) )
        {
            if ( (!is_file($lang_path . $file)) && (!is_link($lang_path . $file)) && ($file != '.') && ($file != '..') && ($file != 'CVS') && ($file != '.svn'))
            {
        		...
        		$file_display = $file_array[0];

        		if($file_display == 'WoW_german')
        		{
        			$selected = 'selected="selected"' ;
        		}
...
function process_step3()
{
   ...
   // Setup the Game
    $default_game_file = $eqdkp_root_path . 'games/' . $default_game ;
	if (file_exists($default_game_file))
	{
        include($default_game_file);
        $manage = new Manage_Game;
		$manage->do_it($db,$table_prefix,true);
