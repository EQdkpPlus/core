...
if( $in->get(URI_NAME) != '' )
{
	...
	$sql = 'SELECT * ,(member_earned-member_spent+member_adjustment) AS member_current FROM ' . MEMBERS_TABLE . "
    ...
    if ( !$member = $db->fetch_record($member_result) )
    {
		...
		if ($pm->check(PLUGIN_INSTALLED, 'gearscore')) {
			...
			$gear_score		= $GearScore->retrieve(array(unsanitize($in->get(URI_NAME))));
			$gear_score_conf	= $GearScore->getConfig();
			$tpl->assign_vars(array(
				'GS_SHOW' 	=> $gear_score_conf['gs_show_viewprofile'],
				'L_GS_LIST_TITLE' 		=> $user->lang['gs_list_title'],
				'L_GEARSCORE'	=> $user->lang['gs_gearscore'],
				'L_GEMSCORE'	=> $user->lang['gs_gemscore'],
				'L_ENCHANTSCORE'	=> $user->lang['gs_enchantscore'],
				'L_HIGHEST'			=> $user->lang['gs_highest'],
				'GS_GEARSCORE'	=> '<a href="http://be.imba.hu/?zone='.strtoupper($conf_plus['pk_server_region']).'&realm='.ucfirst($conf_plus['pk_servername']).'&character='.unsanitize($in->get(URI_NAME)).'" target="_blank">'.$gear_score[unsanitize($in->get(URI_NAME))]['gearscore'].'</a>',
				'GS_GEARSCORE_HIGHEST' => $gear_score[unsanitize($in->get(URI_NAME))]['highestscore'],
				'GS_GEMSCORE'	=> '<a href="http://be.imba.hu/?zone='.strtoupper($conf_plus['pk_server_region']).'&realm='.ucfirst($conf_plus['pk_servername']).'&character='.unsanitize($in->get(URI_NAME)).'" target="_blank">'.$gear_score[unsanitize($in->get(URI_NAME))]['gemscore'].'</a>',
				'GS_ENCHANTSCORE'	=> '<a href="http://be.imba.hu/?zone='.strtoupper($conf_plus['pk_server_region']).'&realm='.ucfirst($conf_plus['pk_servername']).'&character='.unsanitize($in->get(URI_NAME)).'" target="_blank">'.$gear_score[unsanitize($in->get(URI_NAME))]['enchantscore'].'</a>',
			));
		}
    ...
    while ( $raid = $db->fetch_record($raids_result) )
    {
        ...
        $tpl->assign_block_vars('raids_row', array(
            ...
            'EARNED'         => runden($raid['raid_value']),
            'CURRENT_EARNED' => runden(sprintf("%.2f", $current_earned)))
        );
    ...
    $sql = 'SELECT i.item_id, i.item_name, i.item_value, i.item_date, i.game_itemid, i.raid_id, r.raid_name
            FROM ( ' . ITEMS_TABLE . ' i
    ...
    while ( $item = $db->fetch_record($items_result) )
    {
        ...
        $tpl->assign_block_vars('items_row', array(
            ...
            'SPENT'         => runden($item['item_value']),
            'CURRENT_SPENT' => runden(sprintf("%.2f", $current_spent)))
        );
    ...
    while ( $adjustment = $db->fetch_record($adjustments_result) )
    {
		...
		$event_icon = getEventIcon($adjustment['raid_name']);
        ...
        $tpl->assign_block_vars('adjustments_row', array(
            ...
            'INDIVIDUAL_ADJUSTMENT'   =>  runden($adjustment['adjustment_value']))
        );
    }
	...
	require_once($eqdkp_root_path . 'pluskernel/include/rendermember.class.php');
	$render = new RenderMember();

	$options['speedy'] = ($_GET[model]=='Java') ? true : false ;
	$options['speedy'] = ($conf_plus['pk_default_modelviewer']=='2') ? true : $options['speedy'] ;
	$options['flash'] = $conf_plus['pk_default_modelviewer'] ;
	$options['flash'] = ($_GET[model]=='flash2') ? 1 : $options['flash'] ;

	$class_race =$render->getRender($member['member_class_id'],$member['member_race_id'],$member['member_name'],$member['member_xml'],$conf_plus['pk_servername'],$options);
	...
	{
		$member_multidkp = $dkpplus-> multiDkpMemberArray($member['member_name']) ; // create the multiDKP Table
  ...
  $comm_settings = array('attach_id'=>$member['member_id'], 'page'=>'member');
    ...
    $tpl->assign_vars(array(
	    ...
	    'SIGNATUR'     => showAllvatarWoW_Signatur($member['member_name'],$class_name['class_name']) ,
	    'SHIRT_SHOP'   => createShirtBox($member['member_id']),
        ...
        'EARNED'         => runden($member['member_earned']),
        'SPENT'          => runden($member['member_spent']),
        'ADJUSTMENT'     => runden($member['member_adjustment']),
        'CURRENT'        => runden($member['member_current']),
