...
class viewcalendar extends page_generic {
	public static function __shortcuts() {
		$shortcuts = array('jquery', 'user', 'tpl', 'in', 'pdh', 'html', 'config', 'core', 'time');
		return array_merge(parent::$shortcuts, $shortcuts);
	}

	public function __construct() {
		$handler = array(
			'move'				=> array('process' => 'move_event'),
			'resize'			=> array('process' => 'resize_event'),
			'deleteid'			=> array('process' => 'delete_event'),
			'json'				=> array('process' => 'get_json'),
			'checkrepeatable'	=> array('process' => 'get_event_repeatable'),
			'ical'				=> array('process' => 'get_icalfeed'),
			'mass_signin'		=> array('process' => 'mass_signin'),
		);
	...
	// generate the ical feed for that calendar
	public function get_icalfeed(){
		require($this->root_path.'libraries/vcalendar/vcalendar.class.php');
		$v = new vcalendar;
		$v->setConfig('unique_id', $this->config->get('server_name'));
		
		$caleventids	= $this->pdh->get('calendar_events', 'id_list', array(true, $this->time->time));
		if(is_array($caleventids) && count($caleventids) > 0){
			foreach($caleventids as $calid){
				$e = new vevent;
				$e->setProperty('dtstart',		array("timestamp" => $this->pdh->get('calendar_events', 'time_start', array($calid))));
				$e->setProperty('dtend',		array("timestamp" => $this->pdh->get('calendar_events', 'time_end', array($calid))));
				$e->setProperty('dtstamp',		array("timestamp" => $this->time->time));
				$e->setProperty('summary',		$this->pdh->get('calendar_events', 'name', array($calid)));
				$e->setProperty('description',	$this->pdh->get('calendar_events', 'notes', array($calid)));
	...
	public function display(){
		...
		$memberrole = $this->jquery->dd_ajax_request('member_id', 'member_role', $drpdwn_members, array(), $presel_charid, 'calendar/viewcalraid.php?ajax=role');
		$raidcal_status = unserialize($this->config->get('calendar_raid_status'));
		$raidstatus = array();
		if(is_array($raidcal_status)){
			foreach($raidcal_status as $raidcalstat_id){
