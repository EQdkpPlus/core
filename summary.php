	...
	// New for 1.3 - grab class info from database
       ...
       $eq_classes = array();
        ...
        $sql = 'SELECT c.class_name AS member_class, count(i.item_id) AS class_drops
                FROM __items i, __members m, __classes c
...
$tpl->assign_vars(array(
	...
	// Date Picker
    'DATEPICK_DATE_FROM'        => $jquery->Calendar('summary_date_from', date('d.m.Y', time())),
	'DATEPICK_DATE_TO'        => $jquery->Calendar('summary_date_to', date('d.m.Y', time())))
);
