...
class viewcharacters extends page_generic {
	...
	public function display(){
		...
		$hptt				= $this->get_hptt($hptt_page_settings, $view_list, $view_list, array('%link_url%' => 'viewitem.php', '%link_url_suffix%' => '', '%itt_lang%' => false, '%itt_direct%' => 0, '%onlyicon%' => 0), $this->url_id, 'isort');
		$this->tpl->assign_vars(array (
				...
				foreach($pfields as $pfname=>$pfoption){
					...
					if($pfoption['category'] == 'character' && $pfoption['enabled'] == '1'){
						$this->tpl->assign_block_vars('pfield_data', array(
							...
							'VALUE'		=> $this->pdh->get('member', 'profile_field', array($this->url_id, $pfname))
						));
					}
				}
			}
		...
		$this->pm->do_hooks('/viewcharacter.php');
