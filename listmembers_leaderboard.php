 ...
 if ($conf_plus['pk_multidkp'] == 1)
{
    ...
    $tpl->assign_block_vars('multi_row', array(
	  ...
	  $showmultifilter = '';
		...
		if ( isset($multifilter) and ( strtoupper($multifilter ) <> 'NONE') and ($multifilter  <> '--------'))
		{
			$html = new htmlPlus(); // plus html class
			$member_multidkp = $html-> multiDkpAllMemberArray($multifilter) ; // create the multiDKP Array
			...
			foreach ($member_multidkp as $key)
			 {
				 ...
				 if($classcounte[$key['class']] > $conf_plus['pk_leaderboard_limit'] )
				 {$skip=true;}
