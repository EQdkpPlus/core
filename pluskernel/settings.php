...
if ($_POST['save_plus']){
   ...
   if (!$_HMODE)
    {
		...
		if ($conf_plus['pk_premium_status']>0){
			$plusdb->UpdateConfig('pk_hide_shop', $_POST['pk_hide_shop'], $isplusconfarray);
		} else {
			$plusdb->UpdateConfig('pk_hide_shop', 0, $isplusconfarray);
		}
	}
  ...
  $plusdb->UpdateConfig('pk_itemstats_max_execution_time', $_POST['pk_itemstats_max_execution_time'], $isplusconfarray);
	...
	$plusdb->UpdateConfig('ex_disable_news', 		$_POST['ex_disable_news'], $isplusconfarray);
	$plusdb->UpdateConfig('ex_disable_items', 		$_POST['ex_disable_items'], $isplusconfarray);
	$plusdb->UpdateConfig('ex_disable_raids', 		$_POST['ex_disable_raids'], $isplusconfarray);
	$plusdb->UpdateConfig('ex_disable_members', 		$_POST['ex_disable_members'], $isplusconfarray);
  ...
  // get the links stuff
   ...
   #Tab GetDKP
   #############
   $output .= $tabs->startTab($plang['pk_externals_tab'], 'config10');
   $output .= $tabs->startTable();
   ...
   $output .= $tabs->startTable();
   $output .= $tabs->tableheader($plang['pk_externals_th']);
   $output .= $html->CheckBox('ex_disable_news', $plang['pk_externals_news'] , $conf['ex_disable_news'], $plang['pk_externals_news']);
   $output .= $html->CheckBox('ex_disable_items', $plang['pk_externals_items'] , $conf['ex_disable_items'], $plang['pk_externals_items']);
   $output .= $html->CheckBox('ex_disable_raids', $plang['pk_externals_raids'] , $conf['ex_disable_raids'], $plang['pk_externals_raids']);
   $output .= $html->CheckBox('ex_disable_members', $plang['pk_externals_members'] , $conf['ex_disable_members'], $plang['pk_externals_members']);
   $output .= $tabs->endTable() ;
   ...
   $output .= $html->TextField("pk_sms_password", '15', $conf['pk_sms_password'],$plang['pk_set_sms_pass'], $plang['pk_set_sms_pass'],'password',false) ;
   	if ($_HMODE)
	{
