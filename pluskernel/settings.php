...
if ($_POST['save_plus']){
	...
	$plusdb->UpdateConfig('pk_itemhistory_dia', $_POST['pk_itemhistory_dia'], $isplusconfarray); # Show Quickdkp?
	...
	//Rss
	$plusdb->UpdateConfig('pk_showRss', $_POST['pk_showRss'], $isplusconfarray); # Show Quickdkp?
	$plusdb->UpdateConfig('pk_Rss_Style', $_POST['pk_Rss_Style'], $isplusconfarray); # Style Quickdkp?
	$plusdb->UpdateConfig('pk_Rss_lang', $_POST['pk_Rss_lang'], $isplusconfarray); # Language
	$plusdb->UpdateConfig('pk_Rss_count', $_POST['pk_Rss_count'], $isplusconfarray); # max count
  ...
  $result = $db->query($sql);
   while ( $row = $db->fetch_record($result) )
   {
   		$output .= "<tr class='".$eqdkp->switch_row_class()."'>
    							<td nowrap='nowrap'><input type='text' name='linkname[".$row['link_id']."]' size='30' maxlength='50' value='".$row['link_name']."' class='input' /></td>
    							<td nowrap='nowrap'><input type='text' name='linkurl[".$row['link_id']."]' size='30' maxlength='75' value='".$row['link_url']."' class='input' /></td>
 								<td nowrap='nowrap'>".$html->CheckBox('linkwindow['.$row['link_id'].']', '' , $row['link_window'],'',1,true)."</td>
 									</tr>";
 			$max_id = ( $max_id < $row['link_id'] ) ? $row['link_id'] : $max_id;
   }
   	$newid = ($max_id +1);
    $output .= " <tr class='".$eqdkp->switch_row_class()."'>
     							<td nowrap='nowrap'><input type='text' name='linkname[".$newid."]' size='35' maxlength='75' value='' class='input' /></td>
    							<td nowrap='nowrap'><input type='text' name='linkurl[".$newid."]' size='35' maxlength='75' value='' class='input' /></td>
  								<td nowrap='nowrap'>".$html->CheckBox('linkwindow['.$newid.']', '' , '','',1,true)."</td>
  							</tr>
								</table>";
   $output .= $tabs->endTab();
