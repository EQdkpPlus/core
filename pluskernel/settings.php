...
if ($_POST['save_plus']){
  ...
  if (($multi_result = $db->query($sql)))
   ...
   # Tab Rss
   ###############
   $output .= $tabs->startTab('RSS', 'config2');
   $output .= $tabs->startTable();
  ...
  // get the links stuff
   ...
   ##########
	$output.=$tabs->startTab('News', 'config6');
	$output .= $tabs->startTable();
   ...
   #Tab Itemstats
   ###############
   if( (strtolower($eqdkp->config['default_game']) == 'wow') or (strtolower($eqdkp->config['default_game']) == 'lotro') or (strtolower($eqdkp->config['default_game']) == 'runesofmagic'))
   {
   ...
   #Tab Bridge
   #############
   $output .= $tabs->startTab('CMS-Bridge', 'config8');
   $output .= $tabs->startTable();
   ...
   #Tab GetDKP
   #############
   $output .= $tabs->startTab('GetDKP', 'config10');
   $output .= $tabs->startTable();
   ...
   #Tab SMS
   #############
   $output .= $tabs->startTab('SMS', 'config11');
   $output .= $tabs->startTable();
