  ...
  // get the links stuff
	$customlink = array();
 	$sql = 'SELECT link_id, link_name, link_url, link_window, link_menu
         	FROM __plus_links ORDER BY link_id';
   $result = $db->query($sql);
   ...
   while ( $row = $db->fetch_record($result) )
   {
    ...
    $output .= " <tr class='".$eqdkp->switch_row_class()."'>
    							...
    							<td nowrap='nowrap'><input type='text' name='linkurl[".$newid."]' size='35' maxlength='85' value='' class='input' /></td>
  								<td nowrap='nowrap'>".$html->DropDown('linkwindow['.$row['link_id'].']', $a_linkMode , 0,$plang[''], $plang['pk_set_link_type_iframe_help'] ,true)."</td>
  								<td nowrap='nowrap'>".$html->DropDown('linkmenu['.$row['link_id'].']', $a_linkMenu , 0,$plang[''], "" ,true)."</td>
  							</tr>
								</table>";   							  
   $output .= $tabs->endTab();
