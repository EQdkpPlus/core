...
if ($_POST['save_plus']){
   ...
   if (!$_HMODE) 
	...
	#auto image resize
 	...
 	$plusdb->UpdateConfig('pk_air_show_warning', $_POST['pk_air_show_warning'], $isplusconfarray);
  ...
  if (($multi_result = $db->query($sql)))
   ...
   $output .= $html->CheckBox('pk_noEvents', $plang['pk_set_noEvents'] , $conf['pk_noEvents'], $plang['pk_help_noRaids']);
   ...
   # Tab Rss
   ###############
   $output .= $tabs->startTab('RSS', 'config3');
   $output .= $tabs->startTable();   
...
$a_linkMode= array(
   ...
   while ( $row = $db->fetch_record($result) )
   {
   		$output .= "<tr class='".$eqdkp->switch_row_class()."'>
						...
						<td nowrap='nowrap'>".
						$html->DropDown('linkwindow['.$row['link_id'].']', $a_linkMode , $row['link_window'],$plang[''], $plang['pk_set_link_type_iframe_help'] ,true)					
						."</td></tr>";
    ...
    $output .= " <tr class='".$eqdkp->switch_row_class()."'>
    							...
    							<td nowrap='nowrap'><input type='text' name='linkurl[".$newid."]' size='35' maxlength='85' value='' class='input' /></td>
  								<td nowrap='nowrap'>".
								$html->DropDown('linkwindow['.$row['link_id'].']', $a_linkMode , 0,$plang[''], $plang['pk_set_link_type_iframe_help'] ,true)					
    							."</td>
  							</tr>
								</table>";   							  
   $output .= $tabs->endTab();
   ...
   ##########
	$output.=$tabs->startTab('News', 'config8');
	$output .= $tabs->startTable();
	...
	$output .= $html->CheckBox('pk_air_show_warning', $plang['pk_air_show_warning'], $conf['pk_air_show_warning'], $plang['pk_air_show_warning']);	
   ...
   if( (strtolower($eqdkp->config['default_game']) == 'wow') or (strtolower($eqdkp->config['default_game']) == 'lotro'))
   {
	...
	'smf2'					=> 'Simple Machines Forum 2 Beta',
   ...
   #Tab GetDKP
   #############
   $output .= $tabs->startTab('GetDKP', 'config7');
   $output .= $tabs->startTable();
