   ...
   function login($username, $password, $auto_login)
   {
		...
		$db_cms->sql_connect($cms_host,$cms_db,$cms_user,$cms_pass,false);
            ...
            if ( $db_cms->sql_numrows($db_cms->query($sql)))
			{
					...
					$sql = "SELECT ID_MEMBER, passwd , emailAddress , passwordSalt, ID_GROUP, additionalGroups FROM ".$cms_tableprefix."members WHERE memberName='$a_username' AND is_activated ='1'";
			...
			if ( $rut_row )
			{
                ...
                if($rut_row['additionalGroups'] != "")
                {
                	$addGroups = explode(",",$rut_row['additionalGroups']);
                ...
                if ((sha1(utf8_encode(strtolower($username)) . htmlspecialchars_decode(utf8_encode(stripslashes($password)))) == $rut_row['passwd']) && $right_group)
                {
					...
					$mail = $db->escape($rut_row['emailAddress']);
