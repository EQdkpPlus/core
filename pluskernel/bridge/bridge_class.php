 ...
 * EQdkp Bridge Class
 *
 * @author Corgan ^ Stefan Knaak
 * @package Eqdkp Plus
 *
 * visit http://www.eqdkp-plus.com
 *
...
include($eqdkp_root_path . 'includes/db/mysql.php');
...
class eqdkp_bridge
{
	...
	function eqdkp_bridge()
	{
		...
		global $dbtype, $dbhost, $dbname, $dbuser , $table_prefix;

		$this->table_prefix = $table_prefix;
		$this->br_db = new SQL_DB($dbhost, $dbname, $dbuser, $dbpass, false);
	}
	 ...
	 * @return Array or 0
	 */
	function get_last_group_items($limit)
	{
		$this->br_db = $this->br_db;
	 ...
	 * ["raid_icon"] - Icon - Die Icons im Format 64x64 liegen in "<img src=/eqdkp/games/gamename/events/"..">" ;
  	 ...
  	 * @return Array or 0
  	 */
  	function get_last_Group_Raids($limit)
	{
		$this->br_db = $this->br_db;
  ...
  function check_is_user($user_name)
	{
		...
		$sql = "SELECT user_id FROM " . USERS_TABLE ." WHERE username = '".$user_name."'";
  ...
  function new_eqdkp($grp_name,$adminemail)
    ...
    ### default config vars für jede eqdkp gruppe
		...
		$sql[] = "INSERT INTO eqdkp_auth_options (auth_id,auth_value, auth_default) VALUES ('34' , 'a_soap_read','N')";
		$sql[] = "INSERT INTO eqdkp_auth_options (auth_id,auth_value, auth_default) VALUES ('35' , 'a_soap_write','N')";
  ...
  function new_member($member_name, $member_level, $member_race, $member_class)
	...
	function new_member2user($user_name,$member_name)
	{
		$this->br_db = $this->br_db;
		$user_name = ucwords($user_name);
		$_sql = "SELECT user_id FROM " . USERS_TABLE ." WHERE username = '".$user_name."'";
		$user_id = $this->br_db->query_first($_sql);
