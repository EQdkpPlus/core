...
class eqdkp_bridge
{
	...
	function get_eqdkp_status()
	{
		...
		// Get total users
		$sql = "SELECT count(user_id) FROM __users";
	...
	function get_last_items($limit=7,$raidID=-1)
	{
		...
		$sql = "SELECT i.*, r.raid_name, r.raid_date, r.raid_note
		        FROM __items i INNER JOIN __raids r
  	...
  	function get_last_Group_Raids($limit=5)
	{
		...
		if($result)
		{
			while($row = $this->br_db->fetch_record($result))
			{
				$sql = "SELECT event_icon from __events WHERE event_name='".$row['raid_name']."'" ;
  ...
  function get_GroupDKPInfo()
	{
		...
		// Get total Raids
		$sql ="select count(raid_id) as alle from __raids";
		$data['allraids'] = $this->br_db->query_first($sql);

		$sql = "SELECT member_earned,member_spent,member_adjustment FROM __members";
		...
		// Get total items
		$sql= "SELECT item_id FROM __items";
		$data['total_items'] = $this->br_db->num_rows($this->br_db->query($sql));

		// Get total members
		$sql = "SELECT member_id FROM __members";
  ...
  function get_memberInfo($member_name)
	...
	function get_ClassID_from_ClassName($classname)
	{
	  	...
	  	$sql = "SELECT class_id from __classes WHERE class_name='".$this->translate_Classname_into_German($classname)."'" ;
	...
	function get_RaceID_from_RaceName($racename)
	{
	  	...
	  	$sql = "SELECT race_id from __races WHERE race_name ='".$this->translate_RaceName_into_German($racename)."'" ;
  ...
  function check_is_user($user_name)
	{
		...
		$sql = "SELECT user_id FROM __users WHERE username = '".$user_name."'";
  ...
  function new_user($is_admin, $user_name, $user_pw, $user_email)
	{
		...
		$maxid = $this->br_db->query_first('SELECT max(user_id) from __users') +1;
   ...
   */
   function set_userAdmin($member_name)
	{
	  	$member_name = ucwords($member_name);
	  	$_sql = "SELECT member_id FROM __members WHERE member_name = '".$member_name."'";
	  	$member_id = $this->br_db->query_first($_sql);


	  	if ($member_id >=1 )
	  	{
			$_sql = "SELECT user_id FROM __member_user WHERE member_id = ".$member_id ;
			$user_id = $this->br_db->query_first($_sql);

			if ($user_id >= 1)
			{
				$sql[] = "UPDATE __auth_users SET auth_setting = 'Y' WHERE user_id = ".$user_id;
			}
	  	}
  ...
  function new_member($member_name, $member_level, $member_race, $member_class)
	{
		...
		$_sql = "SELECT rank_id FROM __member_ranks WHERE rank_name = 'Member'";
	...
	{
		$user_name = ucwords($user_name);
		$_sql = "SELECT user_id FROM __users WHERE username = '".$user_name."'";
		$user_id = $this->br_db->query_first($_sql);
		...
		$sql[] = "INSERT INTO __member_user (`member_id` , `user_id`) VALUES ('".$member_id."', '".$user_id."');";
		return $this->sql_prozess($sql);
	}# end function
  ...
  function delete_user($user_name)
	{
	    // Delete attendance
	    $sql[] = "DELETE FROM __users WHERE username='" . $user_name . "'";

	  	$_sql = "SELECT user_id from __users WHERE username='".$user_name."'" ;
	  	$user_id = $this->br_db->query_first($_sql);
	  	if(isset($user_id))
	  	{
	    	$sql[] = "DELETE FROM __auth_users WHERE user_id=" . $user_id ;
