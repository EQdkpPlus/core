   ...
   function login($username, $password, $auto_login)
   {
            ...
            if ($res = $db_cms->query($sql))
            {
               if ( $row = $db_cms->fetch_record($res) )
            	{
            $sql = "SELECT userID, password, salt, email FROM `".$db_cms->dbname."`.".$cms_tableprefix."user WHERE username='" . $a_username . "'";
            ...
            if ( $rut_row )
            {
                ...
                $sql = "SELECT *  FROM `".$db_cms->dbname."`.".$cms_tableprefix."user_to_groups WHERE groupID ='".$group_ids." 'and userID='" . $rut_row['userID'] . "'";
                ...
                if ( (sha1($rut_row['salt'].sha1($rut_row['salt'].sha1($password))) == $rut_row['password']) && $rgt_row['groupID'] == $group_ids) // Gucken ob das eingegebene Paswort mit dem CMS passwort übereinstimmt (das Passwort wird mit dem salt aus der datenbank natürlich gesalzen) und ob er berechtigt ist zuzugreifen
                {
