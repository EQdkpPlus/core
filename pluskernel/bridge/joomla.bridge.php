   ...
   function login($username, $password, $auto_login)
   {
		...
		$db_cms->sql_connect($cms_host,$cms_db,$cms_user,$cms_pass,false);
			...
			if ( $db_cms->sql_numrows($db_cms->query($sql)))
			{
					...
					$sql = "SELECT id, block, gid, password, email FROM ".$cms_tableprefix."users WHERE username='$a_username'";
			...
			if ( $rut_row )
			{
				//Build an array of all groups of which the user is member (joomla! will get the possibility to have more than one group)
				$memgrp = array($rut_row['gid']);
				...
				//Preparation of joomla's! password and salting ;)
                $exploded = explode(":",$rut_row['password']); 
                
                //Check all inputs against the data in the cms table
                if ( (md5($password.$exploded[1]) == $exploded[0]) && $rut_row['block'] == "0" && $right_group) // Gucken ob das eingegebene Paswort mit dem CMS passwort übereinstimmt (das Passwort wird mit dem salt aus der datenbank natürlich gesalzen) und ob er berechtigt ist zuzugreifen
                {
