   ...
   function login($username, $password, $auto_login)
   {
            ...
            if ($res = $db_cms->query($sql))
            {
               if ( $row = $db_cms->fetch_record($res) )
            	{
            $sql = "SELECT id, password, email FROM `".$db_cms->dbname."`.".$cms_tableprefix."users WHERE username='" . $a_username . "'";
            ...
            if ( $rut_row )
            {
                ...
                $sql = "SELECT block  FROM `".$db_cms->dbname."`.".$cms_tableprefix."users WHERE (gid )='".$group_ids." 'and id='" . $rut_row['id'] . "'";
                ...
                $exploded = explode(":",$rut_row['password']); // Aufteilen des Datenbankfeldes da Joomla die Eigenheit hat, das Passwort in der Form : "Passwort:salt" speichert
                if ( (md5($password.$exploded[1]) == $exploded[0]) && $rgt_row['block'] == "0") // Gucken ob das eingegebene Paswort mit dem CMS passwort übereinstimmt (das Passwort wird mit dem salt aus der datenbank natürlich gesalzen) und ob er berechtigt ist zuzugreifen
                {

                    if ($lut_row = $db->fetch_record($local_users_table))
                    {
                        $sql = "UPDATE ``".$db->dbname."``.__users SET user_password='" . md5($password) ."',user_email='".$rut_row['email']."' WHERE username='" . $a_username . "'";
                        $db->query($sql);   // Aktualisieren der Lokalen Informationen
                        $userid = $lut_row['user_id'];
                    }
                    else
                    {
                          $sql = "INSERT into `".$db->dbname."`.__users SET username='".$a_username."', user_password='".md5($password)."', user_email='".$rut_row['email']."', user_active='1', user_style='".$eqdkp->config['default_style']."', user_lang='".$eqdkp->config['default_lang']."'";
