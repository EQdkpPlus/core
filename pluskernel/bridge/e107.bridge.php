   ...
   */
   function login($username, $password, $auto_login)
   {
            ...
            if ( $rut_row = $db_cms->fetch_record($remote_users_table) )
            {
                ...
                $sql = "SELECT user_ban  FROM ".$db_cms->dbname.".".$cms_tableprefix."user WHERE user_class ='".$group_ids." 'and user_id='" . $rut_row['user_id'] . "'";
                ...
                if ( (md5($password) == $rut_row['user_password']) && $rgt_row['user_ban'] == "0") // Gucken ob das eingegebene Paswort mit dem CMS passwort übereinstimmt (das Passwort wird mit dem salt aus der datenbank natürlich gesalzen) und ob er berechtigt ist zuzugreifen
                {
