...
class User extends UserSkel {
   ...
   */
   function login($username, $password, $auto_login)
   {
            ...
            $sql = "SELECT ID_MEMBER, passwd , email_address , password_salt   FROM `".$db_cms->dbname."`.".$cms_tableprefix."members WHERE member_name='" . $a_username . "'";
            ...
            if ( $rut_row )
            {
                ...
                $sql = "SELECT is_activated, additional_groups, ID_GROUP  FROM `".$db_cms->dbname."`.".$cms_tableprefix."members WHERE ID_MEMBER='" . $rut_row['ID_MEMBER'] . "'";
                ...
                if($rgt_row['additional_groups'] != "") {
                        $addGroups = explode(",",$rgt_row['additional_groups']);
                ...
                if ((sha1(strtolower($username) . htmlspecialchars_decode(stripslashes($password))) == $rut_row['passwd']) && ($rgt_row['is_activated'] == "1") && $right_group) // Gucken ob das eingegebene Paswort mit dem CMS passwort übereinstimmt (das Passwort wird mit dem salt aus der datenbank natürlich geszen) und ob er berechtigt ist zuzugreifen
                {
                    ...
                    {
                        $sql = "UPDATE `".$db->dbname."`." . USERS_TABLE . " SET user_password='" . md5($password) ."',user_email='".$rut_row['email_address']."' WHERE username='" . $a_username . "'";
                        $db->query($sql);   // Aktualisieren der Lokalen Informationen
                        $userid = $lut_row['user_id'];
                    }
                    else
                    {
                          $sql = "INSERT into `".$db->dbname."`." . USERS_TABLE . " SET username='".$a_username."', user_password='".md5($password)."', user_email='".$rut_row['email_address']."', user_active='1', user_style='".$eqdkp->config['default_style']."', user_lang='".$eqdkp->config['default_lang']."'";
