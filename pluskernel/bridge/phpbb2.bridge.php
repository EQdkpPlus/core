   ...
   */
   function login($username, $password, $auto_login)
   {
            ...
            if ( $rut_row = $db_cms->fetch_record($remote_users_table) )
            {
                ...
                $sql = "SELECT user_pending  FROM ".$db_cms->dbname.".".$cms_tableprefix."user_group WHERE group_id ='".$group_ids." 'and user_id='" . $rut_row['user_id'] . "'";
                ...
                if ( (md5($password) == $rut_row['user_password']) && $rgt_row['user_pending'] == "0") // Gucken ob das eingegebene Paswort mit dem CMS passwort übereinstimmt (das Passwort wird mit dem salt aus der datenbank natürlich gesalzen) und ob er berechtigt ist zuzugreifen
                {
