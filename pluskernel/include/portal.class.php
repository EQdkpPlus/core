...
class portal
{
  ...
  var $THIRD_C = false;
	...
	function portal()
	{
		global $eqdkp_root_path, $db, $tpl, $user, $eqdkp, $pluslang, $conf_plus, $wherevalue, $sql_db;
		...
		foreach($this->positions as $wherevalue)
		{
  		...
  		if(($wherevalue == 'middle' || $wherevalue == 'right') && basename($_SERVER['PHP_SELF']) != $eqdkp->config['start_page']){
  ...
  function InstallModule($d_plugin_code, $portal_module, $plugin='')
  {
    ...
    if ($portal_module[$d_plugin_code] && isset($portal_module[$d_plugin_code])){
      ...
      $query = $db->build_query('INSERT', array(
              ...
              'plugin'      => $plugin,
              'enabled'     => ($install_sett['autoenable']) ? $install_sett['autoenable'] : 0,
              'position'    => ($install_sett['defaultposition']) ? $install_sett['defaultposition'] : '',
              'number'      => ($install_sett['defaultnumber']) ? $install_sett['defaultnumber'] : '',
              'visibility'  => ($install_sett['visibility']) ? $install_sett['visibility'] : 0,
              'collapsable' => ($install_sett['collapsable']) ? $install_sett['collapsable'] : 1,
            )
          );
