...
class htmlPlus
{
	...
	function AutoTextField($name, $size, $value = '', $text='', $help='', $type = 'text',$notable=false,$class='autocomplete')
	{
		$textfield = "<tr class=row1><td>".$this->HelpTooltip($help)."</td><td>";
		$textfield .= " <input name='".$name."' size='".$size."' value='".$value."' class='".$class."' /> ".$text." </td></tr>";
		return $textfield;
	}
	...
	function createVTable($misc,$header)
	{
		...
		$start = ' <table width="100%" border="0" cellspacing="1" cellpadding="2" bgcolor="#000000" class="forumline">
			  	   <tr>
	  			   	<th class="smalltitle" align="center">'.$header.'</th>
	  			   </tr>';
		$end = '</table>';
		$out = $start.$misc.$end ;
