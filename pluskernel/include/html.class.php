...
class htmlPlus
{
		...
		$copyright='<br/><center><span class="copy">
								...
								<br />
								   ...
								   &copy; '.$this->CopyRightYear().' by <a href="http://www.eqdkp-plus.com" target="_new" class="copy">'.EQDKPPLUS_AUTHOR.'</a>
									| follow us on <a href="http://twitter.com/EQdkpPlus" target="_new" ><img src='.$eqdkp_root_path.'images/Twitter-icon-16.png></a>
												 <a href="http://www.facebook.com/pages/EQdkp-Plus/164694353225" target="_new" ><img src='.$eqdkp_root_path.'images/FaceBook-icon-16.png></a>
									</span></center><br />
									';
		return $copyright;
	}
  ...
  /**
	 ...
	 * @return string
	 */
  function growlMessage($text='', $title='', $kind='default'){
    global $jqueryp;
  ...
  function TooltipStyle($content, $divstyle, $icon='')
  {
	 ...
	 */
	function itemstats_item($item, $itemid=0,$onlyIcon=false, $update=false, $urlOnly=false, $itemcolorOnly = false)
	{
		global  $conf_plus ;
		$ret_val = $item ;
		if($conf_plus['pk_itemstats'] == 1 ) {
			$ret_val = itemstats_decorate_name(stripslashes($item),$itemid,$onlyIcon, $update, $urlOnly , $itemcolorOnly);
		}
		return $ret_val ;
	}
	 ...
	 * @return Itemstats string
	 */
	 function itemstats_itemHtml($item, $itemid=0, $update=false)
	{
		global  $conf_plus ;
		$ret_val = $item ;
	   ...
	   if($conf_plus['pk_itemstats'] == 1 )
	   {
    		$ret_val = itemstats_get_html(stripslashes($item),$itemid,$update) ;
	   }
		return $ret_val ;
	}


	function createVTable($misc,$header,$id = '')
    {
      global $eqdkp_root_path;
      $id = 'coll'.(($id != '') ? $id : strtolower($header));
      $start = '<div class="forumline"><table width="100%" border="0" cellpadding="0" cellspacing="0">
			  	        <tr>
                      ...
                      </a><span align="center" id="txtrecruitment">'.$header.'</span>
                    </th>
	  			        </tr>
                  <tr>
                    <td>
	...
	function createGraph($a_daten)
	{
		...
		//Array umdrehen und Wert gl�tten auf max 97 da die
	...
	function newsloot($showRaids_id)
	{
		...
		foreach($raid_ids as $raid_ID)
		{
			...
			if($raid_ID)
			{
		    	$sql2 = 'SELECT item_id, item_buyer, item_name, item_date, item_value, game_itemid
				         FROM ' . ITEMS_TABLE . "
				...
				if (($results = $db->query($sql2)) )
				{
					while ( $item = $db->fetch_record($results) )
					{
						...
						if ($conf_plus['pk_itemstats'] == 1){
							$loot .= $this->itemstats_item(stripslashes($item['item_name']), $item['game_itemid']).'</a> -> ';
						}else{
