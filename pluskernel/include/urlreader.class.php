...
class urlreader
{
  ...
  /**
   * Do a curl_exec but take care of redirect -> Function broken, do NOT use. Will try to code a new and safe one later (Ghoschdi)
   *
   * @param object $ch                 CURL handle
   * @param int    $allowed_redirects  Max allowed redirects, will be decremented on each recursion
   * @return string
   */
  function Get_CURL_redirect_exec($ch, $allowed_redirects = 5)
  {
    $getdata = null;

    // end of redirect following?
    ...
    if ($getdata)
    {
      // get HTTP code and check if this is a redirect
      $http_code = curl_getinfo($ch, CURLINFO_HTTP_CODE);
      switch ($http_code)
      {
        case 301:
        case 302:
          // Redirection, so try to find "Location:" or "URI:" and follow
          list($http_header) = explode("\r\n\r\n", $getdata, 2);
          $matches = array();
          if (preg_match('/(Location:|URI:)(.*?)\n/', $http_header, $matches) != 0)
          {
            $url = trim(array_pop($matches));
            $url_parsed = parse_url($url);
            if ($url_parsed)
            {
              // set new URL to curl and execute again
              curl_setopt($ch, CURLOPT_URL, $url);
              $getdata = $this->Get_CURL_redirect_exec($ch, $allowed_redirects - 1);
            }
          }
          break;
        default:
          list(,$getdata) = explode("\r\n\r\n", $getdata, 2);
          break;
      }
    }
