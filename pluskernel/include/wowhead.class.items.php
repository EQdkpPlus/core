...
class wowheadLoader
{
	function wowheadItemLoader($utf8test)
	{
		$this->stringIsUTF8 = ($this->isUTF8($utf8test) == 1) ? true : false;
	}
	
	function GetwowheadData($item)
	{
		$wowurl = $this->BuildLink(stripslashes($item));
		$xmldata = $this->read_url($wowurl);
		return $xmldata;
	}
	...
	{
		$url 		= 'http://de.wowhead.com/?item='.$item.'&xml';
    	return $url;
	}

	// get the XML
	function read_url($url, $lang='de')
	{
