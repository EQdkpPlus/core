...
class dkpplus
{
	...
	function multiDkpMemberArray($membername)
	{
		...
		// earned
		$pv_sql = "SELECT __raids.raid_name, SUM(raid_value)
				   FROM
				   __raid_attendees LEFT JOIN __raids ON __raid_attendees.raid_id = __raids.raid_id
				   WHERE __raid_attendees.member_name = '".$membername."' GROUP by __raids.raid_name";
	    ...
	    while( $pv_row = $db->fetch_record($pv_result, false) )
	    {
			...
			//
			// spend
			$ps_sql = "SELECT __raids.raid_name, SUM(__items.item_value)
			   FROM
			   __items  LEFT JOIN __raids ON __items.raid_id = __raids.raid_id
			   WHERE
			   __items.item_buyer = '".$membername."' GROUP by __raids.raid_name;";
	...
	function multiDkpAllMemberArray($sortID)
	{
	   ...
	   member_status, r.rank_name, r.rank_hide, r.rank_prefix, r.rank_suffix,
	    ...
	    c.class_max_level AS max_level
     	FROM __members m, __member_ranks r, __class c
		...
		while ( $row = $db->fetch_record($members_result) )
    {
			...
			$pv_sql = "SELECT __raids.raid_name, SUM(raid_value)
					   FROM
					   __raid_attendees LEFT JOIN __raids ON __raid_attendees.raid_id = __raids.raid_id
					   WHERE __raid_attendees.member_name = '".$row['member_name']."' GROUP by __raids.raid_name";
	    ...
	    while( $pv_row = $db->fetch_record($pv_result, false) )
	    {
			...
			$ps_sql = "SELECT __raids.raid_name, SUM(__items.item_value)
			   ...
			   __items  LEFT JOIN __raids ON __items.raid_id = __raids.raid_id
			   WHERE
			   __items.item_buyer = '".$row['member_name']."' GROUP by __raids.raid_name;";
      ...
      $multi2event_results = get_events_for_poolid($sortID);
      reset($multi2event_results);
      while ( list($unused_key, $a_multi) = each($multi2event_results) )
			{ // gehe alle Events durch, die einem Konto zugewiesen wurden
				 $current = 0 ;
			...
			$members_rows_multidkp[$row['member_name']]['class_id']=$row['member_class_id'];
	...
	function showDKPLeaderboard($multifilter)
	{
					...
					{						
						if (!array_search(strtolower($key),$a_classes)  and (strtolower($key) <> 'unknown'))
						{
							...
							$tpl->assign_block_vars('classheader_row', array(
									'ICON'		 		  	=> get_ClassIcon($key,$a_classes_temp[$key]),
						...
						{							 
						 ...
						 $a_member_count[] = $membercount;						 						 	
						 
						 
					 }; # end for each classes
					 				 
					$maxhigh = round((max($a_member_count)*2.2));
				
					//Define the maximum height of the dynamicleaderboard
					$tpl->assign_vars(array('MAX_HIGH'			=> $maxhigh       				
       				));
		...
		else # kein multidkp -> normale ansicht
		{
			...
			$sql= 'SELECT DISTINCT class_name,  class_id  FROM __classes';
