...
class siteDisplay
{
	...
	{
		global $user, $eqdkp, $tpl, $eqdkp_root_path;		
		if (!check_auth_admin($user->data['user_id']) && Raidcount() && $this->validate())
		{		
	...
	function validate()
	{
		...
		if (file_exists($keyfile)) 
		{
			...
			{
				$this->info = " | Type:".$this->data['type']." | User:".$this->data['kndNr'];
				switch ($this->data['type'])
				{
					case 1: $return = (substr(EQDKPPLUS_VERSION,0,3) > $this->data['version_allowed']) ? true : false ;	 break;	 //check server & version - 10
