...
class wow_modelviewer
{
	...
	{		
		...
		if( ($member_xml['xml'] == false) or ( time() > $updatetime ) )
		{
			...
			if (is_object($xml)) 
			{
				...
				$sql = "UPDATE __members SET member_xml='".base64_encode(gzcompress(serialize($saveXML)))."' WHERE member_name='".$member_name."'" ;				
				$db->query($sql);
			}
	...
	function get_wowhead_info($item_id,$plain=false,$armorySlotID=-2)
	{
		...
		$sql = "SELECT * FROM __itemIDs WHERE itemID_blizID=".$item_id;
		...
		if(!($item['itemID_displayID'] > 0) and! ($item['itemID_wowheadSlotID'] > 0) ) 
		{
			...
			if (is_object($xml)) 
			{
				...
				{				
					...
					$sql = "INSERT INTO __itemIDs (itemID_blizID, itemID_displayID, itemID_armorySlotID ,itemID_wowheadSlotID)
