...
class dbPlus
{
	...
	function UpdateConfig($fieldname,$insertvalue, $confarray = false, $striptag=true)
      {
        ...
        $sql = "UPDATE __plus_config SET config_value='".$myinsertvalue."' WHERE config_name='".$fieldname."';";
        if ($db->query($sql)){
          return true;
        } else {
          return false;
    ...
    function InsertConfig($name)
      {
        global $db;
        	$sql = "INSERT INTO __plus_config ( `config_name` ) VALUES ('".$name."');";
        	$blubb = $db->query($sql);
      }
    ...
    function CheckDBFields($table, $field)
    {
      ...
      {
        global $table_prefix, $db;
        	$sql = "UPDATE __plus_update SET `".$fieldname1."`='".strip_tags(htmlspecialchars($insertvalue))."' WHERE `name`='".$pluginname."';";
        	$blubb = $db->query($sql);
      	if ($blubb){
          return true;
        } else {
   ...
   function InsertUpdateCache($pluginname)
      {
        global $table_prefix, $db;
        	$sql = "INSERT INTO __plus_update ( `name` ) VALUES ('".$pluginname."');";
        	$blubb = $db->query($sql);
      	if ($blubb){
          return true;
        } else {
  ...
  function InitConfig()
  {
		global $table_prefix, $db;
		// get the config
		$sqlplus = 'SELECT * FROM __plus_config';
	...
	function ProcessLinks($p_linkname, $p_linkurl, $p_linkwindow, $p_link_menu)
	{
    	...
    	{
	        $sql = "DELETE FROM __plus_links WHERE link_id='".$link_id."'";
	        $db->query($sql);
	        ...
	        }else
	        {
	            ...
	            $db->query("INSERT INTO __plus_links ".$query);
	         }
    	}
	}
	...
	function ProcesspRecruitment($p_classes)
	{
		...
		{
	        $sql = "DELETE FROM __plus_config WHERE config_name like '%pk_recruitment_class%'";
	        $db->query($sql);
			...
			foreach ( $p_classes as $class_id => $class_count )
	    	{
				...
				if (is_array($class_count)) // wow skillungsarray
				{
					foreach ($class_count as $skilcount => $skillvalue )
					{
			            ...
			            $db->query("INSERT INTO __plus_config ".$query);
					}
				}else //normal
				{
