...
class UpdateCheck EXTENDS EQdkp_Plugin
{
	...
	*/
	...
	{
		global $conf_plus, $table_prefix, $db, $eqdkp,$urlreader;
		$plusdb = new dbPlus();
		if ($this->CheckLink($this->vcurl))
		{
			...
			else
			{
				...
				{
					//Corgan: URL und Version wird mit übergeben
					$value .='|'.$eqdkp->config['server_name'].$eqdkp->config['server_path'].'|'.EQDKPPLUS_VERSION;
					$getdata = $urlreader->GetURL($this->vcurl.'/version/'.$value) ;
					...
					if(function_exists('curl_init'))
					{
						...
						curl_setopt($ch, CURLOPT_URL, $this->vcurl.'/version/'.$value);
		    		...
		    		{
						$getdata = @file_get_contents($this->vcurl.'/version/'.$value);
					}else
					{
						$pparray = file ($this->vcurl.'/version/'.$value);
						$getdata = $pparray[0];
					}
					*/
