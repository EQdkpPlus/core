...
class UpdateCheck EXTENDS EQdkp_Plugin
{
	...
	function UpdateChecker(){
		global $db, $user, $pm, $lang, $updatelevel;
		...
		if ($toupdate['window'] == 1){			
				$text .= "<tr><td width='90px'><img src='images/attention.png' class='reflect' style='float: left;' alt='".$lang['pk_alt_attention']."' \/></td>
									<td><div class='warning'>".$lang['pk_updates_avail']."</div></td>
									</tr><tr><td></td></tr><table><br/>
									<table class='updatetable'><tr><th width='160px'>".$lang['pk_module_name']."</th>
									<th width='70px'>".$lang['pk_inst_version']."</th>
									<th width='70px'>".$lang['pk_act_version']."</th>
									<th width='70px'>".$lang['pk_plugin_level']."</th>
									<th width='70px'>".$lang['pk_release_date']."</th>
									<th width='60px'></th></tr>";
			}else{
				$text .= "<tr><td width='90px'><img src='images/ok.png' class='reflect' style='float: left;' alt='".$lang['pk_alt_ok']."' \/></td>
								<td><div class='warning'>".$lang['pk_updates_navail']."</div></td>
								</tr><tr><td></td></tr><table><br/>";
			}
		foreach ($toupdate as $key => $value) {
			...
			if ($key != 'window'){
				...
				$level_txt = ( $updatelevel[$toupdate[$key]['level']]) ? $updatelevel[$toupdate[$key]['level']] : $lang['pk_level_other'];
				...
				$text .= '<td align="center">'.$toupdate[$key]['plugin'].'</td><td align="center">'.$toupdate[$key]['server'].'</td>
									...
									<td align="center">'.date($lang['pk_date_settings'] ,$toupdate[$key]['release']).'</td>
									<td align="center">
										<a href="'.$toupdate[$key]['changelog'].'" target="_blank"><img src="images/changelog.png" border="0" alt="'.$lang['pk_changelog'].'"/></a>
										<a href="'.$toupdate[$key]['download'].'" target="_blank"><img src="images/download.png" border="0" alt="'.$lang['pk_download'].'"/></a>
									</td></tr>';
			} // end if window
			}else{
				$upd_status = ($conf_plus['pk_updatecheck'] == 1) ? '<b><font color="green">'.$lang['pk_enabled'].'</font></b>!' : '<b><font color="red">'.$lang['pk_disabled'].'</font></b> '.$lang['pk_auto_updates2'];
				$text .= $lang['pk_no_updates'].'<br/>';
				...
				$text .= '<br/><center><table width="80%" border="0" cellspacing="1" cellpadding="2">
    							 ...
    							 <td class="row1" width ="54px" ><img class="reflect" src="images/info.png" width ="48px" height="48px" /></td>
    								<td class="row1">'.$lang['pk_auto_updates1'].' '.$upd_status.'</td>
  								</tr>
  								<tr>
									</table></center>';				
			}
			...
			$text .= "<tr><td width='90px'><img src='images/connect_no.png' class='reflect' style='float: left;' alt='".$lang['pk_alt_error']."' \/></td>
								<td><div class='warning'>".$lang['pk_no_conn_header']."</div></td>
								</tr><tr><td></td></tr><table><br/>";
			$text .= $lang['pk_no_server_conn'].'<br/>';
		}
		$text .= "</table>
		</center>";
		$text .= "<br/><center><a href='updates.php?reset=true'>".$lang['pk_reset_warning']."</a></center>";
		return $text;
	}
