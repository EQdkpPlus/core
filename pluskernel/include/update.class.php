...
class UpdateCheck EXTENDS EQdkp_Plugin
{
	...
	// Plugin Version
	var $PlusUpdateClassVersion 	= '1.0.0';
	var $PlusUpdateClassBuild		= '270220071351';
	
	function UpdateCheck($url1, $url2, $pluglist, $coltable, $version, $author, $auturl){
		$this->vcurl 				= $url1;
		$this->vcserver			= $url2;
		$this->pluginlist		= $pluglist;
		$this->colortable		= $coltable;
		$this->plusversion	= $version;
		$this->plusauthor		= $author;
		$this->authorurl		= $auturl;
	}
		...
		if ($this->CheckLink($this->vcurl)){
			...
			foreach ($this->pluginlist as $value) {
    			...
    			$ch = curl_init();
					...
					curl_setopt($ch, CURLOPT_URL, $this->vcurl.'?plugin='.$value);
...
function Copyright()
{
	global $eqdkp ;
		$copyright='<br/><center><span class="copy"> 
								...
								<br /><a href=http://www.eqdkp-plus.com target=_new class=copy>EQDKP Plus</a> '.$this->PlusVersion().' &copy; '.$this->CopyRightYear().' by
