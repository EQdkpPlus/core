...
{ 
  ...
  { 
    ...
    function Save()
    {
      ...
      if($this->UserID){
        $htmlinsert = htmlentities(strip_tags($_POST['comment']), ENT_QUOTES); /*No html or javascript in comments*/
    ...
    function Content($atachid, $page, $rpath='', $issave = false, $mylang='')
    {
      ...
      $bbcode->SetSmiliePath($myrootpath.'pluskernel/include/jquery/img/editor/icons');
      ...
      if (is_array($commdata)) 
      {
	      ...
	      {
          // Username Convertion Shit
          $myStupidUsername = ($issave) ? htmlspecialchars(utf8_encode($row['username'])) : htmlspecialchars($row['username']);
          $textwithsmilies  = $bbcode->MyEmoticons($row['message']);
          $rowcolor         = ($i%2) ? 'rowcolor2' : 'rowcolor1';
	        $out[] .= '<div class="'.$rowcolor.' clearfix">
	                      <div class="floatLeft" style="overflow: hidden; width: 15%;">'.$myStupidUsername.'</div>
          ...
          $out[] .= '<br class="clear"/><span class="comment_text">'.$textwithsmilies.'</span><br/>
	                      </div>
	                    </div><br/>';
	        $i++;
	      }
