 ...
 * (c) 2008 by Wallenium 
 * ---------------------------
 * $Id: comments.class.php 1602 2008-03-04 09:01:31Z wallenium $
 ******************************/
...
if (!class_exists("rpComments")) { 
  ...
  { 
    ...
    function rpComments($table, $adminauth='a_config_man'){
      ...
      $this->langprf  = (defined(EQDKPPLUS_VERSION)) ? 'comments_' : 'rp_';
      $this->version  = '1.0.0';
    }
    ...
    function Save(){
      ...
      VALUES 
      ('".$_POST['attach_id']."', '".time()."', '".$_POST['userid']."', '".$bbcode->toHTML($_POST['comment'])."', '".$_POST['page']."');";
      $db->query($sql);
  		echo $this->Content($_POST['attach_id'], $_POST['page']);
    }
    ...
    function Delete(){
      ...
      $db->query($sql);
      echo $this->Content($_GET['attach_id'], $_GET['page']);
    }
      ...
      while($row = $db->fetch_record($comm_result)){
        ...
        $out[] .= '<div class="'.$rowcolor.' clearfix">
                      ...
                      <div class="floatLeft" style="overflow: hidden; width: 85%;">
                        ...
                        <br class="clear"><span class="comment_text">'.$row['text'].'</span><br>
                      </div>
                    </div><br>';
        $i++;
      }
