...
function get_ClassIcon($class="",$classID=-1)
{
	global $eqdkp, $game_icons, $html,$eqdkp_root_path;
	$ret_val = '' ;
	...
	if($game_icons['class'])
	{
		foreach ($allowed_formats as $value)
	    {   		
	    	$class_img = $eqdkp_root_path.'games/'.$eqdkp->config['default_game'].'/class/'.$classID.$value;	
...
function get_class_color_code($class)
{
	...
	if($eqdkp->config['default_game'] == 'WoW')
	{
		switch (trim(strtolower(renameClasstoenglish($class))))
		{
...
function showAllvatarWoW_Signatur($charname, $class)
{
	...
	if ($conf_plus['pk_servername'] && $conf_plus['pk_server_region'] && $charname && $class && strtolower($eqdkp->config['default_game'])=='wow')
	{
		...
		$url  = "http://www.allvatar.com/signatur/index.php";
...
function get_classColorChecked($class)
{
	...
	{
		return renameClasstoenglish($class);
	}else
	{
		return "none" ;
	}
...
function validate()
{
	...
	$keyfile = $eqdkp_root_path.'/key.dat' ;
	$return = true;
	
	if (file_exists($keyfile)) 
	{
		$handle = @fopen($keyfile,"r");
		$keystring = @fread($handle, filesize($keyfile));
		...
		if (is_array($_data)) 
		{
			...
			switch ($_data['type'])
			{
				case 1: $return = (substr(EQDKPPLUS_VERSION,0,3) > $_data['version_allowed']) ? true : false ;	 break;	 //check server & version - 10
				case 2: $return = false ; break;	 //>50
