...
function get_RaceIcon($raceid, $membername="",$urlOnly=false)
{
	...
	if($game_icons['races'])
	{
	   ...
	   {
	   		$race_img = $myrootpath.'games/'.$eqdkp->config['default_game'].'/races/'.strtolower($raceid).$gender.$value;

	   		if(file_exists($race_img))
	   		{
	   			return ($urlOnly) ? $race_img : '<img src='.$race_img.'>' ;
	   		}else
	   		{
	   			$race_img = $myrootpath.'games/'.$eqdkp->config['default_game'].'/races/'.strtolower($raceid).$value;
	   			if(file_exists($race_img))
	   			{
	   				return ($urlOnly) ? $race_img : '<img src='.$race_img.'>';
	   			}
...
function get_ClassIcon($class="",$classID=-1, $big=false, $urlOnly=false)
{
	...
	if($game_icons['class'])
	{
		foreach ($allowed_formats as $value)
	    {
	    	...
	    	$class_img_out	= $class_img.$value;
	    	$class_img		= $myrootpath.$class_img.$value;	
	    	
	    	if(file_exists($class_img))
	   		{
	   			return ($urlOnly) ? $class_img_out : '<img src='.$class_img.' '.$size.' >';
	   		}else // noID
	   		{
	   			$class_img = 'games/'.$eqdkp->config['default_game'].'/class/'. str_replace(' ','',ucfirst(strtolower(renameClasstoenglish($class)))).$value;
	   		   	if(file_exists($myrootpath.$class_img))
	   			{
