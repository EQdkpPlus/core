...
function runden($value){
	...
	$ret_val = $value;
	$precision = ($conf_plus['pk_round_precision']) ? $conf_plus['pk_round_precision'] : 0;
...
function renameRacetoenglish($class)
{
	...
	if($eqdkp->config['default_game'] == 'WoW')
	{
		switch ($class)
		{
			...
			case "Gnome"       		: $_return = "Gnome";break;
			case "Humain" 			: $_return = "Human";break;
			case "Nain"        		: $_return = "Dwarf";break;
	...
	if($game_icons['rank'])
	{
		if (($conf_plus['pk_rank_icon'] == 1))
		{
			$rank_img = $eqdkp_root_path.'games/'.$eqdkp->config['default_game'].'/rank/'.strtolower($rank).'.gif';
		   	...
		   	{
		   		return '<img src='.$rank_img.' title="'.$rank.'" alt="Rank='.$rank.'">';
		   	}
...
function get_RaceIcon($raceid, $membername="",$urlOnly=false)
{
	global $eqdkp, $game_icons, $html,$eqdkp_root_path;
	$ret_val = '' ;
	$myrootpath = ($urlOnly) ? '' : $eqdkp_root_path;
	...
	if($game_icons['races']){
	   ...
	   $allowed_formats = array('.gif','.png','.jpg');
	   foreach ($allowed_formats as $value){
	   		$race_img = 'games/'.$eqdkp->config['default_game'].'/races/'.strtolower($raceid).$gender.$value;

	   		if(file_exists($eqdkp_root_path.$race_img)){
	   			return ($urlOnly) ? $race_img : '<img src='.$eqdkp_root_path.$race_img.'>' ;
	   		}else{
	   			$race_img = 'games/'.$eqdkp->config['default_game'].'/races/'.strtolower($raceid).$value;
	   			if(file_exists($eqdkp_root_path.$race_img)){
	   				return ($urlOnly) ? $race_img : '<img src='.$eqdkp_root_path.$race_img.'>';
	   			}
	   		}
...
function get_ClassIcon($class="",$classID=-1, $big=false, $urlOnly=false)
{
	global $eqdkp, $game_icons, $html, $eqdkp_root_path;
	$ret_val = '' ;
	...
	if($game_icons['class'])
	{
		foreach ($allowed_formats as $value)
	    {
	    	$class_img = 'games/'.$eqdkp->config['default_game'].'/class/'.$classID;
	    	...
	    	if(file_exists($eqdkp_root_path.$class_img)){
//echo ($urlOnly) ? $class_img_out : '<img src='.$class_img.' '.$size.'<br>';
	   			return ($urlOnly) ? $class_img : '<img src='.$eqdkp_root_path.$class_img.' '.$size.' >';
	   		}else // noID
	   		{
	   			$class_img = 'games/'.$eqdkp->config['default_game'].'/class/'. str_replace(' ','',ucfirst(strtolower(renameClasstoenglish($class)))).$value;
	   		   	if(file_exists($eqdkp_root_path.$class_img))
	   			{
	   				return ($urlOnly) ? $class_img : '<img src='.$eqdkp_root_path.$class_img.' height=18 width=18 >';
		   		}
 ...
 * @return html string
 */
function getEventIcon($event, $size='16',$urlOnly=false)
{
	...
	if($game_icons['event'])
	{
       if(empty($en2ei))
       {
       	...
       	$icon = $en2ei[$event];
       	$img_path = 'games/'.$eqdkp->config['default_game'].'/events/' ;
		if($size <> 'org'){$_size = "height='".$size."' width='".$size."'";}
       
   	   if(file_exists($eqdkp_root_path.$img_path.$icon))
	   {
	   		return ($urlOnly) ? $img_path.$icon : "<img ". $_size." src='".$eqdkp_root_path.$img_path.$icon."'> ";
	  	}else 
	  	{
	  	    $allowed_formats = array('.png', '.jpg');
	  	    $filename = explode ( '.' , $icon );	  	
	   		foreach ($allowed_formats as $value)
	   		{
				$tmpname = $filename[0].$value ;
				if(file_exists($eqdkp_root_path.$img_path.$tmpname))
	   			{
	   				return ($urlOnly) ? $img_path.$tmpname : "<img ". $_size." src='".$eqdkp_root_path.$img_path.$tmpname."'> " ;
	   			}
			}
	...
	if ($pm->check(PLUGIN_INSTALLED, 'charmanager') and isset($member))
	{
  		...
  		if (!$gender_member)
  		{
			if (empty($gender_member))
			{
		    	...
		    	$pdc->put('dkp.functions.Gender',$gender_member,86400,false,true);
			}
...
{
	global $user, $conf_plus, $html, $eqdkp,$game_icons,$eqdkp_root_path;
...
function check_auth_admin($user_id=0)
{
    ...
    if ( $db->num_rows($myrows) > 0) return true;
    else return false;
}

function Raidcount()
{
	global $db, $eqdkp, $_HMODE ;
	$sql = "SELECT count(raid_id) from ".RAIDS_TABLE ;
	$count = $db->query_first($sql);	
	$raidcount = ($count > 2) ? true : false ;
	$runtime_log = false;
	
	if ($_HMODE) 
	{
		$sql = "SELECT log_date from ".LOGS_TABLE." ORDER BY log_id LIMIT 1";
		$first_log_date = $db->query_first($sql);	
		$runtime_log = ($first_log_date-1209600 > 1) ? true : false;
	}else {
		$runtime = (time()-$eqdkp->config['eqdkp_start'] > 1814400) ? true : false ;		
	}
	
	if ($raidcount or $runtime or $runtime_log)
	{
		return	true ;
	}else
	{
 ...
 *
 * 	Usage: getLanguageArrayFromHTTPAcceptString($_SERVER['HTTP_ACCEPT_LANGUAGE']);
 * 	foreach ((array)$arrBrowserLanguages as $lanKey => $weigth)
 *	{if ($lanKey == 'de') $_SESSION['lang'] = 'de';break;}
 * 
 * @param $_server array 
 * @return array
 */
function getLanguageArrayFromHTTPAcceptString($strHTTPAccept)
{
		$arrLang = array();
		if (empty($strHTTPAccept)) return $arrLang;
		
		$arrParts = explode(",",$strHTTPAccept);
		foreach ((array)$arrParts as $strPart) 
		{
			if (preg_match("/(.*);q=([0-1]{0,1}\.\d{0,4})/i",$strPart,$arrMatch)) $arrLang[$arrMatch[1]] = $arrMatch[2];
			else $arrLang[$strPart] = 1;
		}
		unset($arrParts);
		
		// begin with smallest and strip not needed countries from locales
		asort($arrLang);
		foreach ((array) $arrLang as $strLang => $strQ)
		{
			$arrParts = explode('-',$strLang);
			$arrLangClean[$arrParts[0]] = $strQ;
			unset($arrParts);
		}

		// resort array and return
		arsort($arrLangClean);
		return $arrLangClean;
}

/**
 * Return the userlanguage from guest AND loged in users
 *
 * check the given Userlanguage if user is loged in.
 * if guest, then use the browser language.
 * return fulltext language like german, english
 * 	
 * @return string
 */
...
{	
	...
	if(!isset($userLang))
	{
		$arrBrowserLanguages = getLanguageArrayFromHTTPAcceptString($_SERVER['HTTP_ACCEPT_LANGUAGE']);
		foreach ((array)$arrBrowserLanguages as $lanKey => $weigth)
		{									
			if($lanKey == 'de')
			{ 
	...
	}else //if user loged in, check of from usa.
	{
		if($user->data['country'] =='US')
		{
			$userLang = 'english-us' ;
		}
	}
...
function isMobilePhone()
{
	...
	if ( $user->data['user_id'] != ANONYMOUS )
	{
     	...
     	$return = ( ereg('iPhone|iPod',$_SERVER['HTTP_USER_AGENT']) > 0) ? true : false ;
