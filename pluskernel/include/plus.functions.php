...
function renameClasstoenglish($class)
{
	...
	elseif ($eqdkp->config['default_game'] == 'LOTRO_german' || $eqdkp->config['default_game'] == 'LOTRO_english') {
		switch ($class)
		{
...
{
	global $eqdkp,$conf_plus;
	$ret_val = $rank ;

	if($eqdkp->config['default_game'] == 'WoW_german' || $eqdkp->config['default_game'] == 'WoW_english')
	{
	   $rank_img = './images/wow_rank/'.strtolower($rank).'.gif';
	   if(file_exists($rank_img) && ($conf_plus['pk_rank_icon'] == 1))
	   {$ret_val = '<img src='.$rank_img.' alt="Rank='.$row['rank_name'].'">';}
	}

	if($eqdkp->config['default_game'] == 'LOTRO_english' || $eqdkp->config['default_game'] == 'LOTRO_german')
	{
	   $rank_img = './images/lotro_rank/'.strtolower($rank).'.gif';
	   if(file_exists($rank_img) && ($conf_plus['pk_rank_icon'] == 1))
	   {$ret_val = '<img src='.$rank_img.' alt="Rank='.$row['rank_name'].'">';}
	}
...
function get_RaceIcon($raceid)
{
	...
	if($eqdkp->config['default_game'] == 'WoW_german' || $eqdkp->config['default_game'] == 'WoW_english')
	{
	   $race_img = './images/wow_races/'.strtolower($raceid).'.gif';
	   if(file_exists($race_img))
	   {$ret_val = '<img src='.$race_img.'>';}}

	if($eqdkp->config['default_game'] == 'LOTRO_english' || $eqdkp->config['default_game'] == 'LOTRO_german')
	{
	   $race_img = './images/lotro_races/'.strtolower($raceid).'.gif';
	   if(file_exists($race_img))
	   {$ret_val = '<img src='.$race_img.'>';}}
	return $ret_val;
}
...
function get_classImgListmembers($class)
{
	...
	if( $eqdkp->config['default_game'] == 'WoW_german'  || $eqdkp->config['default_game'] == 'WoW_english'
		&& array_search($class,$wow_classes)>0 )
	{
		$img= "<img width=18 height=18 src='".$eqdkp->config['server_path']."./images/wow_class/".ucfirst(strtolower(renameClasstoenglish($class))).".gif' alt=''>";
	...
	{
		$img= "<img width=18 height=18 src='".$eqdkp->config['server_path']."./images/lotro_class/".strtolower(renameClasstoenglish($class)).".gif' alt=''>";
...
function get_classNameImgListmembers($class)
{
	...
	if($eqdkp->config['default_game'] == 'WoW_german' || $eqdkp->config['default_game'] == 'WoW_english'
	  && array_search(strtolower($class),$wow_classes)>0 )
	{
		$img= "<img width=18 height=18 src='".$eqdkp->config['server_path']."./images/wow_class/".ucfirst(strtolower(renameClasstoenglish($class))).".gif' alt=''>";
...
function get_wow_talent_spec($class, $skill1, $skill2, $skill3, $member, $last_update)
{
 ...
 $icon = $img= "<img src='".$eqdkp->config['server_path']."./images/wow_talents/".strtolower($class).$spec_number.".png' alt=''>";
