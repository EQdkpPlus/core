...
abstract class elFinderVolumeDriver {
	...
	public function file($hash) {
		...
		//Security
		if (!isFilelinkInFolder($path, get_absolute_path($this->root)) && ($path !== $this->root)) return $this->setError(elFinder::ERROR_FILE_NOT_FOUND);
	...
	public function upload($fp, $dst, $name, $tmpname) {
		...
		//Max Files in Dir
		if (isset($this->options['maxFilesInDir'])){
			$intFilesInDir = count($this->scandir($dst));
			if ($intFilesInDir > $this->options['maxFilesInDir']){
				return $this->setError(elFinder::ERROR_UPLOAD_TOTAL_SIZE);
			}
		}
		...
		// $this->clearcache();
		if (($path = $this->_save($tmpname, $dstpath, $name, $mime, $w, $h)) == false) {
			return false;
		}
	...
	protected function rmTmb($tmb) {
		...
		file_exists($tmb) && register('pfh')->Delete($tmb);
		clearstatcache();
	}
