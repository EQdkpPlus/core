...
// Begin
    ...
    $u_list_items = 'itemsearch.php'.$SID.'&amp;search=' . $_GET['search'] . '&amp;search_type=' . $_GET['search_type'] . '&amp;unique_result=' . $_GET['unique_result'] . '&amp;';
...
if (isset($_GET['search']))
{
	if ($_GET['search_type'] == 'itemname')
		{
		$searchString = " AND i.item_name LIKE '%" . $_GET['search'] . "%' ";
		$search_type = 'item_name';
		$search_table = ITEMS_TABLE;
	}elseif ($_GET['search_type'] == 'buyer'){
		$searchString = " AND i.item_buyer LIKE '%" . $_GET['search'] . "%' ";
		$search_type = "item_buyer";
		$search_table = ITEMS_TABLE;
	}elseif ($_GET['search_type'] == 'raidname'){
		$searchString = " AND r.raid_name LIKE '%" . $_GET['search'] . "%' ";
	...
	$total_items = $db->query_first('SELECT count(*) FROM ' . $search_table . ' WHERE ' . $search_type . " LIKE '%" . $_GET['search'] . "%' ");// . $group_unique);
    ...
    $pagination = generate_pagination('itemsearch.php'.$SID.'&amp;search=' . $_GET['search'] . '&amp;search_type=' . $_GET['search_type'] . '&amp;unique_result=' . $_GET['unique_result'] . '&amp;o='.$current_order['uri']['current'],
