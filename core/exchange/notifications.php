				...
				foreach($arrNotifications as $arrNotification){
					if (isset($arrNotification['persistent'])) {
						...
						$arrOut[] = array(
							...
							'link'		=> $arrNotification['link'],
							'time'		=> (!$arrNotification['read']) ? $this->env->buildlink().$this->routing->build('Notifications', false, false, false, true).'&redirect='.$arrNotification['id'] : $this->env->link.$this->controller_path_plain.$arrNotification['link'],
