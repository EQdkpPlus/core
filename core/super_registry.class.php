...
abstract class super_registry {
	...
	public static function init($root_path, $lite=false) {
		...
		if(!$lite) {
			...
			//Rules accepted?
			if (registry::fetch('user')->is_signedin() && (int)registry::fetch('user')->data['rules'] != 1){
				if (registry::register('environment')->request_page != 'register.php'){
	...
	public function isset_const($name) {
