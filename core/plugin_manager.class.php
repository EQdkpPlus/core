...
class plugin_manager extends gen_class {
	public static $shortcuts = array('pdh', 'pdl');
	...
	*/
	...
	private function register_hooks($plugin_code) {
		...
		foreach($hooks as $hook => $function) {
			$this->hooks[$hook][$plugin_code] = $function;
		}
	}
	
	private function short_hook($hook) {
	...
	/*
	 * Do Hooks (call plugin-functions)
	 * @string				$hook
	 * @mixed				$params
	 * @string/int/boolean	$recursive	The Key in the Params-Array, where the retval of the previous plugin shall be put.
	 * @string				$plugin
	 * return @mixed
	 */
	public function do_hooks($hook, $params=array(), $recursive=false, $plugin='') {
		$retval = array();
		$hook = $this->short_hook($hook);
		if(!isset($this->hooks[$hook])) return ($recursive !== false) ? $params[$recursive] : array();
		foreach($this->hooks[$hook] as $plugin_code => $function) {
			if((!$plugin OR $plugin == $plugin_code) && $this->check($plugin_code, PLUGIN_INSTALLED & PLUGIN_INITIALIZED)){
				if($recursive) {
					$retval = $this->get_plugin($plugin_code)->$function($params);
					$params[$recursive] = $retval;
				} else {
					$retval[$plugin_code] = $this->get_plugin($plugin_code)->$function($params);
				}
			}
		}
		return $retval;
