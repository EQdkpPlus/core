...
if (!class_exists("comments")){
	class comments extends gen_class {
		public static $shortcuts = array('user', 'tpl', 'pdh', 'time', 'in',
			'bbcode'	=> 'bbcode',
		);
		...
		public function Content($attachid, $page, $rpath='', $issave = false){
			...
			// The delete form
			$html	= '<form id="comment_delete" name="comment_delete" action="'.$this->root_path.'exchange.php?out=comments" method="post">';
			$html	.= '</form>';
				...
				foreach($comments as $row){
					...
					$out[] .= '<div class="'.(($i%2) ? 'rowcolor2' : 'rowcolor1').' clearfix">
								<div class="floatLeft" style="overflow: hidden; width: 15%;">
									<div class="comment_avatar"><img src="'.(($avatarimg) ? $avatarimg : $myrootpath.'images/no_pic.png').'" alt="Avatar" /></div>
								</div>
		...
		private function Form($attachid, $page){
			...
			$html .= '<form id="comment_data" name="comment_data" action="'.$this->root_path.'exchange.php?out=comments" method="post">
			...
			$jscode = "
						...
						$(document).on('click', '.comments_delete', function(){
							...
							$('#comment_delete').ajaxSubmit({
								...
								url:		myrootpath+'exchange.php?out=comments&deleteid='+deleteid+'&page='+page+'&attach_id='+attachid+'&rpath='+myrootpath,
								success: function() { 
