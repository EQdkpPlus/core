...
class wbb54_bridge extends bridge_generic {

	public static $name = 'Woltlab Community Suite 5.4';
	...
	private function sso($arrUserdata, $boolAutoLogin, $strPassword){
		...
		//Load Config
		$config = array();
		$objQuery =  $this->bridgedb->query("SELECT * FROM ".$this->prefix."option WHERE optionName = 'cookie_prefix' OR optionName = 'signature_secret'");
		if($objQuery){
		...
		$xsrfToken = $this->createSignedString($bytes, $config['signature_secret']);
		...
		$arrVirtualSet = array(
		    ...
		    'sessionVariables'    => \serialize([
		        'acp' => [],
		        'frontend' => ['__SECURITY_TOKEN' => $xsrfToken],
		    ])
		);
		
		//Is there an existing virtual Session?
		$this->bridgedb->prepare("INSERT INTO __user_session :p")->set($arrVirtualSet)->execute();
		...
		//XSRF Cookie
		setcookie('XSRF-TOKEN', $xsrfToken, $expire, $config['cookie_path'], $config['cookie_domain'], $this->env->ssl);
		
		
		//SID Cookie
		setcookie($config['cookie_prefix'].'user_session', $this->createSignedString(\pack(
		    'CA20C',
		    1,
		    \hex2bin($strSessionID),
		    0
		    ), $config['signature_secret']), $expire, $config['cookie_path'], $config['cookie_domain'], $this->env->ssl);
		return true;
	}
	
	public function createSignature($value, $secret){
	    return \hash_hmac('sha256', $value, $secret);
	}
	
	public function createSignedString($value, $secret){
	    return $this->createSignature($value, $secret) . '-' . base64_encode($value);
	}
	...
	public function autologin($arrCookieData){
		...
		$strSession = isset($_COOKIE[$config['cookie_prefix'].'user_session']) ? $_COOKIE[$config['cookie_prefix'].'user_session'] : null;
        if(!$strSession) return false;
		
		$strValueFromSessionCookie = $this->getValueFromSignedString($strSession, $config['signature_secret']);
		if ($strValueFromSessionCookie && \mb_strlen($strValueFromSessionCookie, '8bit') === 22) {
		    $sessionID = \bin2hex(\mb_substr($strValueFromSessionCookie, 1, 20, '8bit'));
		} else {
		    return false;
		}
		
		$result = $this->bridgedb->prepare("SELECT * FROM ".$this->prefix."user_session WHERE sessionID=?")->execute($sessionID);
		if ($result){
	...
	public function getValueFromSignedString($string, $secret){
	    if (!$this->validateSignedString($string, $secret)) {
	        return null;
	    }
	...
	public function validateSignedString($string, $secret){
	    $parts = \explode('-', $string, 2);
	    if (\count($parts) !== 2) {
	        return false;
	    }
	    ...
	    return \hash_equals($signature, $this->createSignature($value, $secret));
	}
	...
	public function logout(){
		...
		setcookie($config['cookie_prefix'].'user_session', 'somevalue', 0, $config['cookie_path'], $config['cookie_domain'], $this->env->ssl);
		setcookie('XSRF-TOKEN', 'somevalue', 0, $config['cookie_path'], $config['cookie_domain'], $this->env->ssl);
	...
	private function __checkPassword($username, $password, $hash) {
		...
		if (($pos = strpos($hash, ':')) !== false) {
		    $hashType = substr($hash, 0, $pos);
		}
		
		// check if password is a valid bcrypt hash
		if ($hashType == "Bcrypt" || self::isBlowfish($hash)) {
		    // drop type from hash
		    if(strlen($hashType)) $hash = substr($hash, strlen($hashType)+1);
