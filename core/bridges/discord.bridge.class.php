...
class discord_bridge extends bridge_generic {
	...
	public function login($strUsername, $strPassword, $boolSetAutologin=false, $boolUseHash=false){
		...
		$result = register('urlfetcher')->post('https://discordapp.com/api/v6/auth/login', json_encode($arrData), 'application/json; charset=utf-8');
		...
		if($result){
			...
			if($arrJSON && is_array($arrJSON)){
				...
				$result = register('urlfetcher')->fetch('https://discordapp.com/api/v6/users/@me', array('Authorization: '.$token));
				if($result){
					$arrJSON = json_decode($result, true);
					if($arrJSON['id']){
