...
class xmpp_messenger extends generic_messenger {
	...
	public function sendMessage($toUserID, $strSubject, $strMessage){
		...
		{
			if(defined('DEBUG') && DEBUG > 1) pd("Jabber Connection Error:<br/>".$this->objJabber->get_log());
			return false;
		}
		
		if (!$this->objJabber->login())
		{
			if(defined('DEBUG') && DEBUG > 1) pd("Jabber Login Error:<br/>".$this->objJabber->get_log());
			return false;
		}
	...
	public function isAvailable(){
		if($this->config->get('xmpp_server') != "" && $this->config->get('xmpp_user') != "" && $this->config->get('xmpp_password') != "" && $this->config->get('xmpp_port') != ""){
			return true;
		}
 ...
 *
 * Only slightly modified by GodMod
 */
class jabber
{
	var $connection = null;
	...
	var $password;
	var $use_ssl;
	var $resource = 'eqdkp-plus.notifications';
				...
				if (isset($this->session['authenticated']))
				{
					...
					$this->send("<iq type='set' id='bind_1'>
						<bind xmlns='urn:ietf:params:xml:ns:xmpp-bind'>
							<resource>" . htmlspecialchars($this->resource) . '</resource>
						</bind>
					</iq>');
					return $this->response($this->listen());
				}
	...
	function send_message($to, $text, $subject = '', $type = 'normal')
	{
		...
		return $this->send("<message from='" . htmlspecialchars($this->session['jid']) . "' to='" . htmlspecialchars($to) . "' type='$type' id='" . uniqid('msg') . "'>
