 ...
 * @version		$Rev: 11419 $
 * 
 * $Id: auth_db.class.php 11419 2011-11-01 12:38:39Z hoofy $
 */
...
class auth extends user_core {
	...
	*/
	public function start(){
		...
		if ($this->sid != ''){
			...
			if ($arrResult && (($arrResult['session_start'] + $this->session_length) > $this->current_time) ){			
				...
				if (($arrResult['session_ip'] === $this->env->ip) && ($arrResult['session_browser'] === $this->env->useragent)){
					...
					registry::add_const('SKEY', $arrResult['session_key']);
					return true;
				}		
			}	
		}
		...
		$this->create($this->data['user_id'], (isset($arrCookieData['data']['auto_login_id']) ? $arrCookieData['data']['auto_login_id'] : ''), $boolSetAutoLogin);
		$this->id = $this->data['user_id'];
		return true;
	}
	...
	* @return true
	*/
	public function create ($user_id, $strPwdHash, $boolSetAutoLogin = false){
		if (!$user_id) $user_id = ANONYMOUS;
		$this->sid = substr(md5(rand().uniqid('', true).rand()).md5(rand()), 0, 40);
		registry::add_const('SKEY', $this->generate_session_key());
		$this->current_time = $this->time->time;
		$arrData = array(
				'session_id'			=> $this->sid,
		...
		// set the cookies
		set_cookie('data', base64_encode(serialize($arrCookieData)), $this->current_time + 31536000);
