			...
			foreach(array(1,2,3,4) as $mk) {
				...
				if ($this->config->get('sort_menu'.$mk)){
					...
					foreach ($menus['menu'.$mk] as $key=>$menu){
						...
						if (isset(${'sort_ary'.$mk}[$link]['sort'])){
							${'tmp_menu'.$mk}[$key] = ${'sort_ary'.$mk}[$link]['sort'];
						} else {
							${'tmp_menu'.$mk}[$key] = 999999;
						}
						if (isset(${'sort_ary'.$mk}[$link]['hide']) && ${'sort_ary'.$mk}[$link]['hide']){
		...
		public function gen_menus(){
			...
			if ( $this->user->is_signedin() ){
				...
				if ($this->config->get('cmsbridge_active') == 1 && $this->config->get('cmsbridge_reg_redirect') == 1){
					...
					$main_menu2[] = array('link' => 'register.php' , 'text' => $this->user->lang('menu_register'));
				}
			}

			// Switch login/logout link
			$main_menu2[] = ($this->user->is_signedin()) ? array('link' => 'login.php' . $this->SID . '&amp;logout=true', 'text' => $this->user->lang('logout') . ' [ ' . $this->user->data['username'] . ' ]') : array('link' => 'login.php' . $this->SID, 'text' => $this->user->lang('login'));
