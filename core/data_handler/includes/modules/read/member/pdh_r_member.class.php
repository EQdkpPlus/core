...
if ( !class_exists( "pdh_r_member" ) ) {
	class pdh_r_member extends pdh_r_generic{
		...
		$shortcuts = array('pdc', 'db', 'pdh', 'game', 'user', 'html', 'config', 'jquery', 'xmltools'=>'xmltools'	);
		...
		public function get_id_list($skip_inactive=false, $skip_hidden=false, $skip_special = true, $skip_twinks=false){
			$members = array();
			$special_members = ($this->config->get('special_members')) ? unserialize($this->config->get('special_members')) : array();
		...
		public function get_html_racename($member_id){
			...
			return $this->game->decorate('races', array($this->get_raceid($member_id),$gender));
		}
