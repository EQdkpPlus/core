		...
		public function init(){
			...
			if($objQuery){
				while($row = $objQuery->fetchAssoc()){
					...
					$this->roles[$row['role_id']]['classes']	= (substr_count($row['role_classes'], "|") > 0) ? explode("|", $row['role_classes']) : ((is_array($row['role_classes']) && count($row['role_classes']) > 0) ? array($row['role_classes']) : array());
