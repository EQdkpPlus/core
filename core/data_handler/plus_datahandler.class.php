		...
		public function comp( $module, $tag, $direction, $params1, $params2 ) {
			...
			if( method_exists( $this->rm($module), $comp_method ) ) {
				...
				if(!is_numeric($value1) && !is_numeric($value2)) {
				    if(is_array($value1) || is_array($value2)) return 0;
				    return strnatcasecmp($value1, $value2)*$direction;
				}
				if( $value1 > $value2 ) {
					return 1 * $direction;
				}
		...
		*/
		...
		public function auto_update_layout( $layout_name ) {
			...
			if($layout_name == $layout_def['base_layout']){
				...
				if($changes){
					$user_layout_name = 'user_'.$layout_name;
					$arrResult = $this->patch_pageobjects($user_layout_name, $layout_def);
					$layout_def = $arrResult[0];
