				...
				foreach($repeatable_events as $parentid=>$a_childid){
					...
					if($parentid > 0){
						...
						while($date_event_add < $end_cronjob){
							...
							$this->pdh->put('calendar_events', 'auto_addchars', array($eventsdata['extension']['raidmode'], $clonedraidid, $eventsdata['extension']['raidleader'], $this->cleanExtensionField($eventsdata['extension']['autosignin_group']), $this->cleanExtensionField($eventsdata['extension']['autosignin_status'], false)));
