			...
			case 'wrapper':
				...
				if (isset($params['id'])){
					...
					if (is_numeric($link_id)){
						$page.=': <a href="'.registry::get_const('root_path').'wrapper.php?id='.$link_id.'">'.sanitize(registry::register('plus_datahandler')->get('links', 'name', array($link_id))).'</a>';
					}else{

						switch($link_id){
							case 'lp':
							case 'board':
								$page.=': <a href="'.registry::get_const('root_path').'wrapper.php?id=board">'.registry::fetch('user')->lang('forum').'</a>';
						}
					}
				}
			break;
...
function infotooltip($name='', $game_id=0, $lang=false, $direct=0, $onlyicon=0, $noicon=false, $char_name='', $server=false, $in_span=false, $class_add='', $slot=''){
	...
	$data = serialize($data);
	$direct = ($direct) ? 1 : 0;
	$str = '<span class="infotooltip '.$class_add.'" id="span_'.$id.'" title="'.$direct.base64_encode($data).'">';
...
function clean_username($strUsername){
	return utf8_strtolower($strUsername);
}
...
// Debug functions
function pd($var='not_set', $die=false) {
