...
class game extends gen_class {
	 ...
	 * @return html string
	 */
	private function decorate_def_events($event_id, $size=16, $pathonly=false){
	...
	private function load_class_color(){
		if(!isset($this->class_colors['fetched']) || $this->class_colors['fetched'] == false) {
			$this->class_colors = $this->pdh->get('class_colors', 'class_colors', array($this->user->style['style_id']));
			if(!is_array($this->class_colors)) {
	...
	public function get_class_color($class_id){
		...
		return (isset($this->class_colors[$class_id])) ? $this->class_colors[$class_id] :  '';
	}
		...
		if(is_array($xml_fields)){
			foreach($xml_fields as $name=>$values) {
				$this->pdh->put('profile_fields', 'insert_field', array(array(
					...
					'option'		=> (isset($values['option']) && is_array($values['option'])) ? $values['option'] : '',
