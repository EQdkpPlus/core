...
class portal extends gen_class {
	public static $shortcuts = array('jquery', 'db', 'in', 'tpl', 'user', 'config', 'game', 'pdh', 'pm', 'env', 'acl');
	...
	public function module_output() {
		...
		if(is_array($enabled_modules)){
			foreach($enabled_modules as $module_id => $posi) {
				if($posi == 'left1' || $posi == 'left2' || $is_start_page || (in_array($posi, $selected_portal_pos) && !defined('IN_ADMIN'))) {
					$path = $this->pdh->get('portal', 'path', array($module_id));
					$plugin = $this->pdh->get('portal', 'plugin', array($module_id));
					$this->load_lang($path, $plugin);
					$this->output[$posi] .= $this->get_module_content($path, $plugin, $module_id, $posi);
				}
			}
		}
