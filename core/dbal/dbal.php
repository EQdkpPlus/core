...
class dbal {
	...
	public static function available_dbals() {
		$arrDbals = array('mysql' => 'MySQL', 'mysqli'	=> 'MySQLi');
...
abstract class dbal_common extends gen_class {
	...
	public function __construct($options=array()) {
		...
		if(isset($options['open'])) {
			...
			$this->query("SET SESSION sql_mode = 'STRICT_TRANS_TABLES'");
		}
	}
	...
	private function build_keysvalues($data) {
		$values = array();
		$do_keys = empty($this->keys);
		foreach($data as $k => $v) {
			if($do_keys) $this->keys[] = $k;
			if(is_null($v) || $v === 'NULL')
				$values[] = 'NULL';
			elseif(is_int($v))
				$values[] = $v;
	...
	public function error($sql = '', $return=false) {
		...
		if(!$this->lite_mode && registry::fetch('user')->check_auth('a_', false)) {
			registry::register('core')->message("<b>SQL Error</b> <br /> <a href=".registry::register('environment')->phpself."?debug=3>Click for more infos</a>", 'Error', 'red');
			$sys_message = true ;
		}
