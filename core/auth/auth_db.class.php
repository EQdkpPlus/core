	...
	*/
	public function login($strUsername, $strPassword, $boolSetAutoLogin = false, $boolUseHash = false){
		...
		if (!$arrStatus){
			...
			//Auth Login
			if (!$arrStatus){
				$result	= $this->db->query("SELECT user_id, username, user_password, user_email, user_active, api_key, failed_login_attempts
								FROM __users 
				...
				if($row){		
					...
					if (((int)$row['user_active'])){
						...
						if ((int)$row['user_active']){
							...
							$this->error = 'user_inactive';
							if ($row['failed_logins'] >= (int)$this->config->get('failed_logins_inactivity') ){
								$this->error = 'user_inactive_failed_logins';
							}
						}
			...
			//If Bridge is active, check if user is Superadmin, if yes, login
		...
		if (!$arrStatus){
			...
			$this->destroy();
			$this->create($arrStatus['user_id'], $arrStatus['password_hash'], ((isset($arrStatus['autologin'])) ? $arrStatus['autologin'] : $boolSetAutoLogin));
			return true;	
		}
		return false;
	}
		...
		if (isset($intCookieUserID) && intval($intCookieUserID) > 0){
			...
			if ($arrUserResult){
				if ($strCookieAutologinKey != "" && $strCookieAutologinKey===$arrUserResult['user_login_key'] && (int)$arrUserResult['user_active']){
					//Successfull Autologin, create new login_key
					$strAutologinKey = hash('sha512', $this->generate_salt());
					$this->updateAutologinKey($intCookieUserID, $strAutologinKey);
