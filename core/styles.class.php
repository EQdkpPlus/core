...
if (!class_exists("styles")){
	class styles extends gen_class {
		public static $shortcuts = array('in', 'user', 'config', 'pdh', 'pfh', 'tpl', 'game', 'core');
			...
			if (file_exists($installer_file)){
				...
				if ($xml){
					...
					if (!in_array($data['style_name'], $styles)){
						...
						$this->core->message( sprintf($this->user->lang('install_style_suc'), $style_data[$stylename]['name']), $this->user->lang('success'), 'green');
					} else {
