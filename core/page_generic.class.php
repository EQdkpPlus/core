...
if(!class_exists('page_generic')){
	class page_generic extends gen_class {
		...
		private $handler = array(
			'del'	=> array('process' => 'delete', 'session_key' => true),
			'add'	=> array('process' => 'add'),
			'upd'	=> array('process' => 'update'),
		...
		public function __construct($pre_check, $handler=false, $pdh_call=array(), $params=null, $cb_name='', $url_id='') {
			...
			$this->tpl->assign_vars(array(
				'SESSION_KEY'	=> $this->user->data['session_key'],
				'ACTION'	=> $this->env->phpself.$this->SID.$this->simple_head_url.'&amp;session_key='.$this->SKEY.$this->url_id_ext,
				'SID'		=> $this->SID)
			);
		}
		...
		protected function confirm_delete($message='', $url='', $single=false, $options=array()) {
			...
			$url .= (strpos($url, 'session_key') !== false) ? '' : 'session_key='.$this->SKEY.'&';
			if ($single){
			...
			$options['url'] = $url.$handler.'=true'.str_replace('&amp;', '&', $this->url_id_ext);
		...
		protected function process() {
			foreach($this->handler as $key => $process) {
				if($this->in->exists($key) AND !is_array(current($process))) {
					...
					if(isset($process['session_key']) AND !$this->check_session_key()) break;
					...
					foreach($process as $subprocess) {
						if($subprocess['value'] == $this->in->get($key)) {
							...
							if(isset($subprocess['session_key']) AND !$this->check_session_key()) break;
		...
		private function check_session_key() {
			if($this->in->exists('session_key') AND $this->in->get('session_key') == $this->SKEY) return true;
