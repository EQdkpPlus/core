...
#                    );
    ...
    // Drops
    //
  	$sql = 'SELECT i.item_id, i.item_name, i.item_value, i.item_buyer, c.class_name, c.class_id
          	FROM (__classes c RIGHT JOIN (__members m RIGHT JOIN __items i ON m.member_name = i.item_buyer) 
          	ON c.class_id = m.member_class_id) WHERE (i.raid_id = \''.$raid['raid_id'].'\')';
    ...
    $sql = "SELECT m.member_class_id, m.member_name, c.class_name AS member_class
    ...
    while ( $row = $db->fetch_record($result) )
    {
        ...
        if ( $member_name != '' )
        {
            ...
            $eq_classesID[ $row['member_class'] ] = $row['member_class_id'] ;
    ...
    foreach ( $eq_classes as $class => $members )
    {
		...
		$chartarray['percent'][] = $percentage ;
		$chartarray['color'][] = get_class_color_code($class);
		$chartarray['labels'][] = $class."( ".$class_count[$class]." - ".$percentage."%)";
		
        $tpl->assign_block_vars('class_row', array(
            'CLASS'     => get_classNameImgListmembers($class,$eq_classesID[$class]),
    ...
    unset($eq_classes);
	$event_icon = getEventIcon($raid['raid_name'],'org');

  $comm_settings = array('attach_id'=>$raid['raid_id'], 'page'=>'raids');
    ...
    $tpl->assign_vars(array(
        ...
        'RAID_VALUE'          => runden($raid['raid_value']),
        ...
        'CLASS_PERCENT_CHART' => $html->createpieGraph($chartarray['percent'],$chartarray['color'],$chartarray['labels'])
