		...
		if ( $raid_id = $this->in->get('r', 0) ){
			...
			foreach($items as $item_id){
				...
				if(isset($loot_dist[$class_id])){
					...
					$loot_dist[$class_id] = array('value' => 1, 'name' => $class_name, 'color' => $this->game->get_class_color($class_id) );
				}
			...
			foreach ( $class_dist as $class_id => $details ){
				...
				$chartarray[]	= array('value' => $percentage, 'name' => $class." (".$class_dist[$class_id]['count']." - ".$percentage."%)", 'color' => $this->game->get_class_color($class_id) );
			...
			$this->tpl->assign_vars(array(
				...
				'LOOT_PERCENT_CHART'	=> (count($loot_dist) > 0) ? $this->jquery->PieChart('loot_dist', $loot_dist, '', $chartoptions, 2) : '',
			));
