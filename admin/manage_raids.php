...
class ManageRaids extends page_generic {
	...
	public function __construct(){
		$handler = array(
			'save' => array('process' => 'save', 'check' => 'a_raid_add'),
			'itemadj_del' => array('process' => 'update', 'check' => 'a_raid_del'),
	...
	$('#r_add_event').click(function() {
		...
		$this->jquery->dialog('r_add_mem_durl', $this->user->lang('add_member'), array('url' => $this->root_path.'addcharacter.php?adminmode=1', 'width' =>'640', 'height' => '520', 'onclosejs' => 'document.getElementById("save_button").click();'));
		$this->jquery->dialog('r_add_event_durl', $this->user->lang('add_event'), array('url' => 'manage_events.php?upd=true&simple_head=true', 'width' =>'700', 'height' =>'550', 'onclosejs' => 'document.getElementById("save_button").click();'));
		...
		if(is_array($this->in->getArray('adjs', 'int'))) {
			foreach($this->in->getArray('adjs', 'int') as $key => $adj) {
				if(!isset($adj['delete']) OR !$this->in->exists('itemadj_del')) {
					...
					if($adj['delete'] AND $adj['group_key'] != 'new' AND $this->in->exists('itemadj_del')) {
		...
		if(is_array($this->in->getArray('items', 'int'))) {
			foreach($this->in->getArray('items', 'int') as $key => $item) {
				if(!isset($item['delete']) OR !$this->in->exists('itemadj_del')) {
					...
					if($item['delete'] AND $item['group_key'] != 'new' AND $this->in->exists('itemadj_del')) {
