...
class Info_Pages extends page_generic {
	...
	public function __construct(){
		...
		$handler = array(
			...
			'mode' => array(
				array('process' => 'reset_votings', 'value' => 'reset_votings'),
				array('process' => 'delete_comments', 'value' => 'delete_comments')),
			...
			foreach ($pagelist as $id) {
				...
				if (is_array($this->pdh->get('pages', 'visibility', array($id)))){
					foreach ($this->pdh->get('pages', 'visibility', array($id)) as $group){
						if ($this->pdh->get('user_groups', 'name', array($group)) AND !in_array($this->pdh->get('user_groups', 'name', array($group)), $visibility)) {
