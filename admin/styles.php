...
class Manage_Styles extends EQdkp_Admin
{
    ...
    function manage_styles()
    {
        ...
        $sql = 'SELECT class_name, class_id FROM '.CLASS_TABLE.' GROUP BY class_name ORDER BY class_name';
          ...
          if(strtolower($row['class_name']) != 'unknown'){
            $tmpclass = str_replace(" ", "", renameClasstoenglish($row['class_name']));
            $this->allclasses[$row['class_id']] = array(
                                             'name'     => $row['class_name'],
                                             'en_name'  => $tmpclass,
                                            );
        	}
        ...
        {
            // Class Color Selection
            $sql = 'SELECT * FROM '.CLASSCOLOR_TABLE." WHERE template='" . $this->url_id . "'";
          	$result = $db->query($sql);
          	while ($row = $db->fetch_record($result)){
                $this->classcolor[$row['name']] = $row['color'];
            }		
            $db->free_result($result);
    ...
    function process_add()
    {
        ...
        foreach($this->allclasses as $clrow){
          $this->ClassColorManagement($style_id, $clrow['en_name'], $_POST['classc_'.strtolower($clrow['en_name'])]);
        }
    ...
    function process_update()
    {
        ...
        foreach($this->allclasses as $clrow){
          $this->ClassColorManagement($this->url_id, $clrow['en_name'], $_POST['classc_'.strtolower($clrow['en_name'])]);
        }
		...
		$pdc->del('user->styles.'.$this->url_id);
		$pdc->del('classes_color_array.'.$this->url_id);
    ...
    function process_confirm()
    {
        ...
        if ($this->url_id == $eqdkp->config['default_style']){
			message_die($user->lang['admin_delete_style_error_defaultstyle']);
		} else {
			$db->query("DELETE FROM __styles WHERE style_id='" . $this->url_id . "'");
			$db->query("DELETE FROM __style_config WHERE style_id='" . $this->url_id . "'");
			$db->query("UPDATE __users SET user_style = '".$db->escape($eqdkp->config['default_style'])."' WHERE user_style = '".$db->escape($this->url_id)."'");
			$pdc->del('user->styles.'.$this->url_id);
        	$pdc->del('classes_color_array.'.$this->url_id);
    ...
    function display_form()
    {
        ...
        // The super "select Color" thing for Classes...
        foreach($this->allclasses as $clrow){
  			$tpl->assign_block_vars('classes', array(
  									'NAME'     => $clrow['name'],
  									'CPICKER'  => $jqueryp->colorpicker('classc_'.strtolower($clrow['en_name']), $this->classcolor['classc_'.strtolower($clrow['en_name'])]),
									));
        }
    ...
    function ClassColorDelete($template){
      global $db;
      $db->query('DELETE FROM '.CLASSCOLOR_TABLE." WHERE template='".$template."'");
    }
    
    function ClassColorManagement($template, $name, $color){
      global $db;
      $query = $db->build_query('INSERT', array(
            'template'    => $template,
            'name'        => 'classc_'.strtolower($name),
            'class'       => $name,
            'color'       => $color
            ));
        $db->query('INSERT INTO ' . CLASSCOLOR_TABLE . $query);
    }
}
