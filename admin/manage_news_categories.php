...
class Manage_News_Categories extends page_generic {
	...
	public function __construct(){
		...
		$handler = array(
			'mode'		=> array('process' => 'delete_icon', 'value' => 'delicon'),
			'deleteid'	=> array('process' => 'delete', 'session_key' => true),
		);
	...
	public function display() {
		...
		foreach($ncategories as $id){
			$this->tpl->assign_block_vars('news_categories', array(
				...
				'ICON'			=> $this->html->widget(array('type' => 'imageuploader', 'name' => 'news_categories['.$id.'][icon]', 'imgpath'	=> $this->pfh->FolderPath('newscat_icons','eqdkp'), 'value' => $this->pdh->get('news_categories', 'icon', array($id)), 'options' => array('prevheight'=>'48', 'resize'=>true, 'deletelink'=>'manage_news_categories.php'.$this->SID.'&mode=delicon&id='.$id))),
			));
		}

		$this->confirm_delete($this->user->lang('confirm_delete_newscat'), '', true, array('custom_js' => "window.location = 'manage_news_categories.php".$this->SID."&session_key=".$this->user->data['session_key']."&deleteid='+selectedID"));
