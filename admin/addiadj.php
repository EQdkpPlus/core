...
class Add_IndivAdj extends EQdkp_Admin
{
    ...
    function error_check()
    {
		...
		// adj date
		$adj_date_day 						= substr($_POST['adjustment_date'],0,2);
		$adj_date_month 					= substr($_POST['adjustment_date'],3,2);
		$adj_date_year 						= substr($_POST['adjustment_date'],6);
		...
		$this->fv->is_valid_date('adjustment_date', $user->lang['fv_date']);
...
if ( (@empty($_POST['raid_name'])) || (@sizeof($_POST['raid_name']) == 0) )
		...
		// adj date
		$this->time = mktime(0, 0, 0, $adj_date_month, $adj_date_day, $adj_date_year);
    ...
    function remove_old_adjustment()
    {
        ...
        $sql = "SELECT a2.*
                FROM (__adjustments a1
                LEFT JOIN __adjustments a2
    ...
    function display_form()
    {
        ...
        while ( $row = $db->fetch_record($result) )
        {
            if ( $this->url_id )
            {
                if (is_array($this->adjustment['member_names']))
                {
            ...
            {
				if (is_array( $_POST['member_names']))
				{
 ...
 //
		...
		'U_ADD_EVENT'      => 'addevent.php'.$SID,
			...
			// No checks anymore because of the date picker
			...
			// Date Picker
            'DATEPICK'        => $jquery->Calendar('adjustment_date', date('d.m.Y', $this->time)),
			...
			'FV_DATE'		=> $this->fv->generate_error('adjustment_date'),
