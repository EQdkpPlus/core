...
<script type="text/javascript" src="'.$eqdkp_root_path.'itemstats/overlib/overlib.js"><!-- overLIB (c) Erik Bosrup --></script>';
  ...
  if($step=='trade')
	...
	if($_GET['actual'] < $_GET['count'])
	{
		...
		$sql2 = "DELETE FROM " . item_cache_table ." where item_name like '".$items[$_GET['actual']]."'";
		$result2 = $db->query($sql2) ;
		$db->free_result($result2);
		
		$output .= 'Item ' . $_GET['actual'] . ' / ' . $ii ."<br>";
		$output .= itemstats_decorate_name($items[$_GET['actual']]) ;
		$output .= itemstats_get_html($items[$_GET['actual']]) ;
		...
		if($_GET['actual'] == $_GET['count'])
		{
      $output = '<table><tr><td width="48px"><img src="../images/ok.png" alt="update" \></td><td>'. $user->lang['updi_update_ready'].'</td></tr></table>';
			echo "<script>
