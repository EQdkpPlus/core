...
class EQDKPBackup extends page_generic{
	...
	public function __construct(){
		...
		$handler = array(
			'backup' => array('process' => 'do_backup'),
			'restore' => array('process' => 'restore_backup'),
			'backup_delete' => array('process' => 'delete_backup'),
			'backup_download' => array('process' => 'download_backup'),
		);
		...
		if (isset($backups) && is_array($backups)){
			foreach ($backups as $key=>$elem){
				...
				$this->tpl->assign_block_vars('backup_list', array(
					'FILE'		=> $key,
					'NAME'		=> $this->time->date("Y-m-d H:i", $elem).$addition,
				));
