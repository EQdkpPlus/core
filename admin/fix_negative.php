...
$sql = "SELECT m.member_name, m.member_raidcount, count(ra.raid_id) AS cnt, sum(r.raid_value), m.member_earned 
	FROM __raids r, __raid_attendees ra, __members m 
...
while ( $row = $db->fetch_record($members_result) )
{
	...
	$sql = "UPDATE __members SET member_raidcount = ".$row['cnt']." WHERE member_name = '".$row['member_name']."'";
	print $sql . "<BR><BR>";
	$db->query($sql);
}
...
$sql = "UPDATE __members m, tempqw_fixearned f
