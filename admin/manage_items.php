...
class ManageItems extends page_generic {
	...
	public function update($message=false) {
		...
		foreach($this->pdh->get('item', 'id_list') as $intItemID){
			$arrAutocomplete[] = array(
				'label'		=> 	$this->pdh->get('item', 'name', array($intItemID)).', '.$this->pdh->geth('item', 'date', array($intItemID)).((!$this->config->get('disable_points')) ? ', '.runden($this->pdh->aget('item', 'value', array($intItemID))).' '.$this->config->get('dkp_name') : '').' (#'.$intItemID.')',
