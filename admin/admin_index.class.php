...
class admin_index extends gen_class {
	...
	public function ajax_twitterfeed(){
		...
		//expired or not available, update from Server
			...
			if ($fetchedData) {
				$objXML = simplexml_load_string($fetchedData);
		
				$arrTwitter = $this->xmltools->simplexml2array($objXML);

				foreach ($arrTwitter['channel']['item'] as $key => $item){
					if (strpos($item['title'], 'EQdkpPlus: @') === 0){
						unset($arrTwitter['channel']['item'][$key]);
						continue;
					}
					if (strlen($item['title']) > 40){
						$truncated = substr($item['title'],0,strpos($item['title'],' ',40));
						if ($truncated == '') {
							$truncated = $item['title'];
						} else {
							$truncated = $truncated.'...';
						}
						$arrTwitter['channel']['item'][$key]['title'] = $truncated;
					}
					$arrTwitter['channel']['item'][$key]['description'] = substr($arrTwitter['channel']['item'][$key]['description'], 11);
					$arrTwitter['channel']['item:'.$key] = $arrTwitter['channel']['item'][$key];
				}
				unset($arrTwitter['channel']['item']);
				$out = null;
				$this->xmltools->array2simplexml($arrTwitter, 'rss', &$out);
				
				$dom = dom_import_simplexml($out)->ownerDocument;
				$dom->encoding='utf-8';
				...
				$this->pdc->put('core.twitterfeed_data', $this->xmltools->prepareSave($fetchedData), $this->rsscachetime*3600);
				$rsstwitter_out = $fetchedData;
			} else {
