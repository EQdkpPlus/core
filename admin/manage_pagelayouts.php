...
class ManagePageLayouts extends page_generic {
	...
	public function __construct(){
		...
		$handler = array(
			'save' => array('process' => 'save_layout'),
			'add_preset' => array('process' => 'add_preset'),
			'add_layout' => array('process' => 'add_layout'),
			'set_current_layout' => array('process' => 'set_current_layout'),
			'del_pre' => array('process' => 'delete_preset'),
			'del' => array('process' => 'delete')
		);
				...
				if(substr($page_object,0,4) == 'hptt'){
					...
					foreach($presets as $preset => $td_add){
						$layout_def['pages'][$page][$page_object]['table_presets'][] = array(
							...
							'th_add' => html_entity_decode($this->in->get($prefix.':td_add:'.$preset, '')),
							'td_add' => html_entity_decode($td_add)
						);
