...
class Manage_Users extends page_generic {
	...
	public function __construct(){
		...
		$handler = array(
			'mode' => array(
				array('process' => 'activate', 'value' => 'activate'),
				array('process' => 'deactivate', 'value' => 'deactivate'),
				array('process' => 'overtake_permissions', 'value' => 'ovperms'),
				),
			'u' => array('process' => 'edit'),
			'submit' => array('process' => 'submit', 'session_key' => true),
			'send_new_pw' => array('process' => 'send_new_pw'),
		);
	...
	public function send_new_pw(){
		...
		$bodyvars = array(
			...
			'U_ACTIVATE'	=> $this->env->link.'login.php?mode=newpassword&amp;key=' . $pwkey['user_key'],
		);
	...
	public function submit() {
		...
		$this->pm->do_hooks('/admin/manage_users.php?action=update');
	...
	public function display() {
		...
		foreach($user_ids as $user_id) {
			...
			if($this->pdh->get('user', 'active', array($user_id))) {
				...
				$activate_icon = '<a href="manage_users.php'.$this->SID.'&amp;mode=deactivate&amp;u='.$user_id.'" title="'.$this->user->lang('deactivate').'"><img src="'.$this->root_path.'images/glyphs/disable.png" alt="" /></a>';
			} else {
				$user_active = "<img src='../images/glyphs/status_red.gif' alt='' />";
				$activate_icon = '<a href="manage_users.php'.$this->SID.'&amp;mode=activate&amp;u='.$user_id.'" title="'.$this->user->lang('activate').'"><img src="'.$this->root_path.'images/glyphs/enable.png" alt="" /></a>';
			}
			...
			$this->tpl->assign_block_vars('users_row', array(
				...
				'U_OVERTAKE_PERMS'=> 'manage_users.php'.$this->SID.'&amp;mode=ovperms&amp;' . 'u' . '='.$user_id,
				'U_DELETE'			=> 'manage_users.php'.$this->SID.'&amp;del=single&amp;user_id='.$user_id.'&amp;session_key='.$this->user->data['session_key'],
				'USER_ID'			=> $user_id,
		...
		$this->jquery->Dialog('EditChar', $this->user->lang('uc_edit_char'), array('withid'=>'editid', 'url'=>"../addcharacter.php?adminmode=1&editid='+editid+'", 'width'=>'640', 'height'=>'520', 'onclosejs'=>$onclose_url));
	...
	public function edit(){
		...
		$this->confirm_delete($this->user->lang('confirm_delete_users').'<br />'.((isset($this->user_data['username'])) ? sanitize($this->user_data['username']) : '').'<br /><label><input type="checkbox" name="delete_associated_members" value="1"> '. $this->user->lang('delete_associated members').'</label>', '', true, array('height'	=> 300, 'custom_js' => "if($('input[name=delete_associated_members]').is(':checked')){ window.location='manage_users.php".$this->SID."&del=true&session_key=".$this->SKEY."&user_id='+selectedID+'&del_assocmem=1';}else{ window.location='manage_users.php".$this->SID."&del=true&session_key=".$this->SKEY."&user_id='+selectedID;}"));
		...
		$this->tpl->assign_vars(array(
			// Form vars
			'F_SETTINGS'				=> 'manage_users.php' . $this->SID.'&amp;session_key='.$this->user->data['session_key'],
		...
		$this->pm->do_hooks('/admin/manage_users.php?action=settings');
		$this->core->set_vars(array(
