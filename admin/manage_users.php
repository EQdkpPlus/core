   ...
   function switch_upd_ins($auth_id, $user_id)
    ...
    function display_list()
    {
       ...
       while ( $row = $db->fetch_record($result) )
       {
          ...
          while( $rowtemp = $db->fetch_record($resulttemp) )
          {
          	$member_rank_id[ $row['member_id'] ] = $rowtemp['rank_name'];
          }
         ...
         while( $rowtemp = $db->fetch_record($resulttemp) )
         {
         	$member_class_id[ $row['member_id'] ] = $rowtemp['class_name'];
         }
       }
        ...
        while ( $row = $db->fetch_record($result) )
        {
	      ...
	      {
	      	$fv_member_id .= get_classNameImgViewmembers($member_names[$row2['member_id']])." - ".$member_rank_id[$row2['member_id']]."<br>";
	      }
