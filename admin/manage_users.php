...
class Manage_Users extends EQdkp_Admin
{
    ...
    function error_check()
    {
        ...
        if ( isset($_POST['submit']) )
        {
            ...
            if ( (!empty($_POST['new_user_password1'])) || (!empty($_POST['new_user_password2'])) )
            {
    			...
    			if(preg_match('+([^\w|#\$%!])+', $_POST['new_user_password1'])) {
    				$this->fv->errors['new_user_password1'] = $user->lang['error_invalid_password'];
    			} else {
    ...
    function process_submit()
    {
        ...
        $sql = 'UPDATE ' . USERS_TABLE . ' SET ' . $query . " WHERE user_id = '" . $this->user_data['user_id'] . "'";
        if ( !($result = $db->query($sql)) )
        {
        ...
        if ( ( $eqdkp->config['account_activation'] == USER_ACTIVATION_ADMIN ) && ( $this->user_data['user_active'] < $_POST['user_active'] ) )
        {
            ...
            $email->set_template('register_activation_none', $_POST['user_lang']);
   ...
   function switch_upd_ins($auth_id, $user_id)
    ...
    function display_list()
    {
       ...
       $sql = 'SELECT m.member_id, m.member_name, c.class_name, c.class_id, r.rank_name, mu.user_id
				FROM (
					(' . MEMBERS_TABLE . ' m INNER JOIN ' . CLASS_TABLE . ' c ON m.member_class_id = c.class_id)
				INNER JOIN ' . MEMBER_RANKS_TABLE . ' r ON m.member_rank_id = r.rank_id)
				LEFT JOIN ' . MEMBER_USER_TABLE . ' mu ON m.member_id = mu.member_id
			';
       ...
       while ( $row = $db->fetch_record($result) )
       {
		 ...
		 $a_members[$membercount]['class_id'] = $row['class_id'] ;
       ...
       $db->free_result($result);
        ...
        while ( $row = $db->fetch_record($result) )
        {
            ...
            foreach ($a_members as $value)
            {
            	...
            	{
            		$fv_member_id .= get_classNameImgViewmembers($value['member_name'],
	      														$value['class_name'],
	      														$value['class_id'] )." - ".$value['rank_name']."<br>";
            	}
            }
        ...
        $user_permissions = array(
            ...
            $user->lang['sms_perm'] => array(
								array('CBNAME' => 'a_sms_send',  'CBCHECKED' => A_SMS_SEND,  'TEXT' => '<b>' . $user->lang['sms_perm2'] . '</b>'),
						),
            // Userlist
            $user->lang['user_list'] => array(
								array('CBNAME' => 'u_userlist', 	 'CBCHECKED' => U_USERLIST, 'TEXT' => $user->lang['user_list']),
						),
            // Members
            ...
            // Members
            $user->lang['users'] => array(
                array('CBNAME' => 'a_users_man',   'CBCHECKED' => A_USERS_MAN,   'TEXT' => '<b>' . $user->lang['manage'] . '</b>'),
                array('CBNAME' => 'u_userlist',    'CBCHECKED' => 34,    		 'TEXT' => $user->lang['list'])
            ),
            // Manage
    ...
    {
        global $db, $eqdkp, $user, $tpl, $pm, $eqdkp_root_path, $jqueryp;
        global $SID;
        ...
        $tpl->assign_vars(array(
            ...
            'BIRTHDAY' => $jqueryp->Calendar('birthday', stripslashes($this->user_data['birthday']), '', array('change_fields'=>true, 'year_range'=>'-80:0')),
        ...
        foreach ($gender_array as $key => $value)
        {
	        $tpl->assign_block_vars('gender_row', array(
                'VALUE' => $key,
                'SELECTED' => ($this->user_data['gender'] == $key ) ? ' selected="selected"' : '',
        ...
        if (file_exists($cfile))
        {
	        ...
	        foreach ($country_array as $key => $value)
	        {
		        $tpl->assign_block_vars('country_row', array(
	                'VALUE' => $key,
	                'SELECTED' => ( $this->user_data['country'] == $key ) ? ' selected="selected"' : '',
	                'OPTION' => $value )
	            );
	        }
        }
