	...
	public function display() {
		...
		foreach($user_ids as $user_id) {
			...
			if($this->pdh->get('user', 'active', array($user_id))) {
				...
				$activate_icon = '<a href="manage_users.php'.$this->SID.'&amp;mode=activate&amp;u='.$user_id.'&amp;link_hash='.$this->CSRFGetToken('mode').'" title="'.$this->user->lang('activate').'"><i class="icon-minus-sign icon-rotate-90 icon-large icon-color-green"></i></a>';
			}
