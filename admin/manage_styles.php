...
class Manage_Styles extends page_generic{
	...
	public function __construct(){
		...
		$handler = array(
			...
			'template_edit_button' => array('process' => 'edit_template'),
			'template_reset_button' => array('process' => 'reset_template'),
	...
	public function edit_template(){
		if ($this->in->get('template_dd') != "" && $this->in->get('template_edit') != ""){
			...
			$storage_folder  = $this->pfh->FolderPath('templates/'.$this->style['template_path'].$admin_folder, 'eqdkp');
			$this->pfh->putContent($storage_folder.$filename, html_entity_decode($this->in->get('template_edit', '', 'raw'), ENT_QUOTES, 'UTF-8'));
	...
	public function edit(){
		...
		foreach($this->game->get('classes') as $class_id => $class_name){
			$this->tpl->assign_block_vars('classes', array(
				'NAME'		=> $class_name,
				'CPICKER'	=> $this->jquery->colorpicker('classc_'.$class_id, str_replace('#', '', $this->game->get_class_color($class_id))),
			));
		}
