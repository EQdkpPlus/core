...
class Manage_Extensions extends page_generic {
	...
	public function __construct(){
		...
		$handler = array(
			...
			'hide_update_warning'	=> array('process' => 'hide_update_warning'),
			'upload' => array('process' => 'process_upload'),
		);
	...
	public function process_upload(){
		...
		} elseif (!in_array($filetype, $this->repo->whitelist)){
			$this->core->message($this->user->lang('plugin_upload_error2'), $this->user->lang('error'), 'red');
		} else {
				...
				if ($extension_name){
					...
					if ($target){
						...
						if (!$blnResult){
							...
							$this->pm->search();
							redirect('admin/manage_extensions.php'.$this->SID.'&cat='.$cat.'&mode=install&code='.$extension_name);
						}
					} else {
	...
	public function mode() {
		switch((int)$this->in->get('cat', 0)){
			...
			case 3:		$path = $this->in->get('selected_id');
						if(!$path) return;
						$id = $this->pdh->get('portal', 'id_list', array(array('path' => $path)));
						$plugin = $this->pdh->get('portal', 'plugin', array($id[0]));
						$name = $this->pdh->get('portal', 'name', array($id[0]));
	...
	public function display(){
		...
		foreach ( $plugins_array as $plugin_code ) {
			...
			if($this->pm->check($plugin_code, PLUGIN_BROKEN)) {
				$this->tpl->assign_block_vars('plugins_row_broken', array(
					...
					'DEL_LINK'		=> 'manage_extensions.php'.$this->SID.'&amp;cat=1&amp;mode=delete&amp;code='.$plugin_code
				));
				continue;
			}
			//dependencies
			...
			if($this->pm->check($plugin_code, PLUGIN_DISABLED)) {
				$link = ( $dep_all ) ? '<a href="manage_extensions.php' . $this->SID . '&amp;cat=1&amp;mode=enable&amp;code=' . $plugin_code. '" title="'.$this->user->lang('plug_enable_info').'">' . $this->user->lang('enable') . '</a>' : $this->user->lang('plug_dep_broken_deps');
				$row = 'yellow';
				...
				if (isset($urgendUpdates[$plugin_code])){
					...
					$row = 'green';
					$link = ( $dep_all ) ? '<a href="manage_extensions.php' . $this->SID . '&amp;cat=1&amp;mode=uninstall&amp;code=' . $plugin_code. '">' . $this->user->lang('uninstall') . '</a>' : $this->user->lang('plug_dep_broken_deps');
				}
				...
				$row = 'grey';
				$link = ( $dep_all ) ? '<a href="manage_extensions.php' . $this->SID . '&amp;cat=1&amp;mode=install&amp;code=' . $plugin_code. '">' . $this->user->lang('install') . '</a>' : $this->user->lang('plug_dep_broken_deps');
			}
		...
		if (isset($arrExtensionList[1]) && is_array($arrExtensionList[1])){
			foreach ($arrExtensionList[1] as $id => $extension){
				...
				$dep['plusv']	= (compareVersion($this->config->get('plus_version'), $extension['dep_coreversion'], '>=') >= 0);
				$dep['games']	= 'skip';
				$dep['phpf']	= 'skip';
				...
				$link = ($dep['plusv']) ? $dl_link : $this->user->lang('plug_dep_'.$key.'_short');
				$this->tpl->assign_block_vars('plugins_row_'.$row, array(
		...
		if ($arrUpdateCount[1]['red']){
			$badge = '<span class="update_available">'.count($arrUpdateCount[1]['red']).'</span>';
		} elseif ($arrUpdateCount[1]['yellow']){
			$badge = '<span class="update_available_yellow">'.count($arrUpdateCount[1]['yellow']).'</span>';
		}
		...
		foreach($arrUninstalledStyles as $key => $install_xml){
			...
			if(isset($allUpdates[$plugin_code])){
				...
				$row = 'grey';
				$link = '<a href="manage_extensions.php' . $this->SID . '&amp;cat=2&amp;mode=install&amp;code=' . $key. '">' . $this->user->lang('install') . '</a>';
			}

			$this->tpl->assign_block_vars('styles_row_'.$row, array(
				'NAME'			=> ($install_xml->name) ? $install_xml->name : stripslashes($key),
		...
		foreach($arrTemplates as $row){
			...
			if (isset($urgendUpdates[$plugin_code])){
				if (isset($arrLocalStyleUpdates[$plugin_code])){
					$rowname = 'red_local';
					$link = '<a href="manage_extensions.php' . $this->SID . '&amp;cat=2&amp;mode=update&amp;code=' . $row['style_id']. '">'.$this->user->lang('uc_bttn_update').'</a>';
				} else {
					$rowname = 'red';
					$link = '<a href="javascript:repo_update(2, \''.$plugin_code.'\');">'.$this->user->lang('uc_bttn_update').'</a>';
					if (!$blnRequirements) $link = '<a href="'.EQDKP_DOWNLOADS_URL.'" target="_blank">'.$this->user->lang('uc_bttn_update').'</a>';
			...
			$this->tpl->assign_block_vars('styles_row_'.$rowname, array(
				...
				'U_DOWNLOAD_STYLE'	=> 'manage_extensions.php' . $this->SID . '&amp;cat=2&amp;mode=export&amp;code=' . $row['style_id'],
				...
				'U_ENABLE'			=> ($row['enabled'] == '1') ? 'manage_extensions.php' . $this->SID . '&amp;cat=2&amp;mode=disable&amp;code=' . $row['style_id'] : 'manage_extensions.php' . $this->SID . '&amp;mode=enable&amp;cat=2&amp;code=' . $row['style_id'],
		...
		$this->jquery->dialog('style_reset_warning', $this->user->lang('reset_style'), array('message' => $this->user->lang('style_confirm_reset'), 'height' => 200, 'url' => $this->root_path.'admin/manage_extensions.php' . $this->SID . "&cat=2&mode=reset&code='+ styleid+'", 'withid' => 'styleid'), 'confirm');
		$this->jquery->dialog('style_delete_warning', $this->user->lang('delete_style'), array('message' => $this->user->lang('confirm_delete_style'), 'height' => 200, 'url'=> $this->root_path.'admin/manage_extensions.php' . $this->SID . "&cat=2&mode=uninstall&code='+ styleid+'", 'withid' => 'styleid'), 'confirm');

		$badge = '';
		if ($arrUpdateCount[2]['red']){
			$badge = '<span class="update_available">'.count($arrUpdateCount[2]['red']).'</span>';
		} elseif ($arrUpdateCount[2]['yellow']){
			$badge = '<span class="update_available_yellow">'.count($arrUpdateCount[2]['yellow']).'</span>';
		}
		...
		if (is_array($arrModules)){
			...
			$this->confirm_delete($this->user->lang('portal_reinstall_warn'), 'manage_extensions.php'.$this->SID.'&session_key='.$this->user->data['session_key'].'&cat=3', true, array('function' => 'reinstall_portal', 'handler' => 'mode'));
		}
		...
		if ($arrUpdateCount[7]['red']){
			$badge = '<span class="update_available">'.count($arrUpdateCount[7]['red']).'</span>';
		} elseif ($arrUpdateCount[7]['yellow']){
			$badge = '<span class="update_available_yellow">'.count($arrUpdateCount[7]['yellow']).'</span>';
		}
