...
if($_GET['delete'] == 'yes')
{
	...
	// Itemstable
	$sql = 'SELECT item_name FROM __items GROUP BY item_name';
	$items_result = $db->query($sql) ;
	while ( $item = $db->fetch_record($items_result) )
	{
		$items[] = $item[item_name];
	...
	if ($pm->check(PLUGIN_INSTALLED, 'ts'))
	{
		...
		if(is_array($items))
		{
			...
			foreach ($items as $value)
			{
				...
				$sql = "DELETE FROM " . item_cache_table . " WHERE item_name = '".mysql_real_escape_string($value)."' LIMIT 1";
				$db->query($sql);
...
else
{
	...
	$items_count 					= $db->query_first('SELECT distinct(count(item_name)) FROM __items') ;
...
$tpl->assign_vars(array(
    ...
    'JS_IS_UPDATEWINDOW'            => $jquery->Dialog_URL('UCCacheWindow', 'Update Itemstats', "updateitemstats_step.php?step='+step+'", '440', '330'),
    'JS_CLOSE_CODE'                 => $jquery->Dialog_close($window_close_array , false, false, true),
