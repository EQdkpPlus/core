...
if($_GET['show'] == 'yes')
{
	...
	// Itemcache
	$sql = 'SELECT item_name, item_id FROM ' . item_cache_table . ';';
	$items_result = $db->query($sql) ;
	while ( $item = $db->fetch_record($items_result) )
	{
		$items[$item['item_name'].$item['item_id']]['name'] = $item['item_name'];
		$items[$item['item_name'].$item['item_id']]['id'] = $item['item_id'];
	}
	$db->free_result($items_result);

	// Itemstable
	$sql = 'SELECT item_name, game_itemid FROM ' . ITEMS_TABLE . ';';
	$items_result = $db->query($sql) ;
	while ( $item = $db->fetch_record($items_result) )
	{
		$items[$item['item_name'].$item['game_itemid']]['name'] = $item['item_name'];
		$items[$item['item_name'].$item['game_itemid']]['id'] = $item['game_itemid'];
	}
	$db->free_result($items_result);
	...
	if(is_array($items))
	{
		...
		{
			$ihtml = itemstats_get_html(stripslashes($sitem['name']), $sitem['id'], true);
		   $tpl->assign_block_vars('items_row', array(
		   			'ROW_CLASS' => $eqdkp->switch_row_class(),
		   			'NAME' => itemstats_decorate_name(stripslashes($sitem['name']),$sitem['id']).'<br>'.$ithml
		   			)
			);
		}
	}
...
$tpl->assign_vars(array(
		...
		'UPDI_HELP_ALTERNATIVE'					=> $user->lang['updi_update_alternative'],

    'JS_IS_UPDATEWINDOW'            => $jqueryp->Dialog_URL('UCCacheWindow', 'Update Itemstats', "updateitemstats_step.php?step='+step+'", '440', '330'),
    'JS_CLOSE_CODE'                 => $jqueryp->Dialog_close($window_close_array , false, false, true),
...
function ping()
{
	global $eqdkp , $GLOBALS ;

	foreach ($GLOBALS["prio"] as $value)
	{$i++;
		$ret_val .=	'-> '.$i.'. '.ucfirst($value)  ;
	 }
