...
class Add_Raid extends EQdkp_Admin
{
	...
	setlocale(LC_ALL, 'de_DE');
		...
		$this->fv->is_valid_date('raid_date', $user->lang['fv_date']);
		...
		// raid date
		$this->time = mktime($_POST['h'], $_POST['mi'], $_POST['s'], $raid_date_month, $raid_date_day, $raid_date_year);
    ...
    function process_add()
    {
        ...
        foreach ( $_POST['raid_name'] as $raid_name )
        {
            ...
            //
            $success_message .= sprintf($user->lang['admin_add_raid_success'], $raid_date_month, $raid_date_day, $raid_date_year, $raid_name) . '<br />';
    ...
    function process_update()
    {
        ...
        // Success message
        //
        $success_message = sprintf($user->lang['admin_update_raid_success'], $raid_date_month, $raid_date_day, $raid_date_year, $_POST['raid_name']);
    ...
    */
    function get_old_data()
    {
        ...
        $sql = "SELECT r.member_name
                FROM __raid_attendees r, __members m
    ...
    */
	...
	if ( isset($_SESSION[$member_name]['level']) && isset($_SESSION[$member_name]['class']) ) {
		...
		$sql = "SELECT member_name, member_race_id, member_class_id, member_level FROM __members
    ...
    */
    function handle_members(&$members_array, $raid_value, $process)
    {
		...
		c.class_name AS member_class, m.member_raidcount
                FROM __members m, __classes c, __races r
        ...
        while ( $row = $db->fetch_record($result) )
        {
            ...
            if ( ($time_check) && ($level_check || $race_check || $class_check) )
            {
                ...
                // Process the update
                $sql  = 'UPDATE __members m, __classes c, __races r
    ...
    */
    ...
    {
        global $db, $eqdkp, $user, $tpl, $pm, $jquery;
        global $SID;
        ...
        $raid_value = ( empty($raid_value)) ? $_POST['raid_value'] : $raid_value;

        $raid_id = ( $this->raid['raid_id'] == 0 );
        if ( !empty($this->raid['raid_name']) )
        {
            // Get their correct earned/spent
            $CORRECT_RAID_VALUE = $db->query_first("SELECT sum(event_value) FROM __events WHERE event_name='" . addslashes($this->raid['raid_name']) . "'");
        }
        ...
        // Start Added By Teclador
        if ( !empty($this->raid['raid_name']) )
        {
            // Get correct raid dkp
            $correct_raid_earned = $db->query_first("SELECT sum(item_value) FROM __items WHERE raid_id=" . $this->url_id . "");
            $current_raid_att = $db ->query_first("SELECT count(*) FROM __raid_attendees WHERE raid_id=" . $this->url_id . "");

            // Raid DKP durch Teilnehmer teilen
            if($current_raid_att >0)
            {
            	$correct_raid_earned = round($correct_raid_earned / $current_raid_att,2);
            }
        }
        // End Added By Teclador
        ...
        $tpl->assign_vars(array(
			...
			// Date Picker
            'DATEPICK'        => $jquery->Calendar('raid_date', date('d.m.Y', $this->time)),
			'H'              => date('H', $this->time),
            'MI'             => date('i', $this->time),
            'S'              => date('s', $this->time),

            // Start Added By Teclador
            'CORRECT_RAID_VALUE' => ( !empty($correct_raid_earned) ) ? $correct_raid_earned : '0.00',
            // End Added By Teclador
			...
			'FV_DATE'		=> $this->fv->generate_error('raid_date'),
