    ...
    */
    ...
    function display_form()
    {
        ...
        if ( !empty($this->raid['raid_name']) )
        {
            ...
            $CORRECT_RAID_VALUE = $db->query_first('SELECT sum(event_value) FROM ' . EVENTS_TABLE . " WHERE event_name='" . addslashes($this->raid['raid_name']) . "'");
        }
        ...
        if ( !empty($this->raid['raid_name']) )
        {
            ...
            $correct_raid_earned = $db->query_first("SELECT sum(item_value) FROM " . ITEMS_TABLE . " WHERE raid_id=" . $this->url_id . "");
            $current_raid_att = $db ->query_first("SELECT count(*) FROM " . RAID_ATTENDEES_TABLE . " WHERE raid_id=" . $this->url_id . "");
        ...
        $tpl->assign_vars(array(
			...
			// Date Picker
            'DATEPICK'        => $jqueryp->Calendar('raid_date', date('d.m.Y', $this->time)),
			'H'              => date('H', $this->time),
            'MI'             => date('i', $this->time),
            'S'              => date('s', $this->time),
        ...
        $eqdkp->set_vars(array(
            ...
            'gen_simple_header'  => ($_GET['nohead'] == 'true') ? true : false,
            'display'       => true)
        );
    }
}
