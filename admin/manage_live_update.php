...
class Manage_Live_Update extends page_generic {
		...
		$shortcuts = array('user', 'tpl', 'in', 'jquery', 'core', 'config', 'pfh', 'repo' => 'repository');
	...
	public function __construct(){
		...
		$handler = array(
			...
			'step'	=> array('process' => 'handle_ajax_steps'),
		...
		foreach ($this->steps as $id	=> $value){
			...
			$this->tpl->add_js(
				"function lu_step".$id."(){
					...
					$.get('manage_live_update.php".$this->SID."&step=".$id."', function(data) {
					  if (data == 'true'){
