 ...
 *
 * $Id: index.php 8 2006-05-08 17:15:20Z tsigo $
 *
 ******************************/
...
else
{
    ...
    $admin_menu = array(
        'events' => array(
            0 => $user->lang['events'], //$dkpurl, //
            1 => array('link' => $dkpurl . 'admin/' . 'addevent.php' . $SID,   'text' => $user->lang['add'],  'check' => 'a_event_add'),
            2 => array('link' => $dkpurl . 'admin/' .'listevents.php' . $SID, 'text' => $user->lang['list'], 'check' => 'a_event_')
        ),
				'gmultidkp' => array(
            0 => $user->lang['Plus_menuentry'], 
            1 => array('link' => '',         'text' => $user->lang['config_plus'],  'check' => 'a_config_man', 'spezial' => 'javascript:Settings();'),
            2 => array('link' => '',         'text' => $user->lang['plus_vcheck'],  'check' => 'a_config_man', 'spezial' => 'javascript:Updates();'),
            3 => array('link' => $dkpurl . 'admin/' . 'addmulti.php' . $SID,   'text' => $user->lang['Multi_addkonto'],  'check' => 'a_config_man'),
            4 => array('link' => $dkpurl . 'admin/' .'listmulti.php' . $SID, 'text' => $user->lang['Multi_viewkonten'], 'check' => 'a_config_man')
        ),        
        'groupadj' => array(
            0 => $user->lang['group_adjustments'],
            1 => array('link' => $dkpurl . 'admin/' .'addadj.php' . $SID,  'text' => $user->lang['add'],  'check' => 'a_groupadj_add'),
            2 => array('link' => $dkpurl . 'admin/' .'listadj.php' . $SID, 'text' => $user->lang['list'], 'check' => 'a_groupadj_')
        ),
        'indivadj' => array(
            0 => $user->lang['individual_adjustments'],
            1 => array('link' => $dkpurl . 'admin/' .'addiadj.php' . $SID,                                      'text' => $user->lang['add'],  'check' => 'a_indivadj_add'),
            2 => array('link' => $dkpurl . 'admin/' .'listadj.php' . $SID . '&amp;' . URI_PAGE . '=individual', 'text' => $user->lang['list'], 'check' => 'a_indivadj_')
        ),
        'items' => array(
            0 => $user->lang['items'],
            1 => array('link' => $dkpurl . 'admin/' .'additem.php' . $SID,   'text' => $user->lang['add'],  'check' => 'a_item_add'),
            2 => array('link' => $dkpurl . 'admin/' .'listitems.php' . $SID, 'text' => $user->lang['list'], 'check' => 'a_item_'),
            3 => array('link' => $dkpurl . 'admin/' .'updateitemstats.php' . $SID, 'text' => 'Update Itemstats', 'check' => 'a_item_')
        ),
        'mysql' => array(
            0 => $user->lang['mysql'],
            1 => array('link' => $dkpurl . 'admin/' .'mysql_info.php' . $SID, 'text' => $user->lang['mysql_info'], 'check' => '')
        ),
        'news' => array(
            0 => $user->lang['news'],
            1 => array('link' => $dkpurl . 'admin/' .'addnews.php' . $SID,  'text' => $user->lang['add'],  'check' => 'a_news_add'),
            2 => array('link' => $dkpurl . 'admin/' .'listnews.php' . $SID, 'text' => $user->lang['list'], 'check' => 'a_news_')
        ),
        'raids' => array(
            0 => $user->lang['raids'],
            1 => array('link' => $dkpurl . 'admin/' .'addraid.php' . $SID,   'text' => $user->lang['add'],  'check' => 'a_raid_add'),
            2 => array('link' => $dkpurl . 'admin/' .'listraids.php' . $SID, 'text' => $user->lang['list'], 'check' => 'a_raid_'),
            3 => array('link' => $dkpurl . 'admin/' .'lua.php' . $SID,   'text' => $user->lang['lua_parse'],  'check' => 'a_lua_import')
        ),
        'turnin' => array(
            0 => $user->lang['turn_ins'],
            1 => array('link' => $dkpurl . 'admin/' .'addturnin.php' . $SID, 'text' => $user->lang['add'], 'check' => 'a_turnin_add')
        ),
        'general' => array(
            0 => $user->lang['general_admin'],
            1 => array('link' => $dkpurl . 'admin/' .'config.php' . $SID,         'text' => $user->lang['configuration'],  'check' => 'a_config_man'),
            2 => array('link' => $dkpurl . 'admin/' .'manage_members.php' . $SID, 'text' => $user->lang['manage_members'], 'check' => 'a_members_man'),
            3 => array('link' => $dkpurl . 'admin/' .'plugins.php' . $SID,        'text' => $user->lang['manage_plugins'], 'check' => 'a_plugins_man'),
            4 => array('link' => $dkpurl . 'admin/' .'manage_users.php' . $SID,   'text' => $user->lang['manage_users'],   'check' => 'a_users_man'),
            5 => array('link' => $dkpurl . 'admin/' .'logs.php' . $SID,           'text' => $user->lang['view_logs'],      'check' => 'a_logs_view')
        ),
        'styles' => array(
            0 => $user->lang['styles'],
            1 => array('link' => $dkpurl . 'admin/' .'styles.php' . $SID . '&amp;mode=create', 'text' => $user->lang['create'], 'check' => 'a_styles_man'),
            2 => array('link' => $dkpurl . 'admin/' .'styles.php' . $SID,                      'text' => $user->lang['manage'], 'check' => 'a_styles_man')
        ),
        'update' => array(
            0 => 'EQDKP Update',
            1 => array('link' => $dkpurl . 'admin/' .'update.php', 'text' => 'Update', 'check' => 'a_config_man'),
       )     

    );

    // Now get plugin hooks for the menu
    #$admin_menu = array_merge($admin_menu, $pm->get_menus('admin_menu'));
        ...
        foreach ( $v as $k2 => $row )
        {
            ...
            if ( ($row['check'] == '') || ($user->check_auth($row['check'], false)) )
            {
            		if($row['spezial']){
            			...
            			$adm_url = '<a href="' . $row['link'] . '">' . $row['text'] . '</a>';
            		}
