...
if ( !defined('IN_ADMIN') )
{
    ...
    */
    ...
    if($in->get('rssajax')){
      ...
      if($conf_plus['pk_rssfeed_lefttime'] < (time()+($rsscachetime*3600))){
        $rssleft_out = $urlreader->GetURL('http://rss.eqdkp-plus.com/notifications.xml');
        $plusdb->UpdateConfig('pk_rssfeed_lefttime', time(), $isplusconfarray);
        $plusdb->UpdateConfig('pk_rssfeed_leftdata', $xmltools->prepareSave($rssleft_out), $isplusconfarray);
      }else{
      	$rssleft_out = $xmltools->prepareLoad($conf_plus['pk_rssfeed_leftdata']);
      }
      if($conf_plus['pk_rssfeed_righttime'] < (time()+($rsscachetime*3600))){
      	$language = ($user->lang['XML_LANG'] == 'de') ? '_de' : '_en';
        $rssright_out = $urlreader->GetURL('http://www.eqdkp-plus.com/news'.$language.'.xml');
        $plusdb->UpdateConfig('pk_rssfeed_righttime', time(), $isplusconfarray);
        $plusdb->UpdateConfig('pk_rssfeed_rightdata', $xmltools->prepareSave($rssright_out), $isplusconfarray);
      }else{
      	$rssright_out = $xmltools->prepareLoad($conf_plus['pk_rssfeed_rightdata']);
      }
      if($conf_plus['pk_rssfeed_twittertime'] < (time()+($rsscachetime*3600))){
        $rsstwitter_out = $urlreader->GetURL('http://rss.eqdkp-plus.com/twitter.xml');
        $plusdb->UpdateConfig('pk_rssfeed_twittertime', time(), $isplusconfarray);
      	$plusdb->UpdateConfig('pk_rssfeed_twitterdata', $xmltools->prepareSave($rsstwitter_out), $isplusconfarray);
      }else{
      	$rsstwitter_out = $xmltools->prepareLoad($conf_plus['pk_rssfeed_twitterdata']);
      }

      // Output the RSS Data
      header('content-type: application/xml');
      switch($in->get('rssajax')){
      	case 'left':		print $rssleft_out;			break;
      	case 'right':		print $rssright_out;		break;
    ...
    // "Game Update" Check
    if(!$show_update_warning)   // Only chck if not true..
    {
    ...
    if($pcache->safe_mode){
      if($pcache->CheckWrite()){
        $tpl->assign_vars(array(
            'SHOW_CHMOD_THING'   => true,
            'CHMOD_TXT'          => $user->lang['pcache_safemode_error'],
          ));
        $show_info_tab = true;
      }
    }
    ...
    if(is_array($pcache->errors)){
      foreach($pcache->errors as $cacheerrors){
        $tpl->assign_vars(array(
            'SHOW_CHMOD_THING'   => true,
            'CHMOD_TXT'          => $user->lang[$cacheerrors],
          ));
        $show_info_tab = true;
      }
    }

    if ($in->get('cache')=='reset') {
    	if ($user->check_auth('a_config_man'))
    	{
    		$pdc->del_suffix('');
    ...
    // THIS WILL BE REMOVED OR UPDATED IN 0.7!!!
    include($eqdkp_root_path.'libraries/pluginUpdates/pluginUpdates.class.php');
    include($eqdkp_root_path.'libraries/PlusUpdCheck/PlusUpdCheck.class.php');
    // END OF OBSOLENT CODE

    // Check for required Plugin Database Updates
    include_once($eqdkp_root_path.'pluskernel/include/php.class.php');
    $plugCheckDB  = new pluginUpdates();
    $UpdateCheck  = new PlusUpdCheck();
    $specialphp   = new phpAdditions();

    //Strict Mode
    $strict_check = $db->query_first('SELECT @@global.sql_mode;');
    $strict_mode = false;
    if($strict_check){
      $strict_array = explode(",", $strict_check);
      if(in_array("STRICT_TRANS_TABLES", $strict_array) || in_array("STRICT_ALL_TABLE", $strict_array)){
        $strict_mode = true;
      }
    }
    ...
    $tpl->assign_vars(array(
        ...
        'PLUG_DB_CHECK'        => $plugCheckDB->OutputHTML(),
        'PLUS_UPDATE_CHECK'    => $UpdateCheck->OutputHTML(),

        // Info Tabs
        'TAB_HEAD_ADMINSETT'   => $jqueryp->Tab_header('admininfos_tabs'),
        'TAB_SELECT_UPDATES'	 => (($show_info_tab || $plugCheckDB->CheckStatus() || $UpdateCheck->CheckStatus()) ? $jqueryp->Tab_Select('admininfos_tabs', '1') : ''),
        ...
        'SERVERINFO_SAFEMODE'  => $specialphp->get_php_setting('safe_mode',1,0),
        'SERVERINFO_REGGLOBAL' => $specialphp->get_php_setting('register_globals',1,0),
        'SERVERINFO_CURL'      => $specialphp->get_curl_setting(1),
        'SERVERINFO_FOPEN'     => $specialphp->check_PHP_Function('fopen',1),
        ...
        'RSS_FEED1_JQUERY'     => $jqueryp->rssFeeder('newsfeed_left', "index.php?rssajax=left", '3', '200'),
        'RSS_FEED2_JQUERY'     => $jqueryp->rssFeeder('newsfeed_right', "index.php?rssajax=right"),
        'RSS_TWITTER_JQUERY'   => $jqueryp->rssFeeder('twitterfeed_left', "index.php?rssajax=twitter"),
        ...
        'L_RSS_HEAD3'   => $user->lang['rssadmin_head3'],

        'L_NEW_ACTIONS' => $user->lang['new_actions'],

        'SHOW_PLUSUPDATE'     => $UpdateCheck->CheckStatus(),
        'SHOW_DBUPDATES'      => $plugCheckDB->CheckStatus(),
...
else
{
    ...
    // Mysterious line... for testing purposes only.. there'll never be a plugin named "plugmanager"
    $specialplugins = ($pm->check(PLUGIN_INSTALLED, 'plugmanager')) ? 'plugins/plugmanager/admin/plugins.php' : 'admin/plugins.php';
     ...
     $raid_menu = array(
      ...
      'gmultidkp' => array(
           99 => 'wand.png',
            ...
            2 => array('link' => 'pluskernel/portalmanager.php',         'text' => $user->lang['portalmanager'],  'check' => 'a_config_man'),
            ...
            5 => array('link' => 'admin/' .'index.php?cache=reset' , 'text' => 'Reset Cache', 'check' => 'a_config_man')
        ...
        'general' => array(
           99 => 'wrench.png',
            ...
            3 => array('link' => $specialplugins . $SID,        'text' => $user->lang['manage_plugins'], 'check' => 'a_plugins_man'),
        ...
        if(!$_HMODE){$admin_menu = array_merge($admin_menu,$mysql_menu); }
       	if($conf_plus['pk_multidkp'] == 1){unset($dkp_menu['groupadj']); }
        if(!$conf_plus['pk_noDKP'] == 1) {$admin_menu = array_merge($admin_menu,$dkp_menu); }
        if(!$conf_plus['pk_noRaids'] == 1) {$admin_menu = array_merge($admin_menu,$raid_menu); }
        if(!$conf_plus['pk_noEvents'] == 1) {$admin_menu = array_merge($admin_menu,$event_menu); }
        if((!$conf_plus['pk_noItemPrices'] == 1) || (!$conf_plus['pk_noItemHistoy'] == 1))
        {
        	$admin_menu = array_merge($admin_menu,$item_menu);
        }
