...
".$jquery->Dialog_close($window_close_array , false, false, true)."
</head>";
  ...
  $ii = 0;
	...
	if($step=='items')
	{
		$sql = "SELECT DISTINCT item_name FROM __items order by item_name" ;
	}
  ...
  if($step=='trade')
 	...
 	if($step=='bad')
	{
 	...
 	if($step=='all')
	{
	 		$sql = 'SELECT DISTINCT item_name FROM ' . item_cache_table . ' __items';
	}
