...
class Manage_Menus extends page_generic {
	...
	public function __construct(){
		...
		$handler = array(
			...
			'mode' => array('process' => 'delete_plink')
		);
			...
			foreach ($sort_ary as $key=>$value){
				if ($value === 'new'){
					if (isset($arrNewLinks['new'.$menuid])){
						...
						$url = $this->user->removeSIDfromString($url);
					}
				...
				$sort[$url] = array('sort'=> $i, 'hide' => $hide_ary[$value]);
				$i++;
			}
	...
	public function display($messages=false){
		...
		foreach ($links as $link){
			...
			$menus['menu'.$link['menu']][] = array($url, $link['name'], 'plus_link_id'	=> $link['id']);
		}
		...
		foreach (array(1,2,3,4) as $menuid){
				...
				if ($this->config->get('sort_menu'.$menuid)){
					...
					if(isset($menus['menu'.$menuid]) && is_array($menus['menu'.$menuid])){
						...
						foreach ($menus['menu'.$menuid] as $key=>$menu){
							...
							if (isset($sort_ary[$link]['sort'])){
								$tmp_menu[$key] = $sort_ary[$link]['sort'];
							} else {
								$tmp_menu[$key] = 999999;
							}
						}
				...
				if(isset($menus['menu'.$menuid]) && is_array($menus['menu'.$menuid])){
					foreach ($menus['menu'.$menuid] as $row){
						...
						$block = (isset($sort_ary[$link]['hide']) && $sort_ary[$link]['hide'] == 1) ? 'menu'.$menuid.'hide_row' : 'menu'.$menuid.'_row';
						$vars = array(
							'NAME'				=> $row[1],
							'LINK'				=> $link,
							'ID'				=> 'm'.md5('menu'.$menuid.$link),
							'SORT'				=> (isset($sort_ary[$link]['sort'])) ? $sort_ary[$link]['sort'] : '',
							'HIDE'				=> (isset($sort_ary[$link]['hide']) && $sort_ary[$link]['hide'] == 1) ? 'checked="checked"' : '',
						);
					...
					if(is_array($v)){
						foreach ( $v as $k2 => $row ){
							...
							if($admnsubmenu){
								...
								$this->jquery->Collapse('#container_'.$ident);
								$this->tpl->assign_block_vars('group_row.menu_row', array(
									'NAME' => '<img src="'.(($row['icon']) ? $image_path.$row['icon'] : $image_path.'plugin.png').'" alt="" /> '.$row['name'],
									'GROUP'	=> $row['name'],
									'IDENT'	=> $ident,
								));
								// Submenu
