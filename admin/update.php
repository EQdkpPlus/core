...
class update_eqdkp extends EQdkp_Admin
{
  ...
  {
    global $db, $eqdkp, $user, $tpl, $pm, $eqdkp_root_path, $SID, $a_system, $a_styles, $in, $pdc;
    parent::eqdkp_admin();
    ...
    if(isset($_GET['update']))
    {
      ...
      if(array_key_exists($update_type, $a_system) || array_key_exists($update_type, $a_styles))
      {
        switch ($update_type)
        {
                ...
                case "631" : $sql_count = $this->update_database($update_type.'.sql');
                		//repair item_cache-table if necessary
                		if(!$db->query("SELECT `item_id` FROM item_cache LIMIT 1;")) {
                			$db->query("ALTER TABLE item_cache ADD `item_id` VARCHAR(100) DEFAULT '0' AFTER `item_name`;");
                		}
                		if(!$db->query("SELECT `item_lang` FROM item_cache LIMIT 1;")) {
                			$db->query("ALTER TABLE item_cache ADD `item_lang` VARCHAR(2) DEFAULT '' AFTER `item_id`;");
                		}
                		//add permissions for userlist to every user
                		$sql =  "INSERT INTO __auth_users (`user_id`, `auth_id`, `auth_setting`) VALUES ";
                		$sqls = array();
                		$result =  $db->query("SELECT u.user_id, a.auth_id FROM __users u, __auth_users a WHERE u.user_id = a.user_id;");
                		$data = array();
                		while ($row = $db->fetch_record($result)) {
                			$data[$row['user_id']][] = $row['auth_id'];
                		}
                		foreach($data as $user_id => $auth_array) {
                			if(!in_array('34', $auth_array)) {
                				$sqls[] = "('".$user_id."', '34', 'Y')";
                			}
                		}
                		unset($data);
				...
				foreach($a_system as $key => $value)
				{
					if($value['state'] == 0)
					{
            			switch ($key)
            			{
		                	...
		                	case "631" : $sql_count = $this->update_database($key.'.sql');
		                		//repair item_cache-table if necessary
		                		if(!$db->query("SELECT `item_id` FROM item_cache LIMIT 1;")) {
		                			$db->query("ALTER TABLE item_cache ADD `item_id` VARCHAR(100) DEFAULT '0' AFTER `item_name`;");
		                		}
