...
class ManageCrons extends page_generic {
	...
	public function __construct() {
		...
		$handler = array(
			'mode' => array(
				array('process' => 'run', 'value' => 'run'),
				array('process' => 'enable', 'value' => 'enable'),
				array('process' => 'disable', 'value' => 'disable'),
		...
		if (is_array($this->crons)){
			...
			foreach ($tmp_crons as $key=>$value){
				...
				$this->tpl->assign_block_vars('cron_row', array(
					...
					'U_RUN_CRON'	=> 'manage_crons.php'.$this->SID.'&amp;mode=run&amp;cron='.$key,
					'U_EDIT_CRON'	=> 'manage_crons.php'.$this->SID.'&amp;mode=edit&amp;cron='.$key,
					'U_ACTIVATE_CRON'	=> 'manage_crons.php'.$this->SID.'&amp;mode='.(($value['active']) ? 'disable' : 'enable').'&amp;cron='.$key,
				));
			} //close foreach	
		} //close if array
