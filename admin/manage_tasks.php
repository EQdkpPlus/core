...
class ManageTasks extends page_generic {
	...
	public function __construct(){
		$handler = array(
			'mode' => array(
				array('process' => 'confirm_all',	'value' => 'confirm_all',	'check' => 'a_members_man'),
				array('process' => 'confirm',		'value' => 'confirm',		'check' => 'a_members_man'),
				array('process' => 'delete_all',	'value' => 'delete_all',	'check' => 'a_members_man'),
				array('process' => 'delete',		'value' => 'delete',		'check' => 'a_members_man'),
				array('process' => 'revoke',		'value' => 'revoke',		'check' => 'a_members_man'),
				array('process' => 'activate',		'value' => 'activate',		'check' => 'a_users_man'),
				array('process' => 'activate_all',	'value' => 'activate_all',	'check' => 'a_users_man'),
				array('process' => 'delete_user',	'value' => 'delete_user',	'check' => 'a_users_man'))
		);
	...
	public function activate(){
		if ($this->in->exists('user')) $this->pdh->put('user', 'activate', array($this->in->get('user', 0)));
		$this->display();
	}
	...
	public function display(){
		...
		$this->jquery->Dialog('ConfirmAllChars', '', array('url' => 'manage_tasks.php'.$this->SID.'&mode=confirm_all', 'message' => $this->user->lang('uc_confirm_msg_all')), 'confirm');
		$this->jquery->Dialog('DeleteChar', '', array('url'=>'manage_tasks.php'.$this->SID.'&mode=delete&member=\'+member+\'', 'withid'=>'member', 'message'=>$this->user->lang('uc_del_warning')), 'confirm');
		$this->jquery->Dialog('DeleteAllChars', '', array('url'=>'manage_tasks.php'.$this->SID.'&mode=delete_all', 'message'=>$this->user->lang('uc_del_msg_all')), 'confirm');
		$this->jquery->Dialog('ActivateAllUsers', '', array('url'=>'manage_tasks.php'.$this->SID.'&mode=activate_all', 'message'=>$this->user->lang('activate_all_warning')), 'confirm');
		$this->jquery->Dialog('DeleteUser', '', array('url'=>'manage_tasks.php'.$this->SID.'&mode=delete_user&user=\'+v+\'', 'withid'=>'member', 'message'=>$this->user->lang('confirm_delete_user')), 'confirm');
