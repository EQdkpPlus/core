...
class EQdkp_Config extends EQdkp_Admin
{
	...
	if (( $_POST['default_game'] != $current_game ))
	{
		...
		$set_defaultgame = $eqdkp->root_path . 'games/' . $_POST['default_game'] . '.php' ;
		if (file_exists($set_defaultgame))
		{
			include($set_defaultgame);
	        $manage = new Manage_Game;
			$manage->do_it($db,$table_prefix,false);
		}
		else d($set_defaultgame);
	...
	// Cheesy, but extensible and effective.
       ...
       $tpl->assign_block_vars('game_row', array(
            'VALUE' => "LOTRO_german",
            'SELECTED' => ( $eqdkp->config['default_game'] == "LOTRO_german" ) ? ' selected="selected"' : '',
            'OPTION' => "LOTRO_german") );

       $tpl->assign_block_vars('game_row', array(
            'VALUE' => "LOTRO_english",
            'SELECTED' => ( $eqdkp->config['default_game'] == "LOTRO_english" ) ? ' selected="selected"' : '',
            'OPTION' => "LOTRO_english") );
