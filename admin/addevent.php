				...
				if($dir=opendir($path))
				{
					...
					if ( @sizeof($files) > 0 )
				        ...
				        for ( $i = 0; $i < $rows; $i++ )
				        {
				            ...
				            {
				                $offset = ($i + ($rows * $j));
				                #$attendee = ( isset($files[$offset]) ) ? $files[$offset] : '';
				                $filename = ( isset($files[$offset]) ) ? $files[$offset] : '';

				                if ( $filename != '' )
				                {
				                    $block_vars += array(
				                       									 'COLUMN'.$j.'_NAME' => $filename ,
				                       									 'COLUMN'.$j.'_CHECKED' => ( $filename == $this->event['event_icon'] ) ? 'checked="checked"' : '',
						  		                     					 'COLUMN'.$j.'_IMG' => "<img height='50' width='50'  src='".$path.'/'.$filename."'>"
				            ...
				            $tpl->assign_block_vars('files_row', $block_vars);
				        }
