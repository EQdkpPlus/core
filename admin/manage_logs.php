...
class Manage_Logs extends page_generic {
	...
	public function __construct(){
		...
		$handler = array(
			'reset'			=> array('process' => 'reset_logs',			'check' => 'a_logs_del'),
			'del_errors'	=> array('process' => 'delete_errors',		'check' => 'a_logs_del'),
			'dellogdays'	=> array('process' => 'delete_log_days',	'check' => 'a_logs_del')
		);
	...
	public function display(){
		...
		$this->jquery->Dialog('delete_warning', '', array('url'=>'manage_logs.php'.$this->SID.'&reset=true', 'message'=>$this->user->lang('confirm_delete_logs')), 'confirm');
		...
		$this->tpl->assign_vars(array(
			...
			'FILTER_SELECT'			=> $this->html->DropDown('plugin', $plugin_list, $this->in->get('plugin'), '', 'onchange="window.location=\'manage_logs.php?plugin=\'+document.post.plugin.value"'),
			'ERROR_FILTER_SELECT'	=> $this->html->DropDown('error_dd', $error_list, $this->in->get('error'), '', 'onchange="window.location=\'manage_logs.php?error=\'+document.post2.error_dd.value"'),
			'ERROR_TYPE_SELECT'		=> $this->html->DropDown('error_type_dd', $type_list, $this->in->get('type'), '', 'onchange="window.location=\'manage_logs.php?type=\'+document.post2.error_type_dd.value"'),
