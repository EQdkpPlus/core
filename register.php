...
class Register extends EQdkp_Admin
{
    ...
    {
        global $db, $eqdkp, $user, $tpl, $pm, $conf_plus, $eqdkp_root_path;
        global $SID;
        ...
        if ( $result = $db->query($sql) )
        {
            if ( $row = $db->fetch_record($result) )
            {
                ...
                //
                include_once($eqdkp->root_path . 'libraries/MyMailer/MyMailer.class.php');
                $email = new MyMailer(
                	array(
                ...
                if($email->SendMailFromAdmin($row['user_email'], $user->lang['new_password'], 'user_new_password.txt', $bodyvars, $conf_plus['lib_email_method'])) {
                	message_die($user->lang['password_sent']);
                } else {
                	message_die($user->lang['error_email_send']);
                }
            }
            else
            {
