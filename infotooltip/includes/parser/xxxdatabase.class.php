...
if(!class_exists('xxxdatabase')) {
  class xxxdatabase extends itt_parser {
	...
	{
		$this->{$type.'list'} = unserialize(file_get_contents($this->cache->FilePath($this->config['game'].'_'.$lang.'_'.$type.'list.itt', 'itt_cache')));
		if(!$this->itemlist OR $forceupdate)
		{
			$urlitemlist = $this->urlreader->GetURL('http://www.'.$this->config['game'].'database.com/xml/'.$this->av_langs[$lang].'/'.$type.'s/'.$type.'list.xml');
			$xml = simplexml_load_string($urlitemlist);
			foreach($xml->children() as $item) {
				$name = (string) $item['name'];
				$this->{$type.'list'}[(int)$item['id']][$lang] = $name;
			}
			$this->cache->putContent(serialize($this->{$type.'list'}), $this->cache->FilePath($this->config['game'].'_'.$lang.'_'.$type.'list.itt', 'itt_cache'));
		}
		return true;
	}
	...
	private function getItemIDfromUrl($itemname, $lang, $searchagain=0)
	{
		...
		$data = $this->urlreader->GetURL('http://'.(($lang == 'en') ? 'www' : $lang).'.'.$this->config['game'].'database.com/search?q='. $codedname);
