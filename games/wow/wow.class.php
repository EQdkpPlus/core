...
if(!class_exists('wow')) {
	class wow extends game_generic {
		...
		public function chartooltip($intCharID){
			...
			$charhtml .= '<br />'.$this->pdh->get('member', 'html_racename', array($intCharID)).$this->pdh->get('member', 'racename', array($intCharID));
		...
		public function get_OnChangeInfos($install=false){
			...
			if($install){
				...
				//default links
				array_push($info['aq'], "INSERT INTO __plus_links (`link_url`, `link_name`, `link_window`, `link_menu`) VALUES ('http://eu.battle.net/wow/', 'WoW Battle.net', 1, 0);");
		...
		public function get_profilefields(){
			...
			$xml_fields['prof1_name']['option'] = $professions;
			$xml_fields['prof2_name']['option'] = $professions;
			return $xml_fields;
		}
