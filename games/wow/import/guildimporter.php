...
class guildImporter extends page_generic {
	...
	public function perform_step1(){
		if($this->in->get('guildname', '') == ''){
			return '<div class="infobox infobox-large infobox-red clearfix"><i class="icon-warning-sign icon-4x pull-left"></i> <span id="error_message_txt>'.$this->game->glang('uc_imp_noguildname').'</span></div>';
		}
		...
		if(!isset($guilddata['status'])){
			...
			$hmtlout .= '<div class="infobox infobox-large infobox-red clearfix"><i class="icon-warning-sign icon-4x pull-left"></i> <span id="error_message_txt">'.$guilddata['reason'].'</span></div>';
		}
		return $hmtlout;
	}
