			...
			foreach ($d_charfeed as $v_charfeed){
				switch ($v_charfeed['type']){
						case 'achievement':
							...
							$bnetLink = $this->game->obj['armory']->bnlink($chardata['name'], $this->config->get('uc_servername'), 'guild-achievements', $this->config->get('guildtag')).'#'.$achievCat.':a'.$v_charfeed['achievementID'];
							$class='';
