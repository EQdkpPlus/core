...
class bnet_armory {
	...
	*/
	public function character($user, $realm, $force=false){
		...
		$wowurl	= $this->_config['apiUrl'].sprintf('wow/character/%s/%s?locale=%s&fields=guild,stats,feed,talents,items,reputation,titles,professions,appearance,companions,mounts,pets,achievements,progression,pvp,quests', $realm, $user, $this->_config['locale']);
	...
	*/
	public function guild($guild, $realm, $force=false){
		...
		$wowurl	= $this->_config['apiUrl'].sprintf('wow/guild/%s/%s?locale=%s&fields=members,achievements,news', $realm, $guild, $this->_config['locale']);
	...
	* 
	* @param $itemid	battlenet quest ID
	* @param $force		Force the cache to update?
	* @return bol
	*/
