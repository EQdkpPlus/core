 ...
 * @version		$Rev: 12953 $
 * 
 * $Id: riftspot.class.php 12953 2013-01-30 15:51:43Z wallenium $
 */
	...
	class riftspot extends itt_parser {
		...
		protected function getItemData($item_id, $lang, $itemname='', $type='items'){
			...
			if (preg_match('#name:\'(.*?)\', quality:\'(.*?)\', content:\'(.*?)\', icon:\'(.*?)\'#', $itemdata, $matches)){
				...
				$content = $matches[3];
				$icon = $matches[4];
				$template_html = trim(file_get_contents($this->root_path.'games/rift/infotooltip/templates/riftspot.tpl'));
