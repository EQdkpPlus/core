...
class ItemCache
{
	...
	function saveItem($item)
	{
		...
		global $db, $eqdkp, $user , $table_prefix;
 		if (!defined('ITEMS_TABLE')) { define('ITEMS_TABLE', $table_prefix . 'items'); }
		$sql = 'show columns FROM ' . ITEMS_TABLE . " like 'game_itemid'";		
		
		$result = $this->sql->query($sql);
		if (!mysql_num_rows($result) > 0)
			{
				$sql = 'ALTER TABLE ' . ITEMS_TABLE . " ADD game_itemid INT(10) UNSIGNED NULL";		
				$this->sql->query($sql);
			}
			
		$sql = "UPDATE " . ITEMS_TABLE . " SET game_itemid = ".$item['item_id']." WHERE item_name = '".addslashes($item['name'])."'";
		$result2 = $this->sql->query($sql);
		
		if (!$result2)
		{
			$error = $this->sql->error();
			$error = $error['message'];
			#echo "Failed to update Game item ID: $name ($error)</br>";
		}
