...
class ParseWowhead
{
	...
	function getItem($name)
    {
		...
		foreach($langs as $lang)
		{
			if($item_data = $this->getItemData('http://'.$lang.'.wowhead.com/item='.$encoded_name.'&xml'))
			{
				...
				if($item_data[0]['child'][0]['child'][0]['data'] == utf8_encode($fixed_name))
				{
					//found an item
					break;
				}
	...
	function buildTooltip($item_data, $item_name='')
	{
		...
		$item['name'] = ($item_name) ? $item_name : utf8_decode($properties['NAME']['data']);
		$item['lang'] = ($this->urlprefix == 'www') ? 'en' : $this->urlprefix;
