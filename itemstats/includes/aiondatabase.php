...
class AionDatabase
{
	...
	//initializes the itemlist. if it does not exists in the cache, get it from: http://www.aiondatabase.com/xml/en_US/items/itemlist.xml
	function getItemlist($forceupdate=false, $lang=false)
	{
		global $pcache, $eqdkp;

		$lang = ($lang) ? $lang : $eqdkp->config['game_language'];

		$itemlist_file = $pcache->FilePath($lang.'.aion_itemlist.itemcache', 'itemstats');
		$filesize = filesize($itemlist_file);
		if(!(!$forceupdate AND $pcache->FileExists($lang.'.aion_itemlist.itemcache', 'itemstats') AND $filesize)) #upate itemlist once a week
		{
		  if(filemtime($itemlist_file) < (time()-(24*60*60)) OR !$filesize) //only update once a day, except we dont have an itemlist
		  {
			$urlitemlist = itemstats_read_url('http://www.aiondatabase.com/xml/'.$this->av_langs[$lang].'/items/itemlist.xml');
			...
			foreach($itemlist as $key => $item)
			{
				...
				$this->itemlist[$key]['name'][$lang] = (isUTF8($item['attr']['NAME'])) ? utf8_decode($item['attr']['NAME']) : $item['attr']['NAME'];
			}
	...
	function getItemIDfromItemlist($itemname, $forceupdate=false, $searchagain=0, $lang=false)
	{
		$searchagain++;
        $lang = ($lang) ? $lang : $eqdkp->config['game_language'];
		$this->getItemlist($forceupdate,$lang);
		$item_id = 0;
		...
		if(!$item_id AND count($this->av_langs) > $searchagain) {
			$toload = array();
		  	foreach($this->av_langs as $c_lang => $langlong) {
		  		if(!in_array($c_lang,$loaded_langs)) {
		  			$toload[] = $c_lang;
		  		}
		  	}
		  	foreach($toload as $lang) {
		  		$item_id = $this->getItemIDfromItemlist($itemname, true, $searchagain, $lang);
		  		if($item_id) {
		  			break;
		  		}
		  	}
	...
	function getItemIDfromUrl($itemname, $searchagain=true, $lang=false)
	{
		global $eqdkp;
		$lang = ($lang) ? $lang : $eqdkp->config['game_language'];
		...
		if($item_id) {
			...
			return ($searchagain) ? $this->getItemIDfromUrl($itemname, false, (($lang == 'de') ? 'en' : 'de')) : null;
		}
	}
	...
	private function getItemData($item_id, $itemname, $lang=false)
	{
        settype($item_id, 'int');
		$lang = ($lang) ? $lang : $eqdkp->config['game_language'];
		...
		$item['link'] = $url."/items/xmls/".$item['id'].".xml";
