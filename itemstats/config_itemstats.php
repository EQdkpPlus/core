...
elseif ((($GLOBALS["prio"][0] == 'wowhead') or ($GLOBALS["prio"][0] == 'armory')) AND strtolower($eqdkp->config['default_game']) == 'wow')  // WoWHead & Armory Support
{
	...
	define('ICON_STORE_LOCATION', 'http://static.wowhead.com/images/wow/icons/medium/');
	define('ICON_EXTENSION', '.jpg');
	define('DEFAULT_ICON', 'inv_misc_questionmark');

   //armory settings
   if($eqdkp->config['game_language'] == 'es')
   {
      $GLOBALS["armory_lang"][]='es';
...
{
	define('tooltip_css', 'aion.css');
	define('ICON_STORE_LOCATION', 'http://www.aiondatabase.com/res/icons/40/');
	define('ICON_EXTENSION', '.png');
	define('DEFAULT_ICON', 'icon_item_box05');
	unset($GLOBALS["prio"]);
	$GLOBALS["prio"][] = 'aion';
}
else
...
// It can be : 'en', 'fr', 'de'
if($eqdkp->config['game_language'])
{
	define('item_lang_default', $eqdkp->config['game_language']);
}
else
{
