...
// If this item has a link to the info site, add this link to the HTML.  If it doesn't have a link, it
	...
	/*	if (!empty($item_tooltip_html))
        ...
        else // Balise Itemico
        {
            ...
            $item_html = "<a id=\"".$item_name."\" ". $item_tooltip_html ." target=\"_blank\" href=\"" . $item_link . "\"  class=\"forumitemlink\"><div class=\"iconmedium\" style=\"float:left;background-image: url(".$item_html.");\" ><div class=\"tile\"></div></div></a>";
	...
	function    getItemNameFromId($id, $lang_object, $update = false)
	{
        if ($lang_object == '')
            $lang_object = item_lang_default;
            if (!isset($_GET["forcedb"])) $proper_name = $this->item_cache->getItemNameFromId($id, $lang_object);
	...
	function updateItemFromName($name,$language='fr')
	{
        if ($name == '')
            return ($name);
