...
//:://////////////////////////////////////////////
//::
//:: File: adminmemberskills.php (admin option page)
...
{
  $tpl->assign_vars(array('E_NO_USE_PROFS' => $user->lang['ts_no_prof_choice'], 'NOPROFS' => TRUE) );
}
else
{
...
$icon_sql = 'SELECT trade_id, trade_icon, trade_name
        	FROM ' . RP_TRADESKILL_TABLE . '
		WHERE trade_id >=0
		ORDER BY trade_id ASC';
		
if (!($icon_result = $db->query($icon_sql))) { message_die('Could not obtain trade information', '', __FILE__, __LINE__, $icon_sql); }

while ( $icon_row = $db->fetch_record($icon_result) )
{
	if($icon_row['trade_id'] != 7)
	{
		$tpl->assign_block_vars('icon_row', array(
		'TRADESKILL_ICON' => $icon_row['trade_icon'],
       		'TRADESKILL_NAME' => $icon_row['trade_name'],
       		'ID' => $icon_row['trade_id'],
		));
	}
}
...
$member_sql = 'SELECT member_name, member_id, member_status FROM ' . MEMBERS_TABLE . ' ORDER BY member_name ASC';
...
{ 
	...
	$member_status = $member_row['member_status'];

	$tpl->assign_block_vars('member_row', array(
	'ROW_CLASS'     => $eqdkp->switch_row_class(),
        'MEMBER_NAME' => ( ($member_status == 1) ? $member_row['member_name'] : '<i>'.$member_row['member_name'].'</i>'),
	'MEMBER_ID' => $member_row['member_id']
	));

	$trade_sql = 'SELECT trade_id
        	FROM ' . RP_TRADESKILL_TABLE . '
		WHERE trade_id >=0
		ORDER BY trade_id ASC';
...
$tpl->assign_vars(array(
    ...
    'F_UPDATE' => "adminmemberskills.php",
...
$eqdkp->set_vars(array(
	...
	'template_file' => 'admin/adminmemberskills.html',
