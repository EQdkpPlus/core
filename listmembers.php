...
else
{
    ...
    $tpl->assign_block_vars('filter_row', array(
	 ...
	 if($eqdkp->config['default_game'] == 'WoW_german' || $eqdkp->config['default_game'] == 'WoW_english')
	 {
    ...
    if (isset($_GET['sortby'])){
	    ...
	    foreach($members_rows as $row)
	    {
		   ...
		   $rankimg = get_RankIcon($row['rank_name']);
  		   ...
  		   #$class_icon = get_classNameImgListmembers($row['member_class']) ;

  		   $race_icon = get_RaceIcon($row['member_race_id']);
		   ...
		   # Amory Link code  by Wallenium
	       	#####################################
	       	if(isset($conf_plus['pk_servername']) && isset($conf_plus['pk_server_region'])
	       		&& ($eqdkp->config['default_game'] == 'WoW_german' || $eqdkp->config['default_game'] == 'WoW_english'))
	       	{
				...
				if(($pm->check(PLUGIN_INSTALLED, 'charmanager')) and ($row['member_id'] > 0))
				{
					...
					$last_update = $db->query_first($ma_sql);
