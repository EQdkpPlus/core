...
else
{
    ...
    'OPTION'   => str_replace('_', ' ', "--------")));
	 ...
	 #Pool1: Pala, Rogue, Shaman =IDs 5,13,2,8,9
	 ############################################## 
    
    $sql = 'SELECT class_name, class_id, class_min_level, class_max_level FROM ' . CLASS_TABLE .'';
	  $sql .= ' WHERE (class_id=5) or (class_id=13) or (class_id=2) or (class_id=8) or (class_id=9)';
	  $result = $db->query($sql); 
	  
	  if($result){ 
		  while ( $row = $db->fetch_record($result) ){
		  	$pool1[] = $row['class_name'];}
		  
		  foreach(array_unique($pool1) as  $key){
		  $pool1_classes .= $key.", ";}

	  	$tpl->assign_block_vars('filter_row', array(
      'VALUE' => 'pool1',
      'SELECTED' => ( strtolower($filter) == strtolower('pool1') ) ? ' selected="selected"' : '',
      'OPTION'   => ( !empty($pool1_classes) ) ? stripslashes($pool1_classes) : '(None)' )
      );
    }     
	 ...
	 ###############################################
	  ...
	  $sql = 'SELECT class_name, class_id, class_min_level, class_max_level FROM ' . CLASS_TABLE .'';
	  $sql .= ' WHERE (class_id=1) or (class_id=12) or (class_id=6) or (class_id=7)';
	  $result = $db->query($sql); 
	  
	  if($result){ 
		  while ( $row = $db->fetch_record($result) ){
		  	$pool2[] = $row['class_name'];}
		  
		  foreach(array_unique($pool2) as  $key){
		  $pool2_classes .= $key.", ";}

	  	$tpl->assign_block_vars('filter_row', array(
      'VALUE' => 'pool2',
      'SELECTED' => ( strtolower($filter) == strtolower('pool2') ) ? ' selected="selected"' : '',
      'OPTION'   => ( !empty($pool2_classes) ) ? stripslashes($pool2_classes) : '(None)' )
      );
    } 
	 
	 #Pool3: Jäger, Magier, Hexenmeister = 3,4,11,10
	 ###############################################
	 	
	 	$sql = 'SELECT class_name, class_id, class_min_level, class_max_level FROM ' . CLASS_TABLE .'';
	  $sql .= ' WHERE (class_id=3) or (class_id=4) or (class_id=11) or (class_id=10)';
	  $result = $db->query($sql); 
	  
	  if($result){ 
		  while ( $row = $db->fetch_record($result) ){
		  	$pool3[] = $row['class_name'];}
		  
		  foreach(array_unique($pool3) as  $key){
		  $pool3_classes .= $key.", ";}

	  	$tpl->assign_block_vars('filter_row', array(
      'VALUE' => 'pool3',
      'SELECTED' => ( strtolower($filter) == strtolower('pool3') ) ? ' selected="selected"' : '',
      'OPTION'   => ( !empty($pool3_classes) ) ? stripslashes($pool3_classes) : '(None)' )
      );
    } 
    ...
    if ($filter == 'pool1')
    {
    		$sql .= " AND ( (c.class_id='5') or (class_id=13) or (class_id=2) or (class_id=8) or (class_id=9) )";
    }

    if ($filter == 'pool2')
    {
    		$sql .= " AND ( (c.class_id='1') or (class_id=12) or (class_id=6) or (class_id=7))";
    }    
    
  	if ($filter == 'pool3') 
    {
    		$sql .= " AND ( (c.class_id='3') or (class_id=4) or (class_id=11) or (class_id=10))";
    }
   ...
   $sql .= ' ORDER BY '.$current_order['sql']; #original
		...
		while ( $row = $db->fetch_record($members_result) )
    {
        ...
        if ( member_display($row) )
        {
        		...
        		$members_rows[$member_count]['member_earned'] = round($row['member_earned'], 2);
        		$members_rows[$member_count]['member_spent'] = round($row['member_spent'], 2);
        		$members_rows[$member_count]['member_adjustment'] = round($row['member_adjustment'], 2);
						$members_rows[$member_count]['member_current'] = round($row['member_current'], 2);        	
        	...
        	// 
        		...
        		while ( $a_multi = $db->fetch_record($multi_results) )
	        		...
	        		if ( !($multi2event_results = $db->query($sql_events)) )
							...
							//Konten2Events
							//Konten verknüpft mit Events	den Multikonten des Member zuweisen (WAT EIN SHICE !!!!!!!)
							//Ich hasse euch dafür, das ihr das einstellbar haben wolltet !! :D
							//###############################################################			
						  ...
						  while ( $a_multi = $db->fetch_record($multi2event_results) )
							{ // gehe alle Events durch, die einem Konto zugewiesen wurden
								...
								//###############################################################
								$members_rows_multidkp[$member_count][$multicount]['earned'] += round($event_data[$a_multi['multidkp2event_eventname']]['earned'], 2); 
								$members_rows_multidkp[$member_count][$multicount]['spend'] += round($event_data[$a_multi['multidkp2event_eventname']]['spend'], 2); 
								$members_rows_multidkp[$member_count][$multicount]['adjust'] += round($event_data[$a_multi['multidkp2event_eventname']]['adjust'], 2); 
								$members_rows_multidkp[$member_count][$multicount]['current'] += round($current, 2); 
							}	
    ...
    if (isset($_GET['sortby'])){
	    ...
	    {   	       
	       ...
	       {$rankimg = $row['rank_name'];}
	       				
	       	
	       	# Amory Link code  by Wallenium
	       	#####################################
	       	if(isset($conf_plus['pk_servername']) and isset($conf_plus['pk_server_region']))
	       	{	
	       			if($conf_plus['pk_server_region'] =="eu")
	       			{
						...
						$menulink = array();
						$menulink[1] = $armoryurl.'/character-sheet.xml?r='.stripslashes(rawurlencode(utf8_encode($conf_plus['pk_servername']))).'&n='.stripslashes(rawurlencode(utf8_encode($row['member_name'])));
						$menulink[2] = $armoryurl.'/character-talents.xml?r='.stripslashes(rawurlencode(utf8_encode($conf_plus['pk_servername']))).'&n='.stripslashes(rawurlencode(utf8_encode($row['member_name'])));
						...
						if($pm->check(PLUGIN_INSTALLED, 'charmanager')) 
						{
							...
							$menulink[3] = ($guild) ? $armoryurl.'/guild-info.xml?r='.stripslashes(rawurlencode(utf8_encode($conf_plus['pk_servername']))).'&n='.stripslashes(rawurlencode(utf8_encode($guild))).'&p=1' : '';
	          ...
	          #'NAME'          => $row['rank_prefix'] . (( $row['member_status'] == '0' ) ? '<i>' . $row['member_name'] . '</i>' : $row['member_name']) . $row['rank_suffix'],
	           'NAME'          => $row['rank_prefix'] . (( $row['member_status'] == '0' ) ? '<i>' . $row['member_name'] . '</i>' : $row['member_name']) . $row['rank_suffix'],
	          #'RANK'          => ( !empty($row['rank_name']) ) ? (( $row['rank_hide'] == '1' ) ? '<i>' . '<a href="'.$row['u_rank_search'].'">' . stripslashes($row['rank_name']) . '</a>' . '</i>'  : '<a href="'.$row['u_rank_search'].'">' . stripslashes($row['rank_name']) . '</a>') : '&nbsp;',
	          'RANK'          => ( !empty($row['rank_name']) ) ? (( $row['rank_hide'] == '1' ) ? '<i>' . '<a href="'.$u_rank_search.'">' . '</a>' . '</i>'  : '<a href="'.$u_rank_search.'">' .$rankimg.'</a>') : '&nbsp;',
	          'RAICON'          => ( !empty($row['rank_name']) ) ? $row['rank_name'] : '&nbsp;',
						'LEVEL'         => ( $row['member_level'] > 0 ) ? $row['member_level'] : '&nbsp;',
	          'CLASS'         => ( !empty($row['member_class']) ) ? $row['member_class'] : '&nbsp;',
						'ARMOR'		=> ( !empty($row['armor_type']) ) ? $row['armor_type'] : '&nbsp;',
	          'EARNED'        => round($row['member_earned']),
	          'SPENT'         => round($row['member_spent']),
	          'ADJUSTMENT'    => round($row['member_adjustment']),
   ...
   {
    	include_once($eqdkp_root_path . 'listmembers_leaderboard.php');
			...
			if(($conf_plus['pk_default_multi'] <> 'none') and isset($conf_plus['pk_default_multi']) and  ($multifilter =='none')){
				showDKPLeaderboard($conf_plus['pk_default_multi']);
			}
			else{
				showDKPLeaderboard($multifilter);
			}	
   }
}
