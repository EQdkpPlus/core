...
else
{
    ...
    if($conf_plus['pk_attendance60']==1)
    {
	    ...
	    $raid_count_60 = $db->query_first('SELECT count(*) FROM __raids WHERE raid_date BETWEEN '.$sixty_days.' AND '.time());
  	}
    ...
    if($conf_plus['pk_attendanceAll']==1)
    {
		...
		$raid_count_all = $db->query_first('SELECT count(*) FROM __raids') ;
 	}
		   ...
		   # Amory Link code  by Wallenium
	       	...
	       	if(is_object($armory_light))
	       	{
				...
				if(($pm->check(PLUGIN_INSTALLED, 'charmanager')) and ($row['member_id'] > 0))
				{
					...
					$last_update = date($user->lang['uc_changedate'],$ginfo['last_update']);
