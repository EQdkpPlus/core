...
else
{
    ...
    //$sql .= ' ORDER BY member_name'; #custom
...
if(!empty($multi_name))
{
	...
	{ 
	  ...
	  while ( $a_multi = $db->fetch_record($multi2event_results) )
		...
		if(isset($_GET['filter']))
		{
			$sortlink_header = 'listmembers.php'.$SID.'&sortby='.urlencode($key['name']).'&sortorder='.$_GET['sortorder'].'&filter='.$_GET['filter'];
		}
		else
		{
			$sortlink_header = 'listmembers.php'.$SID.'&sortby='.urlencode($key['name']).'&sortorder='.$_GET['sortorder'];
		}
