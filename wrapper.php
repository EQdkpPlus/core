...
class wrapper extends page_generic {
	public static function __shortcuts() {
		$shortcuts = array('user', 'tpl', 'in', 'pdh', 'config', 'core', 'env');
	...
	public function handle_id(){
		...
		if ($linkID == 'lp'){
				...
				if($this->VerifyLink(rawurldecode($this->in->get('f')), array($mywpthing['host']))){
					$this->data = array(
						'url'	=> rawurldecode($this->in->get('f', '')),
						'title'	=> $this->user->lang('forum'),
		...
		//Recruitment
		if(($linkID == 'recemb') and (strlen($this->config->get('pm_recruitment_url')) > 0)){
			$this->data = array(
		...
		//Quicksearch
		/*
		if(($linkID == 'quicksearch') and (strlen($this->config->get('pm_quicksearch_newwindow')) == 0)){
			// whitelist...
			require($this->root_path.'portal/quicksearch/whitelist.php');
			if($this->VerifyLink(rawurldecode($this->in->get('f')), $mywpthing)){
				$url	= rawurldecode($this->in->get('f'));
			}
		}
		*/
	...
	public function display(){
		if (!$this->data){
			...
			if(!$sop){
				...
				$output .='<!--[IF IE]>
								<iframe id="boardframe" src="'.$this->data['url'].'" allowtransparency="true" width="100%" scrolling="no" marginwidth="0" marginheight="0" frameborder="0" vspace="0" hspace="0"></iframe>
							<![if ! IE]><!-->
								<iframe id="boardframe" src="'.$this->data['url'].'" width="100%" scrolling="no" marginwidth="0" marginheight="0" frameborder="0" vspace="0" hspace="0"></iframe>
							<!--><![ENDIF]><![ENDIF]-->';
			}
