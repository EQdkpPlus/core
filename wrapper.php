...
if (isset($url))
{
	...
	if ((($urldata['link_window'] == '3') OR (check_browser($_SERVER['HTTP_USER_AGENT']) AND $linkID == 'board' )))
	{
		$forum_out .= $html->CreateDynamicIframeJS();			
		$forum_out .='<iframe onload="window.scrollTo(0,0)" id="boardframe" src="';
	  	$forum_out .= $url;
		$forum_out .= '" allowtransparency="true" width="100%" scrolling="no" marginwidth="0" marginheight="0" frameborder="0" vspace="0" hspace="0"></iframe>';
	}else 
	{
	  $forum_out .='<iframe onload="window.scrollTo(0,0)" id="boardframe" src="';
	  $forum_out .= $url;
