...
class characters extends page_generic {
	...
	public function __construct() {
		$handler = array(
			'connection_submit'	=> array('process' => 'update_connection', 'check' => 'u_member_conn'),
			'delete_id'			=> array('process' => 'delete_char', 'check' => 'u_member_del'),
			'maincharchange'	=> array('process' => 'ajax_mainchar', 'check' => 'u_member_man'),
			'defrolechange'		=> array('process' => 'ajax_defaultrole', 'check' => 'u_member_man'),
	...
	public function display(){
		...
		$this->jquery->Dialog('AddChar', $this->user->lang('uc_add_char'), array('url'=>'addcharacter.php?simple_head=true', 'width'=>'640', 'height'=>'600', 'onclose'=>$this->env->link.'characters.php'));
		$this->jquery->Dialog('EditChar', $this->user->lang('uc_edit_char'), array('withid'=>'editid', 'url'=>"addcharacter.php?simple_head=true&editid='+editid+'", 'width'=>'640', 'height'=>'600', 'onclose'=>$this->env->link.'characters.php'));

		// The javascript for the mainchar change
		$this->tpl->add_js("
			$('.cmainradio').change( function(){
				$.post('characters.php', { maincharchange: $(this).val() },
					function(data){
						$('#notify_container').notify('create', data);
					});
				});
			$('.cdefroledd').change( function(){
				$.post('characters.php', { defrolechange: $(this).val(), defrolechange_memberid: $(this).attr('name').replace('defaultrole_', '') },
					function(data){
						$('#notify_container').notify('create', data);
					});
			});
		...
		if($this->game->get_importAuth('u_member_add', 'char_import')){
			$this->jquery->Dialog('AddCharArmory', $this->user->lang('uc_ext_import_sh'), array('url'=>$this->game->get_importers('char_import', true), 'width'=>'600', 'height'=>'300', 'onclose'=>$this->env->link.'characters.php'));
		}
		if($this->game->get_importAuth('u_member_view', 'char_update')){
			$this->jquery->Dialog('UpdateChar', $this->user->lang('uc_ext_import_sh'), array('url'=>$this->game->get_importers('char_update', true)."?member_id='+memberid+'", 'width'=>'600', 'height'=>'300', 'onclose'=>$this->env->link.'characters.php', 'withid'=>'memberid'));
		}
		if($this->game->get_importAuth('a_members_man', 'char_mupdate')){
			$this->jquery->Dialog('MassUpdateChars', $this->user->lang('uc_cache_update'), array('url'=>$this->game->get_importers('char_mupdate', true), 'width'=>'600', 'height'=>'450', 'onclose'=>$this->env->link.'characters.php'));
		}
