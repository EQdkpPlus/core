 ...
 */
function System_Message($text='', $title='', $kind='default'){
  global $html, $tpl;
  $tpl->assign_block_vars(
    'msystem', array(
                'MESSAGE' => $html->growlMessage($text, $title, $kind)
    )
  );
}
 ...
 * @param     bool       $whitelist    Array with allowed domain names, p.e. array('localhost','kompsoft.de');
 * @param     string     $sheme        URL Sheme, default is http, might be ftp, http, https....
 * @return    mixed                    true or false...
 */
function VerifyLink($url, $whitelist, $scheme=''){
	global $eqdkp;

	// Parse the Domain...
	$myDomain = parse_url($url);
	$protocol	= ($scheme) ? $scheme : 'http';

	// Check the Sheme matches the Protocol...
	if($myDomain['scheme'] != $protocol){
		return 0;
	}

	// Add the Server path to the whitelist
	if(is_array($whitelist)){
		array_push($whitelist, $eqdkp->config['server_name']);
	}else{
		$whitelist = array($eqdkp->config['server_name']);
	}

	// Check if the Server matches the whitelist...
	if(!in_array($myDomain['host'], $whitelist)){
		return 0;
	}
...
function news_parse(&$message)
{
    ...
    $message = $bbcode->EmbeddedVideo($message);
	  ...
	  // Perform BB Code
    $message = $bbcode->toHTML($message, false);
   ...
   //Achievements-BBCode
		if ($pm->check(PLUGIN_INSTALLED, 'achievements')){	
			include($eqdkp_root_path."plugins/achievements/includes/game_loader.php");
				$message = $acclass->achievementBBcode($message);
		}
	//Close Achievements-BBCode
