...
class Template {
	...
	function set_template($template = '', $root_dir = '')
	{
        ...
        if ( $root_dir == '' )
        {
            $this->root = $eqdkp_root_path . 'templates/' . $template;
        }
        else
        {
        ...
        if ( defined('PLUGIN') )
        {
            $this->cachedir = $eqdkp_root_path . 'templates/cache/' . $template . '/' . PLUGIN . '/';
        }
        else
        {
            $this->cachedir = $eqdkp_root_path . 'templates/cache/' . $template . '/';
        }
    	...
    	if (!file_exists($this->cachedir))
		{
            ...
            {
                // To avoid confusing the user, we're going to replace ./../../ (etc) with just a /
                $cachedir = str_replace($eqdkp_root_path, '/', $this->cachedir);
                $cache_parent = preg_replace('/^(.+)\/.+\/$/', '\1/', $this->cachedir);
                //trigger_error('Could not create template cache dir: ' . $cachedir .' - chmod ' . $cache_parent . ' to 777', E_USER_WARNING);
                unset($cachedir, $cache_parent);
            }
		}

        // Special handling for admin folder
        $admin_folder = $eqdkp_root_path . 'templates/cache/' . $template . '/admin/';
        if ( !file_exists($admin_folder) )
        {
            @umask(0);
            if ( !@mkdir($admin_folder, 0777) )
            {
                //trigger_error('Could not create template cache dir: ' . $admin_folder . ' - chmod ' . str_replace('/admin/', '', $admin_folder) . ' to 777', E_USER_WARNING);
                unset($admin_folder);
            }
        }
	...
	function loadfile($handle)
	{
		...
		{
			trigger_error("Template->loadfile(): Error - file $handle - '".$this->files[$handle]."' does not exist or is empty", E_USER_ERROR);
		}
	...
	function compile_write(&$handle, $data)
	{
        ...
        {
            $handle_filename_array = split("/", $handle_filename);
	...
	function compile_cache_show(&$template)
	{
		...
		$dp = opendir($eqdkp_root_path . 'templates/cache/' . $template . '/');
		while ($file = readdir($dp))
		{
			if (strstr($file, '.html') && is_file($eqdkp_root_path . 'templates/cache/' . $template . '/' . $file))
			{
